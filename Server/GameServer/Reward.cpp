...
#ifdef	REWARD_RELOAD
void CRewardManager::ReloadTable() {

	initRewardDataInfo();
	m_dataList.clear();
	loadReward(&gserver->m_dbdata);
	LOG_INFO("Re-Load Rewards - Cloud");
}
#endif
...
bool CRewardManager::item2PC( CPC * _pCh, REWAD_DATA_INFO* _rewarddata)
{
 ...
 if (_rewarddata->index == 81 || _rewarddata->index == 82)
  		isLockedBox = true;
