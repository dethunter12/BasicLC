...
{
	// Chatting
	Add(MSG_CHAT, do_Chat, 0, true, true);			// ä��

	// �̵� ����
	Add(MSG_MOVE, do_Move, 0, false, false);			// �̵�
	Add(MSG_GO_ZONE, do_GoZone, 0, false, false);		// ���̵�
...
//N1InPersonalDungeon : bw
	...
	Add(MSG_AUTO_SETTING, do_SaveAutoOption, 0, false, false);	// Val add auto igni/pickup chat
#ifdef BATTLE_PASS_SYSTEM
	Add(MSG_BATTLE_PASS, do_BattlePass, 0, false, false);
#endif
#ifdef PREMIUM_CHAR
	Add(MSG_PREMIUM_CHAR, do_PremiumChar, 0, false, false);
#endif
#ifdef DOMINATION_EVENT_SYSTEM
	Add(MSG_DOMINATION_EVENT, do_DominationEvent, 0, false, false);
#endif
#ifdef PROMO_KEY_03312023
	Add(MSG_EX_PROMO, do_PromoKey, 0, false, false); //we're receiving this from the client and processing it in the server
#endif
}
...
void CCmdList::AddExMessage()
{
	...
	Add(MSG_EX_NAMECHANGE,	do_Ex_NameChange, 0, false, false);		// �̸� ���� ������	: tab_idx(uc) row_idx(uc) col_idx(uc) item_idx(n) name(str)
	...
	Add(MSG_EX_IDENTIFY_ITEMS_USE, do_IdentifyItems, 0, false, false);//CAPTCHA_1302023
#endif
