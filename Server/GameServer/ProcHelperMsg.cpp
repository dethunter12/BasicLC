...
void ProcHelperNameChange(CNetMsg::SP& msg)
{
	...
	if(bguild == 1)
	{
		...
		{
			CItem* item = pc->m_inventory.FindByDBIndex(item_idx, 0, 0);
			...
			if (item == NULL)
			{
...

				{
					//롤백
					CNetMsg::SP rmsg(new CNetMsg);
					HelperNameChangeReqMsg(rmsg, 3, pc->m_guildInfo->guild()->index(), pc->m_guildInfo->guild()->name(), item_idx);
			...
			{
				CNetMsg::SP rmsg(new CNetMsg);
				HelperNameChangeReqMsg(rmsg, 5, pc->m_guildInfo->guild()->index(), name, item_idx);
				SEND_Q(rmsg, gserver->m_helper);
			}
		...
		if (error == MSG_EX_NAMECHANGE_ERROR_SUCCESS)
		{
			...
			if (item == NULL)
			{
...

				{
					//롤백
					CNetMsg::SP rmsg(new CNetMsg);
					HelperNameChangeReqMsg(rmsg, 4, pc->m_index, pc->GetName(), item_idx);
					SEND_Q(rmsg, gserver->m_helper);
				}
...
void ProcHelperWhisperReq(CNetMsg::SP& msg, int seq, int server, int subno, int zone)
{
	int isItem, lSocketInfo, itemTotal, uniIndex, index, plus, plus2, flagMain, flag, used, used2, nowDurability, maxDurability, starPos, endPos, lOptionLevel, lOriginOptionVar;
	char wearing, sbOptionCount, sbOptionType;
	int sidx;
...
#pragma region SectionItem
			RefMsg(rmsg) << isItem;
			if (isItem)
			{
				RefMsg(msg) >> itemTotal;
				RefMsg(rmsg) << itemTotal;
				for (int i = 0; i < itemTotal; i++)
				{
					...
					RefMsg(rmsg) << uniIndex
						...
						<< starPos
						<< endPos;
					if (flagMain & ITEM_FLAG_ORIGIN)
					{
						RefMsg(msg) >> sbOptionCount;
						RefMsg(rmsg) << sbOptionCount;
						for (int k = 0; k < sbOptionCount; k++)
						{
							RefMsg(msg) >> sbOptionType
								>> lOptionLevel
								>> lOriginOptionVar;
							RefMsg(rmsg) << sbOptionType
								<< lOptionLevel
								<< lOriginOptionVar;
						}
						char sbBelong, sbSkillcont, sbSkillLevel;
						int lSkillIndex;
						RefMsg(msg) >> sbBelong
							>> sbSkillcont;
						RefMsg(rmsg) << sbBelong
							<< sbSkillcont;
						for (int k = 0; k < sbSkillcont; k++)
						{
							RefMsg(msg) >> lSkillIndex
								>> sbSkillLevel;
							RefMsg(rmsg) << lSkillIndex
								<< sbSkillLevel;
						}
					}
					else
					...
					RefMsg(msg) >> plus2;
					RefMsg(rmsg) << plus2;
					if (flagMain & ITEM_FLAG_SOCKET)
					{
						for (int k = 0; k < MAX_SOCKET_COUNT; k++)
						{
							RefMsg(msg) >> lSocketInfo;
							RefMsg(rmsg) << lSocketInfo;
						}
					}
				}
			}
...
void ProcHelperNewGuildMemberListRep( CNetMsg::SP& msg )
{
	...
	LONGLONG  cumulatePoint[GUILD_MAX_MEMBER]; //RomeoST Adjust
...
{
	int guildindex;
	LONGLONG guildpoint; //RomeoST Add
	char guildincline;
	int guildmaxmeber;
	int landcount;
	int land[256];
...
void ProcHelperNewGuildMemberList( CNetMsg::SP& msg )
{
	...
	LONGLONG cumulatePoint; //RomeoSt Adjust
	int channel;
	int zoneNum;
...
void ProcHelperNewGuildPointUpdateMsg( CNetMsg::SP& msg )
{
	int charindex;
	int guildindex;
	LONGLONG  guildpoint; //RomeoST Adjust
	RefMsg(msg) >> charindex
		>> guildindex
		>> guildpoint;
	CPC* pc = PCManager::instance()->getPlayerByCharIndex( charindex );
...
void ProcHelperNewGuildMemberPointSaveMsg(CNetMsg::SP& msg)
{
	int nCharIdx;
	int nGuildIdx;
	LONGLONG nMemberPoint; //ROmeoST Adjust
