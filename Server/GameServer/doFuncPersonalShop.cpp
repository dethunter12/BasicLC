...
void do_PersonalShopSellStart(CPC* ch, CNetMsg::SP& msg)
{
	...
	for (i = 0; i < normalCount; i++)
	{
		...
		{
			LOG_INFO("Personal Shop Error Item 1");
			CNetMsg::SP rmsg(new CNetMsg);
	...
	if (shoptype & PST_PACKAGE)
	{
		...
		for (i = 0; i < packcount; i++)
		{
			...
			if (ps->FindItem(true, itemindex) != -1)
			{
				LOG_INFO("Personal Shop Error Item 2");
				CNetMsg::SP rmsg(new CNetMsg);
				PersonalShopErrorMsg(rmsg, MSG_PERSONALSHOP_ERROR_DUPLICATION);
				SEND_Q(rmsg, ch->m_desc);
				return ;
			...
			{
				LOG_INFO("Personal Shop Error Item 3");
				CNetMsg::SP rmsg(new CNetMsg);
				PersonalShopErrorMsg(rmsg, MSG_PERSONALSHOP_ERROR_DUPLICATION);
				SEND_Q(rmsg, ch->m_desc);
				return ;
...
void do_PersonalShopBuy(CPC* ch, CNetMsg::SP& msg)
{
	...
	// 실시간 저장
	DBManager::instance()->SaveCharacterInfoSmall(ch->m_desc, false);
	DBManager::instance()->SaveCharacterInfoSmall(tch->m_desc, false);
