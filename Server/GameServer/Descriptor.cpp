...
bool CDescriptor::IncreaseHackCount(int n)
{
	...
	if (m_hackCount >= limitHackCount)
	{
		...
		MsgWindowLog(rmsgs, MSG_EX_STRING_OUTPUT_SYS, "[HACK] Exceed amount of warnings");
		SEND_Q(rmsgs, this);
		//GoZone(this->m	_pChar, 0, 0, 1113, 960);
		m_hackCount = 0;
		return false;
			...
			if (stage == 6)
			{
				...
				if (hacked)
				{
					...
					else
					{
						...
						char buff[255];
						sprintf(buff, "%s%d", "SkillHack:", bExtension);

						GAMELOG << init("HACKUSER EXTENSION", ch) << end;

						LOG_ERROR("HACKUSER EXTENSION user is dis.");
						this->Close(buff);
						GAMELOG << init("HACKUSER EXTENSION_LOG", ch->m_nick) << end;
				...
				//ANIIMATION DETECTION
				float fakeAnimation = ((100 - this->m_pChar->m_skillSpeed) / 100.0f);
				if (animSpeed < fakeAnimation && animSpeed < 0.05f)
				{
					GAMELOG << init("HACK_ANIMATION")
						<< "index " << delim
						<< this->m_pChar->m_index << delim
						<< "anim_id " << delim
						<< animId << delim
						<< "anim_speed " << delim
						<< animSpeed << delim
					...
					if (this->m_pChar->m_animation_hack < 3) {
						this->m_pChar->m_animation_hack++;
					}
					else
					{
						char buff[255];
						sprintf(buff, "%s%f", "HACK_ANIMATION:", animSpeed);


						GAMELOG << init("HACKUSER HACK_ANIMATION", ch) << end;

						LOG_ERROR("HACKUSER HACK_ANIMATION user is dis.");
						this->Close(buff);
						GAMELOG << init("HACKUSER HACK_ANIMATION-LOG", ch->m_nick) << end;
					}
				} else {
					this->m_pChar->m_animation_hack = 0;
				}

				//ATTACKSPEED DETECTION
				if (attackspeed != this->m_pChar->m_attackSpeed)
				{
					GAMELOG << init("HACK_SKILL_ATTACK")
						<< "client_speed " << delim
						<< (float)attackspeed << delim
						<< "server_speed " << delim
						<< (float)this->m_pChar->m_attackSpeed
						<< end;
					...
					else
					{
						...
						char buff[255];
						sprintf(buff, "%s%d", "Attack Speed Hack:", attackspeed);

						GAMELOG << init("HACKUSER Attack Speed Hack", ch) << end;

						LOG_ERROR("HACKUSER Attack Speed Hack user is dis.");
						this->Close(buff);
						GAMELOG << init("HACKUSER Attack Speed Hack", ch->m_nick) << end;
					}
				}
				else
				{
				...
				//SKILLCD DETECTION
				if (iCDvar != this->m_pChar->m_opDecreaseSkillDelay + this->m_pChar->m_skillCooltime) // m_skillCooltime)
				{
					...
					else
					{
						...
						char buff[255];
						sprintf(buff, "%s%d", "Cool Down Hack:", iCDvar);

						GAMELOG << init("HACKUSER Cool Down Hack", ch) << end;

						LOG_ERROR("HACKUSER Cool Down Hack user is dis.");
						this->Close(buff);
						GAMELOG << init("HACKUSER Cool Down Hack", ch->m_nick) << end;
				...
				if (iskillSpeed != this->m_pChar->m_skillSpeed)
				{
					...
					else
					{
						...
						char buff[255];
						sprintf(buff, "%f", "Skill Speed Hack:", iskillSpeed);

						GAMELOG << init("HACKUSER Skill Speed Hack", ch) << end;

						LOG_ERROR("HACKUSER Skill Speed Hack user is dis.");
						this->Close(buff);
						GAMELOG << init("HACKUSER Skill Speed Hack", ch->m_nick) << end;
				...
				//SKILL SPEED DETECTION 2
				float serverspeed = (100 - iskillSpeed) / 100.0f;
				if (ch->m_currentSkill != NULL)
				{
					if (skillspeed > 0 && skillspeed != serverspeed)
					{
						if (this->m_pChar->m_skillspeed_hack < 3)
						{
							this->m_pChar->m_skillspeed_hack++;
						...
						else
						{
							...
							char buff[255];
							sprintf(buff, "%s%f", "Skill Speed Hack2:", skillspeed);

							GAMELOG << init("HACKUSER Skill Speed Hack2", ch) << end;

							LOG_ERROR("HACKUSER Skill Speed Hack2 user is dis.");
							this->Close(buff);
							GAMELOG << init("HACKUSER Skill Speed Hack2", ch->m_nick) << end;
				...
				//SKILL SPEED DETECTION 3
				if (ch->m_currentSkill != NULL)
				{
					if (skillspeedfix > 0 && skillspeedfix != serverspeed)
					{
						if (this->m_pChar->m_skillspeed_hack < 3)
						{
							this->m_pChar->m_skillspeed_hack++;
						...
						else
						{
							...
							char buff[255];
							sprintf(buff, "%s%f", "Skill Speed Hack3:", skillspeedfix);

							GAMELOG << init("HACKUSER Skill Speed Hack3", ch) << end;

							LOG_ERROR("HACKUSER Skill Speed Hack3 user is dis.");
							this->Close(buff);
							GAMELOG << init("HACKUSER Skill Speed Hack3", ch->m_nick) << end;
				...
				float trunspeed2 = runspeed * 2 + 38.1f;
				...
				if (ch->m_pZone->m_index == ZONE_DRATAN_CASTLE_DUNGEON)
				{
					...
					if (pCastle->m_dvd.GetZone() && pCastle->m_dvd.GetEnvRate() >= 90)
					{
						...
						trunspeed2 = trunspeed2 + trunspeed2 * 0.1;
					}
				}
				...
				{
					GAMELOG << init("HACK_RUN_SPEED")
						<< "client_run_speed " << delim
						<< runspeed << delim
						<< "server_run_speed " << delim
						<< trunspeed << delim
						<< end;
					...
					else
					{
						...
						char buff[255];
						sprintf(buff, "%s%d", "RUNSPEED HACK:", runspeed, (this->m_pChar->m_pArea->m_index == ZONE_DRATAN_CASTLE_DUNGEON ? false : true));

						GAMELOG << init("HACKUSER RUNSPEED HACK", ch) << end;

						LOG_ERROR("HACKUSER RUNSPEED HACK user is dis.");
						this->Close(buff);
						GAMELOG << init("HACKUSER RUNSPEED HACK", ch->m_nick) << end;
...
void CDescriptor::sendData_StartAndMoveZone()
{
	...
	gserver->m_dominationEvent.SendUpdateInfo(m_pChar);
	#endif
}
