...
//	void nick(const char* str) { if (str) m_nick = str; else m_nick = ""; }
	...
	void cumulatePoint( LONGLONG  point ) //RomeoST Adjust
	{
		m_cumulatePoint = point;
	}
	void AddCumulatePoint( LONGLONG  point ) //RomeoST Adjust
	{
		m_cumulatePoint += point;
	}
	...
	LONGLONG GetcumulatePoint() //RomeoST ADjust
	{
...
class CGuild
{
	...
	int				m_landcount;
	int				m_land[256];					// 소유지
	LONGLONG		m_GuildPoint;			// RomeoST Adjust
...
public:
	...
	void guildPoint( LONGLONG guildPoint ) //RomeoST Adjust
	{
	...
	void AddGuildPoint( LONGLONG GuildPoint ); //RomeoST Adjust
	...
	LONGLONG  GetGuildPoint() //RomeoST ADjust
	{
		return m_GuildPoint;
	}
	char GetIncline()
