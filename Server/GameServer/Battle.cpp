...
int ProcAttack(CCharacter* of, CCharacter* df, MSG_DAMAGE_TYPE damageType, CSkill* skill, int magicindex, int damage)
{
	...
	std::vector<CJobSkillDamage*>::const_iterator cit;
	std::vector<CJobSkillDamage*>::const_iterator citEnd = gserver->m_job_skill_damage.end();
	CJobSkillDamage* pSkillDmg = NULL;
	float perc = 100.0;

	if (IS_PC(df) && IS_PC(of) && proto) {
		for (cit = gserver->m_job_skill_damage.begin(); cit != citEnd; cit++) {
			pSkillDmg = *cit;
			if ( proto->m_index == pSkillDmg->m_skill_index) {
				damage = (int)(damage * ((float)pSkillDmg->m_damage / perc));
				if (of->m_opPvpDamageRate > 0) //PvEDamageRate Increase add 3/9/2023
				{
					damage += damage * (of->m_opPvpDamageRate / 100.0f);				
				}
			}
		}
		
	}
	else if (IS_NPC(df) && IS_PC(of) && proto) 
	{ 
		CNPC* npc = TO_NPC(df);
		for (cit = gserver->m_job_skill_damage.begin(); cit != citEnd; cit++) 
		{
			...
			if ( proto->m_index == pSkillDmg->m_skill_index)
			 {
				damage = (int)(damage * ((float)pSkillDmg->m_damage_npc / perc));

				//if (gserver->isActiveEvent(A_EVENT_INCREASE_DAMAGE))
				//damage = damage + (damage * .5);
//
				if (of->m_opPveDamageRate > 0)
				{
					int beforeDamage = damage;
					damage += damage * (of->m_opPveDamageRate / 100.0f);
					// LOG_INFO("Regular Damage Before: %d : %d Damage PVE Rate :", beforeDamage, damage);
				}
				//if (npc->m_proto->IsBossInList() && of->m_reborn > 0 )
				// damage = damage + (damage * (of->m_reborn * .07));
			}
		}
	}
...
void ApplyDamage(CCharacter* of, CCharacter* df, MSG_DAMAGE_TYPE damageType, const CSkillProto* proto, const CMagicProto* magic, int damage, char flag, bool isCharge)
{
	...
	if (IS_PC(of) && IS_PC(df)) {
		...
		of->m_assist.CureAssist(SF_PVP_REMOVE);
#endif
#ifdef PVP_SKILL_REMOVAL_09262022
		df->m_assist.CureAssist(SF_PVP_REMOVE);
#endif
	}
...
int CalcNewDamage(CCharacter* of, CCharacter* df, int damage)
{
	if( IS_PC(of) && IS_PC(df) )
	{
		CPC* of_pc = TO_PC(of);
		CPC* df_pc = TO_PC(df);
		/* //dethunter12 pk penalty remove damage reduction
		if(of_pc->IsChaotic() || df_pc->IsChaotic())
		{
			//°ø°YAU A³¸®
			if(of_pc->IsChaotic() == true)
			{
				damage = damage - (damage * of_pc->m_pkPenalty / 200 / 100 * (-1.5));
			}
			else
			{
				damage = damage + (damage * of_pc->m_pkPenalty / 200 / 100 * 3.5); //dethunter12 1000 test
			}
		}
		*/
		//¹æ¾i±¸°­E­¿¡ μu¸? ¼oA¤
...
void CalcNewHitProb(CCharacter* of, CCharacter* df, float& hit, float& avoid)
{
	if( IS_PC(of) && IS_PC(df) )
	{
		...
		//A≪¿A ¸ðμa AI °æ¿iAC PVP E¸CC °¨¼O //dethunter12 pk penalty remove 
		/*if(df_pc->IsChaotic() == true)
		{
			avoid = avoid + ((df_pc->m_pkPenalty / (of_pc->m_level + df_pc->m_level)) - of_pc->m_level * 3); //dethunter12 test 
		}*/
	}
}
