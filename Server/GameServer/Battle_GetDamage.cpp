	...
	case MSG_DAMAGE_MELEE:
		{
			...
			{
				CPC* pc = TO_PC(of);
				if(!pc->m_currentSkill && pc->m_assist.m_avAddition.charge_attack && IS_NPC(df))
				{
	...
	case MSG_DAMAGE_MAGIC:
		{
			...
			{
				CPC* pc = TO_PC(of);
				if(!pc->m_currentSkill && pc->m_assist.m_avAddition.charge_magic && IS_NPC(df))
				{
...
//#endif
	...
	if (IS_PC(of) && IS_PC(df))
	{
		...
		if (magic == NULL) //auto damage
		{
				ret = ret - (ret * .44); //dethunter12 add 
		}
	}
...
char GetHitType_Adult(CCharacter* of, CCharacter* df, MSG_DAMAGE_TYPE type)
{
	...
	LONGLONG nHitrate = (LONGLONG)(fHitrate * 100); //1000->100
	LONGLONG nAvoid = (LONGLONG)(fAvoid * 100);//1000->100
