...
void do_SkillArenaHeal(CPC* ch, CNetMsg::SP& msg);
...
#ifdef ARTIFACT_INVIS_BLOCK11162022
		
		if (skill->m_proto->m_index == 131 && ArtifactManager::instance()->isOwnerPC(ch->m_index) == true)
		{
			throw MSG_SKILL_ERROR_CANNOT_ARTIFACT_INVIS;
		}
#endif
...
void do_SkillArenaHeal (CPC* ch, CNetMsg::SP& msg)
{
	if (ch->m_level < 18 || ch->m_pZone->m_index != ZONE_START || ch->GetPlayerState() == PLAYER_STATE_PKMODE || ch->GetPlayerState() == PLAYER_STATE_PKMODEDELAY)
	{
		//not correct level write to client message and exit function
		// 레벨제한으로 인한 버프를 받지 못함.
		CNetMsg::SP rmsg(new CNetMsg);
		ResponseClient::makeSkillSupportMsg(rmsg, MSG_SKILL_ARENA_HEAL, 0);
		SEND_Q(rmsg, ch->m_desc);
		return;
	}

	CSkill* pSkill = gserver->m_skillProtoList.Create(347, 2);									// SKILL INDEX
	if (pSkill == NULL)
	{
		LOG_ERROR("not found skill[%d]. charIndex[%d]", 1002, ch->m_index);
		return;
	...
	if (ch->CanApplySkill(pSkill->m_proto, pSkill->m_proto->Level(pSkill->m_level)))
	{
		...
		ApplySkill(ch, ch, pSkill, -1, bApply);
		// 버프 받음.
		//if we applied the skill send a message to the client stating Sucess:
		CNetMsg::SP rmsg(new CNetMsg);
		ResponseClient::makeSkillSupportMsg(rmsg, MSG_SKILL_ARENA_HEAL, 1);
		SEND_Q(rmsg, ch->m_desc);
	}
}
void do_RemoveBuffs( CPC* ch, CNetMsg::SP& msg )
