...
void do_GMUpdateEvent(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	gserver->m_eventAutomationMgr.reloadEvents();
}
void do_GMReloadJobDamage(CPC* ch, const char* arg, std::vector<std::string>& vec) 
{
...
void do_GMFunc(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
...
#ifdef REWARD_RELOAD
	else if (strcmp(tmpBuf, "reload_rewards") == 0)
	{
	gserver->m_rewardMgr.ReloadTable();//Ok
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, 0, "", "", "Rewards Reloading...");
		SEND_Q(rmsg, ch->m_desc);
	}
	}
#endif
#ifdef OPTION_RELOAD
	else if (strcmp(tmpBuf, "reload_options") == 0)
	{
	gserver->m_optionProtoList.ReloadTable();//Ok
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, 0, "", "", "Options Reloading...");
		SEND_Q(rmsg, ch->m_desc);
	}
	}
#endif
#ifdef LUCKDRAW_RELOAD
	else if (strcmp(tmpBuf, "reload_luckdrawn") == 0)
	{
	gserver->m_LuckyDrawBox.ReloadTable();//Ok
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, 0, "", "", "Luckdrawn Reloading...");
		SEND_Q(rmsg, ch->m_desc);
	}
	}
#endif
	//reload exp table
	else if (strcmp(tmpBuf, "reload_exp") == 0)
	{
	InitExp();//Ok
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, 0, "", "", "Exp Reloading...");
		SEND_Q(rmsg, ch->m_desc);
	}
	}
#ifdef BATTLE_PASS_SYSTEM
	else if (strcmp(tmpBuf, "reset_bp") == 0)
	{
	...
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, 0, "", "", "Battle Pass Reseted");
		SEND_Q(rmsg, ch->m_desc);
	}
	}

	else if (strcmp(tmpBuf, "reload_bp") == 0)
	{
	gserver->LoadBattlePass();
	{
		CNetMsg::SP rmsg(new CNetMsg);
		SayMsg(rmsg, MSG_CHAT_GM, 0, "", "", "Battle Pass Reloaded");
		SEND_Q(rmsg, ch->m_desc);
	}
	}
	else if (strcmp(tmpBuf, "addbp") == 0)
	{
	int addbp = 0;
	arg = AnyOneArg(arg, tmpBuf);
	addbp = atoi(tmpBuf);
	if (addbp < 0 || addbp > 1000000) // 0º¸´U AU°A³ª, 1,000,000º¸´U Aⓒ¸e return
		return;

	ch->m_battlePassPoints += addbp;
	{
		CNetMsg::SP rmsg(new CNetMsg);
		rmsg->Init(MSG_EXTEND);
		RefMsg(rmsg) << MSG_EX_STRING
...
void do_GM_guildPoint(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	...
	LONGLONG guildpoint = (LONGLONG)ATOLL(vec[1].c_str()); //RomeoST ADjust
...
void do_spawnNPC(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	char tmpBuf[MAX_MESSAGE_SIZE] = { 0, };
...
void do_debug(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	// assert parameters
	if (!ch)
		return;
	...
	// set debug mode
	ch->m_bDebugMode = !ch->m_bDebugMode;
	...
	if (ch->m_bDebugMode)
		RefMsg(rmsg) << (unsigned char)MSG_ARTIFACT_DEBUG_ON;
	else
		RefMsg(rmsg) << (unsigned char)MSG_ARTIFACT_DEBUG_OFF;
	SEND_Q(rmsg, ch->m_desc);
}

void do_banPlayer(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	if (arg == NULL)
		return;
	...
	LOG_INFO("Ban player %s", kickedName.c_str());

	int userCode;
	std::string query;
	{
		query = boost::str(boost::format("SELECT a_user_index FROM t_characters WHERE a_nick like '%s'") % kickedName.c_str());
		...
		LOG_INFO("Found user %d", cmd.m_nrecords);

		cmd.GetRec("a_user_index", userCode);
	}

	LOG_INFO("Set blocked user %d", userCode);

	{
		query = boost::str(boost::format("UPDATE bg_user SET blocked = 1 WHERE user_code = %d") % userCode);
		LOG_INFO("Query %s", query.c_str());
		CDBCmd cmd;
...
void do_GMDominationEvent(CPC* ch, const char* arg, std::vector<std::string>& vec)
{
	CNetMsg::SP rmsg(new CNetMsg);
	if (vec.size() > 0) 
	{
		if (vec[0] == "open") 
		{
			gserver->m_dominationEvent.Open();
		}
		else if (vec[0] == "stop") 
		{
			gserver->m_dominationEvent.Stop();
		}
	}
}
#endif