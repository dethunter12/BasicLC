...
void DBManager::_SaveCharaterQuery(std::vector<std::string> & vec, CPC* pChar, int table_no)
{
	...
	updateQuery += boost::str(boost::format(", a_color_name = %d") % pChar->m_color_name);
	...
	updateQuery += boost::str(boost::format(", a_job=%d") % (int)pChar->m_job);
	updateQuery += boost::str(boost::format(", a_job2=%d") % (int)pChar->m_job2);
	updateQuery += boost::str(boost::format(", a_wins=%d") % pChar->m_wins);
	updateQuery += boost::str(boost::format(", a_loses=%d") % pChar->m_loses);
#ifdef CUBE_RANKING_12202022
	updateQuery += boost::str(boost::format(", a_cube_point=%d") % pChar->m_CubePoints);
	updateQuery += boost::str(boost::format(", a_cube_pc_kill=%d") % pChar->m_CubePlayerKill);
	updateQuery += boost::str(boost::format(", a_cube_boss_kill=%d") % pChar->m_CubeBossKill);
	updateQuery += boost::str(boost::format(", a_cube_point_daily=%d") % pChar->m_CubePointDaily);
#endif
	...
	updateQuery += boost::str(boost::format(", a_battle_pass_level = %1%") % pChar->m_battlePassLevel);
	updateQuery += boost::str(boost::format(", a_battle_pass_points = %1%") % pChar->m_battlePassPoints);
	updateQuery += boost::str(boost::format(", a_battle_pass_premium = %1%") % pChar->m_battlePassPremium);
#endif
#ifdef GLADES_RANKING_04182023
	updateQuery += boost::str(boost::format(", a_glade_kills=%d") % pChar->m_gladeKillPts);
#endif
	...
	updateQuery += boost::str(boost::format(", a_premium_phoenix_active=%d") % pChar->m_bPremium_Phoenix);
	...
	updateQuery += boost::str(boost::format(", a_exp_weekly=%llu") % pChar->m_exp_weekly);
...
void DBManager::_SaveCharacterMemposQuery(std::vector<std::string> & vec, CPC* pChar)
{
	...
	insertQuery = boost::str(boost::format(
								 ...
								 "a_mem_8, a_mem_9, a_mem_10, a_mem_11, a_mem_12, a_mem_13, a_mem_14,a_mem_15,a_mem_16,a_mem_17,a_mem_18,a_mem_19,a_mem_20,a_mem_21,a_mem_22,a_mem_23,a_mem_24,a_mem_25,a_mem_26,a_mem_27,"
								 "a_mem_28,a_mem_29,a_mem_30,a_mem_31,a_mem_32,a_mem_33,a_mem_34,a_mem_35,a_mem_36,a_mem_37,a_mem_38,a_mem_39,a_mem_40,a_mem_41,a_mem_42,a_mem_43,a_mem_44,a_mem_45,a_mem_46,a_mem_47,a_mem_48,a_mem_49) VALUES (%d") % pChar->m_index);
...
void DBManager::SaveCharacterInfoSmall(CDescriptor* d, bool disconnect)
{
	save_char_queryt_t* pushdata = new save_char_queryt_t;
	for (int index = 0; index < CHAR_SAVE_TYPE_TCHAR_MAX; ++index)
	{
	...
	GAMELOG << init("CHAR_UPDATE SMALL", d->m_pChar->m_name, d->m_pChar->m_nick, d->m_idname);
	GAMELOG << d->m_pChar->m_inventory.getMoney();
	GAMELOG << end;
}
