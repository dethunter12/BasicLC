...
{
	if(wearPos == 11 ) 
		wearPos = 7; //dethunter12 adjust 12/20/2022
	//��ȿ�� �˻�
...
{
	if( wearPos == 11) wearPos = 7;//dethunter12 adjust 12/20/2022
...
bool CWearInvenManager::CanDressNormal(CItem* item, int wearPos)
{
	...
	if (item->m_itemProto->getItemWearing() != wearPos)
	{
		...
		if (item->IsAccessary())
		{
			...
			if(_owner->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY1] == NULL &&
				...
				wearPos = WEARING_SPECIAL_ACCESSORY1;

			if (_owner->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY2] == NULL &&
				item->m_itemProto->getItemWearing() == WEARING_SPECIAL_ACCESSORY2)
				wearPos = WEARING_SPECIAL_ACCESSORY2;

			if (_owner->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY3] == NULL &&
				item->m_itemProto->getItemWearing() == WEARING_SPECIAL_ACCESSORY3)
				wearPos = WEARING_SPECIAL_ACCESSORY3;

			if (_owner->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY4] == NULL &&
				item->m_itemProto->getItemWearing() == WEARING_SPECIAL_ACCESSORY4)
				wearPos = WEARING_SPECIAL_ACCESSORY4;
	...
	if (item2 != NULL && item2->m_itemProto->getItemWearing() != wearPos)
	{
		...
		if (item2->IsAccessary())
		{
			...
			else if (item2->m_itemProto->getItemWearing() == WEARING_SPECIAL_ACCESSORY2 && wearPos != WEARING_SPECIAL_ACCESSORY2) {
				CNetMsg::SP rmsg(new CNetMsg);
				SysMsg(rmsg, MSG_SYS_CANNOT_WEAR);
				SEND_Q(rmsg, _owner->m_desc);
				_lastError = ResponseClient::WEAR_ERR_CANNOT_WEAR;
			...
			else if (item2->m_itemProto->getItemWearing() == WEARING_SPECIAL_ACCESSORY3 && wearPos != WEARING_SPECIAL_ACCESSORY3) {
				CNetMsg::SP rmsg(new CNetMsg);
				SysMsg(rmsg, MSG_SYS_CANNOT_WEAR);
				SEND_Q(rmsg, _owner->m_desc);
				_lastError = ResponseClient::WEAR_ERR_CANNOT_WEAR;
			...
			else if (item2->m_itemProto->getItemWearing() == WEARING_SPECIAL_ACCESSORY4 && wearPos != WEARING_SPECIAL_ACCESSORY4) {
				CNetMsg::SP rmsg(new CNetMsg);
				SysMsg(rmsg, MSG_SYS_CANNOT_WEAR);
				SEND_Q(rmsg, _owner->m_desc);
				_lastError = ResponseClient::WEAR_ERR_CANNOT_WEAR;
...
//ADD
	...
	{
		CItem* item2 = _owner->m_wearInventory.getWearItem(COSTUME2_WEARING_WEAPON);
		CItem* item3 = _owner->m_wearInventory.getWearItem(COSTUME2_WEARING_SHIELD);

		if (item2 != NULL)
		{
			if (item->m_itemProto->getItemSubTypeIdx() != item2->m_itemProto->getItemSubTypeIdx())
			{
				CNetMsg::SP rmsg(new CNetMsg);
				MsgWindowLog(rmsg, MSG_EX_STRING_OUTPUT_WINDOW, "Invalid item type C2");
				SEND_Q(rmsg, _owner->m_desc);
				return;
			}
		}
		...
		if (item3 != NULL) 
		{
			if (item->m_itemProto->getItemSubTypeIdx() != 0)
			{
				CNetMsg::SP rmsg(new CNetMsg);
				MsgWindowLog(rmsg, MSG_EX_STRING_OUTPUT_WINDOW, "Invalid item type C2");
				SEND_Q(rmsg, _owner->m_desc);
	...
	if (wearPos == WEARING_SHIELD)
	{
		CItem* item2 = _owner->m_wearInventory.getWearItem(COSTUME2_WEARING_SHIELD);
...
bool CWearInvenManager::CanDressCostum(CItem* item, int wearPos)
{
	...
	*/
	...
	{
		if(wearPos == 7 ){//dethunter12 adjust 12/20/2022
...
{
	if(wearPos == 11) //dethunter12 adjust 12/20/2022
		wearPos = 7;
    ...
    if (wearPos == WEARING_WEAPON)
	{
		CItem* item2 = _owner->m_wearInventory.getWearItem(WEARING_WEAPON);
		...
		if (item2 != NULL) 
		{
			...
			{
				CNetMsg::SP rmsg(new CNetMsg);
				MsgWindowLog(rmsg, MSG_EX_STRING_OUTPUT_WINDOW, "Invalid item type");
				SEND_Q(rmsg, _owner->m_desc);
				return;
			}
		}
		...
		{
			CNetMsg::SP rmsg(new CNetMsg);
			MsgWindowLog(rmsg, MSG_EX_STRING_OUTPUT_WINDOW, "Invalid item type equipe a weapon first");
			SEND_Q(rmsg, _owner->m_desc);
			return;
		}
	}
	...
	if (wearPos == WEARING_SHIELD)
	{
		CItem* item2 = _owner->m_wearInventory.getWearItem(WEARING_SHIELD);
		...
		{
			CNetMsg::SP rmsg(new CNetMsg);
			MsgWindowLog(rmsg, MSG_EX_STRING_OUTPUT_WINDOW, "Invalid item type equip a shield first");
			SEND_Q(rmsg, _owner->m_desc);
			return;
		}
	}
