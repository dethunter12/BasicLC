				...
				if (castle && castle->GetState() != WCSF_NORMAL)
				{
					...
					{//liquid mcs
						if ((IS_DEFENSE_TEAM(ch->GetJoinFlag(ch->m_pZone->m_index)) && IS_ATTACK_TEAM(target->GetJoinFlag(ch->m_pZone->m_index))) ||
							(IS_ATTACK_TEAM(ch->GetJoinFlag(ch->m_pZone->m_index)) && IS_ATTACK_TEAM(target->GetJoinFlag(ch->m_pZone->m_index))) ||
							(IS_ATTACK_TEAM(ch->GetJoinFlag(ch->m_pZone->m_index)) && IS_DEFENSE_TEAM(target->GetJoinFlag(ch->m_pZone->m_index))) )
							return true;
...
void CPC::ApplyTitle()
{
	...
	for(int i = 0; i < MAX_TITLE_OPTION; i++)
	{
		...
		case OPTION_INCREASE_HP:
			m_maxHP += nvalue;
			break;
		case OPTION_CASH_ONLINE_BONUS:
			m_opOnlineTimeAdd += nvalue;
			break;
		case OPTION_DAMAGE_INCREASE_PVE:
			m_opPveDamageRate += nvalue;
			break;
		case OPTION_NAS_UP_RATE:
			m_opNasRate += nvalue;
			break;
		case OPTION_DAMAGE_INCREASE_PVP: //dethunter12 add 3/9/2023
			m_opPvpDamageRate += nvalue;
			break;
		default:
			break;
		}
