...
public:
	...
	void	CheckEventTime();
	void 	IncreaseDead();
	void 	ProcDrop(CPC *pc, CNPC* npc);
	void	SummonAll();
	bool	CheckNpc(int index) { return index == m_npc_idx; }
...
class CBattlePass
{
	...
	void addReward(BATTLE_PASS_REWARD* reward) {
	...
	std::string m_name;
	std::vector<BATTLE_PASS_REWARD*> m_rewards;
};

typedef std::map<int, CBattlePass*> map_battle_pass_t;
...
typedef std::vector<CJobSkillDamage*> job_skill_damage_t;
#ifdef CUBE_RANKING_12202022
typedef std::vector<CCubeRankReward> vec_listCubeRank_t; //Modify Original CCubeRankReward*
#endif
#ifdef GLADES_RANKING_04182023
typedef std::vector<CGladesRankReward> vec_listGladesRank_t; //Glades Rank Reward
#endif
...
#define STATISTICS(x, v)		gserver->m_statistics##x += (v)
	...
	job_skill_damage_t	m_job_skill_damage;
#ifdef CUBE_RANKING_12202022
	vec_listCubeRank_t	m_list_cube_rank;
#endif
#ifdef GLADES_RANKING_04182023
	vec_listGladesRank_t m_top_glades_ranking;
#endif
	//////////////
	// NPC 관련
...
//0704
	...
	bool bIsSendTop4Completed;//dethunter12 test
...
public:
	...
	bool ReloadJobDamage();

	void ModifyDungeonTime(int flag);
	void CheckDungeonTime();
	void CheckUpdateRanking();
#ifdef CUBE_RANKING_12202022
	bool SendTop4CubeRewards();
	void ResetCubeRank();
	void CheckCubeRankingTop();
#endif
#ifdef GLADES_RANKING_04182023
	bool SendTop4GladesRewards();
	void ResetGladesRank();
	void CheckGladesRankingTop();
#endif
...
#endif
	#ifdef DOMINATION_EVENT_SYSTEM
	DominationEvent m_dominationEvent;
	#endif

	#ifdef PROMO_KEY_03312023
	cPromoKey m_promoKey;
	#endif
