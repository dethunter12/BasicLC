	...
	case MSG_CHAR_PC:
		{
			...
			packet->color_name = ch->m_color_name;
			packet->a_wins = ch->m_wins;
			packet->a_loses = ch->m_loses;
			...
			// newslot - alanssoares
			if (ch->m_wearInventory.wearItemInfo[WEARING_ACCESSORY4 + 1])
			...
			if (ch->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY1 + 1])
				...
				packet->accessory[4] = -1;

			if (ch->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY2 + 1])
				packet->accessory[5] = ch->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY2 + 1]->getDBIndex();
			else
				packet->accessory[5] = -1;

			if (ch->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY3 + 1])
				packet->accessory[6] = ch->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY3 + 1]->getDBIndex();
			else
				packet->accessory[6] = -1;

			if (ch->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY4 + 1])
				packet->accessory[7] = ch->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY4 + 1]->getDBIndex();
			else
				packet->accessory[7] = -1;

			// newslot - alanssoares
			if (ch->m_wearInventory.wearItemInfo[WEARING_SPECIAL_ACCESSORY5 + 1])
			...
			packet->expedLabel = ch->GetExpedLabel();		//������ ǥ��(PC)

			// ���� ��ų�� ���� �ִٸ�, ���� �ð��� ��ٸ�
			if (ch->m_currentSkill && ch->m_currentSkill->m_state == SKILL_CON_FIRE)
			{
				// AppearMsg�� �������Ƿ� ������ �ʴ� üũ���� skillfireMsg�� ������.
	...
	case MSG_CHAR_NPC:
		{
			...
			packet->mapAttr = ch->GetMapAttr();
			packet->expedLabel = ch->GetExpedLabel();		//������ ǥ��(NPC)
...
void TargetClearMsg(CNetMsg::SP& msg, int targetIndex)
{
	...
	packet->targetIndex = targetIndex;				//targetIndex�� -1�̸� ���� ��� �ִ� Ÿ���� �������ʰ� ������ ���� ��Ų��.
...
// NPC ���ݷ� ���� ǥ�ô� ������ �մϴ�.
	...
	if (ch->m_admin < 10)
	{
		...
		if (ch->GetMapAttr() & MATT_FREEPKZONE || ch->GetMapAttr() & MATT_WAR || ch->IsSetPlayerState(PLAYER_STATE_PKMODE) == true)
		{
			if (runSpeed > 25.0f)
				runSpeed = 25.0f;//Cloud Move Speed

			if (flySpeed > 25.0f)
				flySpeed = 25.0f;//Cloud Move Speed
		}
		else
		{
			if (runSpeed > PC_MAX_RUNSPEED)
				runSpeed = PC_MAX_RUNSPEED;//Cloud Move Speed

			if (flySpeed > PC_MAX_RUNSPEED)
				flySpeed = PC_MAX_RUNSPEED;//Cloud Move Speed
		}
	}
	...
	///////////////////////////////////////////////////////
	float avoid_magic_org = 0.0, avoid_magic = 0.0;			// ����ȸ��
	float avoid_melee_org = 0.0, avoid_melee = 0.0;			// ����ȸ��
	float fhitrate_magic_org = 0.0, fhitrate_magic = 0.0;	// ��������
	float fhitrate_melee_org = 0.0, fhitrate_melee = 0.0;	// ��������
	...
	packet->attack_diff = attack_diff; // (������)
	packet->magic = magic;
	packet->magic_diff = magic_diff; // (������)
	packet->defense = defense;
	packet->defense_diff = defense_diff; // (������)
	packet->resist = resist;
	packet->resist_diff = resist_diff; // (������)
	packet->avoid_melee = avoid_melee;	// ����ȸ��(����)
	packet->avoid_melee_org = avoid_melee_org; 	// ����ȸ��(�߰��Ǵ°� �� ��)
	packet->avoid_magic = avoid_magic;	// ����ȸ��(����)
	packet->avoid_magic_org = avoid_magic_org; // ����ȸ��(�߰��Ǵ°� �� ��)
	...
	if( ch->IsSetPlayerState(PLAYER_STATE_FLYING))
	{
		...
		packet->run_or_fly_speed_org = ch->m_flySpeed; // (�߰��Ǵ°� �� ��)
	}
	else
	{
		packet->run_or_fly_speed = runSpeed;
		packet->run_or_fly_speedfix = runSpeed * 2 + 38.1f;//Cloud ant-hack																	   
		packet->run_or_fly_speed_org = ch->m_runSpeed_org; // (?????��? ?? ??)
	}
	...
	packet->autopickup = ch->m_autopickup;
	packet->autoigni = ch->m_autoigni;
	packet->color_name = ch->m_color_name;
	packet->a_wins = ch->m_wins;
	packet->a_loses = ch->m_loses;
	packet->opResistStone = ch->m_opResistStone;
	packet->opResistSturn = ch->m_opResistSturn;
	packet->opResistSilent = ch->m_opResistSilent;
	packet->opabsorbPVPDamageRate = ch->m_absorbPVPDamageRate;
	packet->opOnlineTimeAdd = ch->m_opOnlineTimeAdd;
	packet->opPveDamageRate = ch->m_opPveDamageRate;
	packet->opNasRate = ch->m_opNasRate;
#ifdef CUBE_RANKING_12202022
	packet->cubePoints = ch->m_CubePoints;
	packet->cubePlayerKill = ch->m_CubePlayerKill;
	packet->cubeBossKill = ch->m_CubeBossKill;
#endif
#ifdef GLADES_RANKING_04182023
	packet->gladePoints = ch->m_gladeKillPts;
#endif

	packet->opPvpDamageRate = ch->m_opPvpDamageRate; //dethunter12 add 3/9/2023
...
void ItemPublicMsg(CNetMsg::SP& msg, CItem * pItem, bool bOption)
{
	...
	if (bOption)
	{
		if (pItem->IsOriginItem())
		{
			...
			// ��ų
			RefMsg(msg) << (char)pItem->GetBelongType()					// �ͼ� ����
...
void ItemOpenBoxRepMsg(CNetMsg::SP& msg, char result)
{
	msg->Init(MSG_ITEM);
	RefMsg(msg) << (unsigned char)MSG_ITEM_OPEN_BOX_REP
		<< result;
}
...
void HelperNameChangeReqMsg(CNetMsg::SP& msg, char bguild, int index, const char* reqname, int itemIdx)
{
...
void MsgWindowLog(CNetMsg::SP& msg, MSG_EX_STRING_Output_TYPE Type, const char* message)
{
	msg->Init(MSG_EXTEND);
	RefMsg(msg) << MSG_EX_STRING
		<< (unsigned char)Type
		<< 7999 //the string that sends messages!
		<< 1
		<< 3
		<< (const char*)message;
}
	...
	else if (ch->m_teachType == MSG_TEACH_STUDENT_TYPE)
	{
		...
		if (ch->m_teachIdx[0] != -1)
		{
			...
			if(tpc)
			{
				...
				// �ڽ��� �л��̸� idx[1]�� ������ ������ġ��, idx[2]�� ������ �缺���� �ʺ���, time[1]�� �Ϸ��ο�, time[2]�� �����ο����� ����.
...
void ExAPetStatusMsg(CNetMsg::SP& msg, CAPet* pet)
{
	...
	// ����� �ʿ��� ǥ�úκ�
	int nAttack = pet->m_nAttack + nEqAttack;	// ���ݷ�
...
void GuildPointInfo(CNetMsg::SP& msg, LONGLONG guildpoint) //RomeoST Adjust
{
	msg->Init(MSG_GUILD);
...
void GuildMemberListRep( CNetMsg::SP& msg, int membercount, int* membercharindex, LONGLONG* cumulatePoint,  const char CharName[][MAX_CHAR_NAME_LENGTH  + 1], const char PositionName[][GUILD_POSITION_NAME+1],  char* job, char* job2, int* level, int* position, CGuild* guild, int* logout_date )
{
	msg->Init( MSG_GUILD );
...
void HelperSaveGuildPointMsg(CNetMsg::SP& msg, int guildindex, LONGLONG  guildpoint ) //Adjust RomeoST
{
	msg->Init( MSG_HELPER_COMMAND );
...
//Adjust RomeoST
void HelperSaveGuildMemberPointMsg( CNetMsg::SP& msg, int guildindex, int charindex, LONGLONG  GuildMemberPoint )
{
	msg->Init( MSG_HELPER_COMMAND );
...
void HelperNewGuildPointUpdate( CNetMsg::SP& msg, int charindex, int guildindex, LONGLONG  point ) //RomeoST ADjust
{
	msg->Init( MSG_HELPER_COMMAND );
...
void CostumeWearingMsgError(CNetMsg::SP & msg, char type)
{
	msg->Init(MSG_EXTEND);
	RefMsg(msg) << MSG_EX_COSTUME2
		<< (unsigned char)MSG_EX_COSTUME2_SYSTEM_ERROR
		<< type;
}
...
void prepareSuitMsg(CNetMsg::SP& msg, CPC* ch)
{
	...
	for(int i = 0, j = 0; i < MAX_COSTUME_WEARING; i++)
	{
		...
		{//dethunter12 adjust 12/20/2022
			if (COSTUME2_WEARING_START + i == COSTUME2_WEARING_WEAPON) //RomeoST
				continue;
...
void SysMsgEventSummonNpcDead(CNetMsg::SP& msg, CLCString of, int totalLeft)
{
	msg->Init(MSG_SYS);
	RefMsg(msg) << (unsigned char)MSG_SYS_EVENT_SUMMON_NPC_DEAD
				<< of 
				<< totalLeft;
}
void SysMsgPlayerDead(CNetMsg::SP& msg, CLCString of, CLCString df)
{
	msg->Init(MSG_SYS);
	RefMsg(msg) << (unsigned char)MSG_SYS_PLAYER_DEAD
		<< of << df;
}
...
{
	// command 0:��û�ð��Դϴ�. 1:5�г��ҽ��ϴ�. 2:����Ǿ����ϴ�.
	msg->Init(MSG_EXTEND);
...
{
	// (s->c) (uc)command 0: ���� ����ڰ� �ƴմϴ�, 1:�̹� ������ �޾ҽ��ϴ�.
	msg->Init(MSG_EXTEND);
	RefMsg(msg) << MSG_EX_ROYAL_RUMBLE
...
void SendMobAggressiveStatus(CNetMsg::SP& msg, CNPC* mob, bool isAggressive) //dethunter12
{
	msg->Init(MSG_EXTEND);
	RefMsg(msg) << (int)MSG_EX_SET_AGGRESSIVE_MOB
		<< mob->m_index
		<< (int)isAggressive;
}
#ifdef PROMO_KEY_03312023
void PromoErrorMsg(CNetMsg::SP& msg, MSG_PROMO_TYPE errortype)
{
	msg->Init(MSG_EXTEND);
	RefMsg(msg) << MSG_EX_PROMO
		<< (unsigned char)MSG_EX_SOCKET_ERROR
		<< (char)errortype;
}
#endif