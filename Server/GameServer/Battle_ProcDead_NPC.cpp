...
void ProcDeadGlades(CPC* op, CNPC* df);
#endif
...
//#endif
	...
	if (df->m_pZone != NULL && df->m_pZone->IsExtremeCube() && (df->m_proto->m_index != 529 || df->m_proto->m_index != 527))
	{
		int cubePoints = 0;
		...
		if (cube)
		{
			switch (df->m_proto->m_index)
			{
			case 833:
				cubePoints += 100;
				opc->m_CubeBossKill += 1;
				break;
			default:
				cubePoints += 1;
				break;
			...
			//Assign the points before the boost
			opc->m_CubePointDaily += cubePoints; //assign points to the daily ranking 

			if (gserver->isActiveEvent(A_EVENT_DOUBLE_CUBE))
				cubePoints *= 2;

			if (opc->m_assist.FindByItemIndex(17190))
			{ //double points then cure afterwords
				cubePoints *= 2;
				opc->m_assist.CureByItemIndex(17190);
			}

			if (opc->m_assist.FindByItemIndex(17189))
				cubePoints *= 2;
			...
			opc->m_CubePoints += cubePoints; // assign the points to the player.
			opc->CalcStatus(true);
			//Todo Write Log displaying how many points are added just copy cash ticket and modify.
			LOG_INFO("Player Cube Points %d", opc->m_CubePoints);
		}
	}
#endif
	...
	if (df && opc)
	{
		...
		for (; it != endit; ++it)
		{
			...
			if (affinity)
			{
				...
				if (opc->m_assist.FindByItemIndex(15197)) {
					point = point * 4;
					affinity->AddPoint(point, opc, bonus);
				}
				//dethunter12 add 3/5/2023
				else if (opc->m_assist.FindByItemIndex(17310)) {
					point = point * 6;
					affinity->AddPoint(point, opc, bonus);
				}
				else {
		...
		if (opc->m_assist.FindByItemIndex(15197)) {
			opc->m_assist.CureByItemIndex(15197);
		}
		//dethunter12 add 3/5/2023
		else if (opc->m_assist.FindByItemIndex(17310)) {
			opc->m_assist.CureByItemIndex(17310);
		}
	}
...
#ifdef GLADES_RANKING_04182023
	if (opc && df && opc->m_pZone->m_index == ZONE_THE_GLADES)
		ProcDeadGlades(opc, df);
#endif
...
{
	if (opc->m_battlePassLevel == MAX_BATTLE_PASS_LEVEL)
		return;
	
	CBattlePass * bp = gserver->FindBattlePassByLevel(opc->m_battlePassLevel + 1);
	if (bp == NULL)
		return;
	
	int calculatedPoints = 10 * df->m_level + 400;
	if (opc->m_assist.FindByItemIndex(17311)) //add battle pass booster here...
	{
		calculatedPoints *= 2;
		opc->m_assist.CureByItemIndex(17311);
	}

	if (opc->m_assist.FindByItemIndex(17313))
		calculatedPoints *= 2;
	
	if (bp->m_points >= opc->m_battlePassPoints + calculatedPoints)
		opc->m_battlePassPoints += calculatedPoints;
	else
		opc->m_battlePassPoints = bp->m_points;

	{
		...
		RefMsg(rmsg) << MSG_EX_STRING
			<< (unsigned char)MSG_EX_STRING_OUTPUT_SYS
			<< 100517//string idx create a string"blabla %d "
			<< 1//arg1
			<< 1//type int
			<< calculatedPoints;//int data1
		SEND_Q(rmsg, opc->m_desc);
	}
...
#ifdef GLADES_RANKING_04182023
void ProcDeadGlades(CPC* opc, CNPC* df)
{
	if (!opc) //if their trying to be bad and sending a null value
		return;

	if (opc->m_pZone->m_index != ZONE_THE_GLADES) // we want to ensure the user is actually in the correct zone to receive the points
		return;

	int killPts = 0;

	switch (df->m_index) //assigning points based off NPC ID's
	{
	case 2189:
	case 2122:
	case 2130:
	...
	case 2202:
		killPts = 25;
		break;
	default: //if not any of these mobs then 1 point
		killPts = 1;
	}

	if (killPts != 0) // we'll send the point message if the points arent equal to 0
	{
		CNetMsg::SP rmsg(new CNetMsg);
		rmsg->Init(MSG_EXTEND);
		RefMsg(rmsg) << MSG_EX_STRING
			<< (unsigned char)MSG_EX_STRING_OUTPUT_SYS
			<< 100532//string idx create a string"blabla %d "
			<< 1//arg1
			<< 1//type int
			<< killPts;//int data1
		SEND_Q(rmsg, opc->m_desc);
	...
	opc->m_gladeKillPts += killPts; //finally we're assigning the points to the actual players killPts;
