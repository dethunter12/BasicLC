...
bool GivePremiumPhoenixItem(CPC* ch, CNetMsg::SP& msg)
{
	...
	if ((pitem12107 = ch->m_inventory.FindByDBIndex(15213, 0, 0)) == NULL)
	{
		CNetMsg::SP rmsg(new CNetMsg);
	...
	CItem* pItem = gserver->m_itemProtoList.CreateItem(16004, -1, 0, 0, 1); // 
	...
	if(pSkill1 != NULL)
	{
		...
		if(bApply == false)
		{
			GAMELOG << init("EVENT_PHOENIX SKILL FAILED ") << ch->m_name << end;// ??? ???? ????
		}
	}
...
void do_EventPhoenix( CPC* ch, CNetMsg::SP& msg )
{
	...
	else if (ch->m_inventory.FindByDBIndex(15213))
	{
	...
	if (ch->m_inventory.FindByDBIndex(3217, 0, 0) == NULL && ch->m_inventory.FindByDBIndex(15213, 0, 0) == NULL )
	{
		CNetMsg::SP rmsg(new CNetMsg);
		EventPhoenixMsg(rmsg, MSG_EVENT_PHOENIX_ERROR_REQUIREMENT);
		SEND_Q(rmsg, ch->m_desc);
...
void do_EventPhoenix_Success( CPC* ch, CNetMsg::SP& msg )
{
	...
	if (ch->m_inventory.FindByDBIndex(15213))
	{
	...
	{
		{ 3218, 3262, 3006, 3221, 3222, 3231, 3232, 3233, 3234, 3235 },		// ????
		{ 3218, 3262, 3007, 3219, 3220, 3236, 3237, 3238, 3239, 3240 },		// ?????
