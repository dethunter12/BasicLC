...
#define ARTIFACT_201605_SAFETIME			900
...
void ArtifactManager::_proc()
{
	...
	for (int i = 0; i < _vec.size();)
	{
		...
		if (pArtifact->remaining <= 0)
		{
			...
			CNetMsg::SP rmsg(new CNetMsg);
			rmsg->Init(MSG_ARTIFACT);
			RefMsg(rmsg)<< (unsigned char)MSG_ARTIFACT_ARTIFACT_NOTICE_TIMEOUT;
			SEND_Q(rmsg, ch->m_desc);
		...
		else if (bIsField == false && pArtifact->remaining == 180)
		{
			...
			CNetMsg::SP rmsg(new CNetMsg);
			rmsg->Init(MSG_ARTIFACT);
			RefMsg(rmsg) << (unsigned char)MSG_ARTIFACT_ARTIFACT_NOTICE_3MIN;
			SEND_Q(rmsg, ch->m_desc);
		}
		else if (bIsField == false && pArtifact->remaining == 60)
		{
			...
			CNetMsg::SP rmsg(new CNetMsg);
			rmsg->Init(MSG_ARTIFACT);
			RefMsg(rmsg)<< (unsigned char)MSG_ARTIFACT_ARTIFACT_NOTICE_1MIN;
			SEND_Q(rmsg, ch->m_desc);
		}
