...
bool isPossibleWarp(CPC* fromChar, CPC* toChar)
{
	...
	if (fromChar->IsSetPlayerState(PLAYER_STATE_PKMODE) || toChar->IsSetPlayerState(PLAYER_STATE_PKMODE))
	{
		return false; //dont allow teleport when pk mode is active naughty naughty :D
	}
