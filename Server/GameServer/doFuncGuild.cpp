...
void do_GuildWarGetTimeUniReq(CPC* ch, CNetMsg::SP& msg)
{
	...
	{
		const int totalZones = 2; // merac and dratan
		...
		{
			CPC* pc = (*iter).pPlayer;
			if (pc == NULL || pc->m_desc == NULL || !pc->m_guildInfo || !pc->m_guildInfo->guild())
				continue;

			if (std::count(guilds.begin(), guilds.end(), pc->m_guildInfo->guild()->index()))
				continue;
				
			if (pc->GetJoinFlag(ZONE_DRATAN) != WCJF_NONE || pc->GetJoinFlag(ZONE_MERAC) != WCJF_NONE) {
				guilds.push_back(pc->m_guildInfo->guild()->index());
			}
		}

		if (guilds.size() > 0 && ch->m_guildInfo && ch->m_guildInfo->guild()) {

			LOG_INFO("Total guild registered war: %d", guilds.size());

			RefMsg(rmsg) << (char)guilds.size();

			iter	= playerMap.begin();
			endIter	= playerMap.end();
			for (; iter != endIter; ++iter)
			{
				CPC* pc = (*iter).pPlayer;
				if (pc == NULL || pc->m_desc == NULL || !pc->m_guildInfo || !pc->m_guildInfo->guild())
				...
				if (std::count(guilds.begin(), guilds.end(), pc->m_guildInfo->guild()->index())) {
					...
					RefMsg(rmsg) << pc->m_guildInfo->guild()->name()
								<< pc->m_guildInfo->guild()->boss()->GetName()
								<< (char)totalZones;

					if(pc->GetJoinFlag(ZONE_MERAC) != WCJF_NONE)
					{
						RefMsg(rmsg) << (char)pc->GetJoinFlag(ZONE_MERAC)
									<< (char)ZONE_MERAC;
					}
					else
					{
					...
					if(pc->GetJoinFlag(ZONE_DRATAN) != WCJF_NONE)
					{
						RefMsg(rmsg) << (char)pc->GetJoinFlag(ZONE_DRATAN)
									<< (char)ZONE_DRATAN;
					}
					else
					{
...
void do_GuildContributeSetAll(CPC* pc, CNetMsg::SP& msg)
{
	...
	RefMsg(msg) >> contribute_exp
		...
		>> contribute_fame_min
		>> contribute_fame_max;
//Veni Bug Fix EXP exploit
	if (contribute_exp < 0 || contribute_exp > 100
		|| contribute_fame < 0 || contribute_fame > 100
		|| contribute_exp_min < 0 || contribute_exp_min > 100
		|| contribute_exp_max < 0 || contribute_exp_max > 100
		|| contribute_fame_min < 0 || contribute_fame_min > 100
		|| contribute_fame_max < 0 || contribute_fame_max > 100)
	{
		LOG_ERROR("do_GuildContributeSetAll < 0 or > 100 contribute_exp[%d], contribute_fame[%d], contribute_exp_min[%d], contribute_exp_max[%d], contribute_fame_min[%d], contribute_fame_max[%d]",
			contribute_exp, contribute_fame, contribute_exp_min, contribute_exp_max, contribute_fame_min, contribute_fame_max);
			return;
	}
