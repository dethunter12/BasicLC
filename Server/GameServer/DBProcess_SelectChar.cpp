...
void DBProcess::SelectChar( boost::any& argv )
{
	...
	dbChar.GetRec("a_cube_point", pChar->m_CubePoints);
	dbChar.GetRec("a_cube_pc_kill", pChar->m_CubePlayerKill);
	dbChar.GetRec("a_cube_boss_kill", pChar->m_CubeBossKill);
	dbChar.GetRec("a_cube_point_daily", pChar->m_CubePointDaily);
#endif

#ifdef GLADES_RANKING_04182023
	dbChar.GetRec("a_glade_kills", pChar->m_gladeKillPts);
#endif
...
// 이벤트 테이블 읽기
	...
	else
	{
		...
		while (dbChar.MoveNext())
		{
			...
			if((pQuest->GetQuestProto()->m_type[1] == QTYPE_REPEAT_DAY) || pQuest->GetQuestProto()->m_type[1] == QTYPE_REPEAT_WEEK) //dethunter12 modify 3/30/23
			{
