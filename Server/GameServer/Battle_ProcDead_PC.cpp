...
void ProcDead(CPC* df, CCharacter* of)
{
	...
	gserver->m_dominationEvent.CheckRespond(df);
	#endif
	if (df != NULL)
	{
	...
	if (df->m_assist.FindBySkillIndex(1856) == 0) {
	...
	#ifdef DOMINATION_EVENT_SYSTEM
	if (gserver->m_dominationEvent.IsWaitTime(df))
	{
		CNetMsg::SP rmsg(new CNetMsg);
		WaitTimeMsg(rmsg, 10);
		SEND_Q(rmsg, df->m_desc);
			...
			if (df->m_pZone != NULL && df->m_guildInfo->guild()->m_cubeUniqueIdx >= 0 && df->m_pZone->IsExtremeCube())
			{
				...
				if (memlist)
				{
					...
					if (opc && opc->m_guildInfo && opc->m_guildInfo->guild())
					{
						...
						//dethunter12 add cube point ranking
						...
						opc->m_CubePoints += cubePoints;
						opc->m_CubePlayerKill += 1;
						opc->CalcStatus(true);
						{
							CNetMsg::SP rmsg(new CNetMsg);
