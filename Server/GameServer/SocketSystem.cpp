...
{
	int socketProb[SCROLL_COUNT][MAX_NORMAL_SOCKET_COUNT + 1] =
	{
    	{5016, 3040, 2800, 2750, 1100, 300, 10},
		{5017, 2500, 2880, 3000, 1200, 400, 20},
		{5250, 2150, 2620, 3200, 1500, 500, 30},
		{5251, 1500, 2500, 2500, 2500, 900, 100},
		{5252, 900, 1000, 3000, 3000, 1600, 500},
		{5253, 900, 1000, 3000, 3000, 1600, 500},
		{15208, 0, 0, 0, 0, 0, 10000}
	};

	st_prob_table* spt;
	int item_index;
	for (int j = 0; j < SCROLL_COUNT; ++j)
	{
		spt = new st_prob_table();
		item_index = socketProb[j][0];

		for (int i = 1; i < MAX_NORMAL_SOCKET_COUNT + 1; i++)
		{
			spt->socket_prob_table[i - 1] = socketProb[j][i];
		}

		prob_map[item_index] = spt;
	}

	int seperateScroll = 6178;
	seperate_scroll.push_back(seperateScroll);
