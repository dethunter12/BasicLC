...
void DBProcess::CreateChar(boost::any& argv)
{
	...
	if (user_index <= 0) return; //crash fix
	...
	try
	{
		...
		for (int i = 0; i < defitemcount; i++)
		{
			...
			{
				wearInven_sql2 += boost::str(boost::format(" (%d, %d, %d, %d, %d, '%s', %d, %d), ") % char_index % itemproto->getItemWearing() % itemidx[i] % plus % 0 % serial % used % 0);
			}
			else
			{
				...
				sql4 += boost::str(boost::format(", %1%, %2%, %3%, %4%, '%5%', %6%, %7%, %8%")
					% itemidx[i] % plus % itemwearpos[i] % 0 % serial % itemcount[i] % used % 0);
				icol++;
			}
...
#ifdef NIGHTSHADOW_START_NOITEM
					if (pChar->m_job != JOB_NIGHTSHADOW)
#endif
					{
						if (char_db_.excute(query) == false)
