...
bool CDescriptor::GetLogin(CNetMsg::SP& msg)
{
	try
	{
		...
		// 				&& strncmp(service_->ip().c_str(), "10.1.", strlen("10.1."))					// ??????
		// 		   )
		// 		{
		// 			// GM ?????? ????
...
#ifdef BLOCK_MAC_ADDR
		{
			CDBCmd dbAuth;
			CLCString blockSql(1000);
			blockSql.Format("select a_index from t_mac_block where a_mac = '%s' ", (const char*)packet->mac );
			dbAuth.Init(&gserver.m_dbAuth);
			dbAuth.SetQuery(blockSql);
			dbAuth.Open();

			LOG_INFO("Block by MAC %d - %s", dbAuth.m_nrecords, (const char*)packet->mac);

			if( dbAuth.m_nrecords > 0 )
			{
				throw MSG_FAIL_CONNECT_SERVER;
			}
		}
		...
		CLCString buf(1024);
		buf.Format("SELECT user_code, user_id, passwd FROM bg_user WHERE BINARY user_id='%s'",(const char*)m_idname);
		CDBCmd dbAuth;
		dbAuth.Init(&gserver.m_dbAuth);
		dbAuth.SetQuery(buf);
...
#ifdef LC_TLD
		sql.Format("select a_index, a_idna	, a_passwd, a_gametype, a_specialsw, a_special from bg_user where a_idname='%s' and a_passwd='%s' and a_gametype = 'LC' ", (const char*)m_idname, (const char*)m_passwd);
#elif defined(LC_MEX) || defined(LC_BRZ) || defined(LC_BILA) || defined(LC_USA)// MD5 Login
		sql.Format("SELECT user_code, user_id, passwd FROM bg_user WHERE BINARY user_id='%s' AND passwd=MD5('%s')", (const char*)m_idname, (const char*)m_passwd);
#if defined(LC_BILA)
		...
		sql.Format("SELECT user_code, user_id, passwd FROM bg_user WHERE BINARY user_id='%s' AND BINARY passwd='%s'", (const char*)m_idname, (const char*)gserver.m_hash);
#else // #ifdef LC_TLD
#ifdef EMPAS_LOGIN
		...
		// ALTER TABLE t_users ADD a_lastip varchar(15) , ADD a_lastnation varchar(3); ???
		CDBCmd dbUser_check;
		...
		// 4Dot ???u?? ???? network byte order ?? long ???·? ????.
		unsigned long ipAddr_before = inet_addr((const char*)service_->ip().c_str());

		// network byte order(10????) -> host byte order(16????)
		unsigned long ipAddr_real	= ntohl(ipAddr_before);
...
// --- 050226 : bs : a_enable 검사 추가
...
#ifdef BLOCK_MAC_ADDR
		{
			...
			insertMacSql.Format("insert into t_mac_log (a_user_code, a_mac) values(%d,'%s')", usercode, (const char*)packet->mac );
			dbAuth.Init(&gserver.m_dbAuth);
