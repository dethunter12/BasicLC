...
void do_CommandNameChange(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if(bguild == 1)
	{
		...
		{
			CNetMsg::SP rmsg(new CNetMsg);
			HelperNameChangeRepMsg(rmsg, bguild, index, name, MSG_EX_NAMECHANGE_ERROR_DUPLICATE, itemIdx);
			SEND_Q(rmsg, dest);
			return;
		}
	...
	else if(bguild == 5)
	{
		...
		CNetMsg::SP rmsg(new CNetMsg);
		HelperNameChangeRepMsg(rmsg, bguild, index, name, MSG_EX_NAMECHANGE_ERROR_SUCCESS, itemIdx);
		gserver.SendToAllGameServer(rmsg);
		return;
		//SEND_Q(rmsg, dest);
	}
	else
	{
		...
		for(int j = 0; j < 10; j++)
		{
			...
			if( !cmd.Update() )
			{
				cmd.Rollback();
				CNetMsg::SP rmsg(new CNetMsg);
				HelperNameChangeRepMsg(rmsg, bguild, index, name, MSG_EX_NAMECHANGE_ERROR_HELPER, itemIdx);
				SEND_Q(rmsg, dest);
				return;
			}
		}
	...
	CNetMsg::SP tmsg(new CNetMsg);
	HelperNameChangeRepMsg(tmsg, bguild, index, name, MSG_EX_NAMECHANGE_ERROR_SUCCESS, itemIdx);
	SEND_Q(tmsg, dest);
}
...
void do_CommandPartyQuitReq(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	CParty* pParty = gserver.FindPartyByMemberIndex(dest->m_subNo, nTargetIndex, false);
	if (pParty == NULL)
		pParty = gserver.FindPartyByBossIndex(dest->m_subNo, nBossIndex);
	...
	nBossIndex = pParty->GetBossIndex();
...
void do_CommandGuildMemberAdjust( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	if( gMember )
	{
		...
		{
			LONGLONG  gp = gMember->guild()->GetGuildPoint(); //RomeoST Adjust
			if(needGP >= gp)
				gp = 0;
			else
				gp = gp - needGP;
			...
			cmd.Init(&gserver.m_dbchar);
			std::string update_extend_guild = boost::str(boost::format(
				"UPDATE t_extend_guild set a_guild_point = %lld  where a_guild_index = %d") % gp % guildindex);
			cmd.SetQuery(update_extend_guild);
			if(!cmd.Update())
			{
...
void do_CommandNewGuildMemberList( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	LONGLONG  cumulatePoint[GUILD_MAX_MEMBER];
...
void do_CommandSaveGuildPoint(CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if( pGuild )
	{
		...
		cmd.Init( &gserver.m_dbchar );
		CLCString sql(256);
		sql.Format( "UPDATE t_extend_guild SET a_guild_point = %lld WHERE a_guild_index = %d", nGuildPoint, nGuildIndex );
		cmd.SetQuery( sql );
		cmd.Update();
	}
}
...
//				<< end;
		...
		sql.Format( "UPDATE t_extend_guildmember SET a_point = a_point + %lld WHERE a_guild_index = %d AND a_char_index = %d", nGuildMemberPoint, nGuildIndex, nCharIndex );
		cmd.SetQuery( sql );
		if (!cmd.Update())
		{
			GAMELOG << init( "GUILD MEMBER POINT SAVE" )
...
void do_CommandNewGuildUpdatePoint( CNetMsg::SP& msg, CDescriptor* dest)
{
	...
	if( pGuild )
	{
		...
		cmd.Init( &gserver.m_dbchar );
		CLCString sql( 1024);
		sql.Format( "UPDATE t_extend_guild SET a_guild_point = a_guild_point + + %lld WHERE a_guild_index = %d ", point, guildindex );
		cmd.SetQuery( sql );
		if( !cmd.Update() )
		{
...
void do_CommandEventPhoenix( CNetMsg::SP& msg, CDescriptor* dest )
{
	...
	// ????? ???? ???? ??? ??????? ?ε????? ?????´?.
	sprintf(select_characters_query, "SELECT a_index, a_phoenix FROM t_characters WHERE a_user_index=%d ", nUserIdx );
	...
	#ifndef PHOENIX_REQUIRE_LEVEL
	level_100_up = true;
	#endif

	LOG_INFO("Start check phoenix");
	...
	if ( level_100_up )
	{
		...
		sprintf(select_characters_query_2, "SELECT a_phoenix,a_premium_phoenix_active FROM t_characters WHERE a_index=%d ", nCharIdx );
		DB_cmd3.SetQuery(select_characters_query_2);
		if (!DB_cmd3.Open())
		{
			LOG_INFO("Not found char %d", nCharIdx);
			// ??????? ???????? ????? ??? ?????
			CNetMsg::SP rmsg(new CNetMsg);
			HelperEventPhoenixMsg(rmsg, MSG_HELPER_EVENT_PHOENIX_ERROR_REQUIREMENT, nCharIdx);
			SEND_Q(rmsg, dest);
		...
		DB_cmd3.GetRec("a_premium_phoenix_active", premPhoenix);

		LOG_INFO("Phoenix %d", phoenix);
		LOG_INFO("Phoenix %d", premPhoenix);
		// ??н? ???? ???? ?????? ĳ???? ????
		if ( phoenix == 0 || ( phoenix ==1 && premPhoenix == 0) )
		{
			// 자격이 있음
			CNetMsg::SP rmsg(new CNetMsg);
