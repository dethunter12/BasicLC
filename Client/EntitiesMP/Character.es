...
//강동민 수정 끝 다중 공격 작업		08.27
	...
	void IdleAnim(void)
	{
		...
		if(m_nWearWeapon != -1  || m_iCostumeWear[WEAR_COSTUME_WEAPON] != -1) //RomeoST
		{
			if(m_bExtension)
			{
				if( m_bRide )
...
*/
		...
		if((m_nWearWeapon != -1 || m_iCostumeWear[WEAR_COSTUME_WEAPON] != -1) && !m_bMobChange) //무기를 들고 있다면,//1008
		{
			if( !m_bExtension )
			{
				if( m_bRide )
...
//RomeoST
		if (pItemData->GetType() == CItemData::ITEM_WEAPON && m_nWearWeapon == -1 && m_iCostumeWear[WEAR_COSTUME_WEAPON] == -1)
		{
			m_bExtension = FALSE;
		}
...
//		if((bException && m_bDisappearWeapon) || (!bException && !m_bDisappearWeapon))
		if(!m_bDisappearWeapon)
		{
			...
			int iCurCostWear = IsThereCostumeWearing(WEAR_COSTUME_WEAPON);
			if (iCurCostWear > -1)
			{
				CItemData  * pItemData = _pNetwork->GetItemData(iCurCostWear);
				CModelInstance * pMI = GetCurrentPlayerModelInstance();
				_pGameState->TakeOffArmor(pMI, *pItemData);
			}
			if (iCurWear > -1 || iCurCostWear > -1)
			{
				m_bExtension = FALSE;
			}
...
/*		for(INDEX i=0; i<_pNetwork->ga_srvServer.srv_actCha.Count(); ++i)
	...
	void AppearWearingWeapon(BOOL bSkillEnd)
	{
		if(m_bDisappearWeapon)
		{
			...
			int iCutCostWear = IsThereCostumeWearing(WEAR_COSTUME_WEAPON); //RomeoST
			/*if(iCurWear == -1) 
			{
				return;
			}*/

			if(iCutCostWear > -1) //RomeoST
			{
				CModelInstance * pMI = GetCurrentPlayerModelInstance();
				CItemData  * pItemData = _pNetwork->GetItemData(iCutCostWear);
	...
	void TakeOffWearing(int wear_pos ,ObjectBase* pObject) //갑옷 벗기고 맨몸 붙이기. // 일본에 적용된 소스 
	{
		...
		if (pItemData->GetType() == CItemData::ITEM_WEAPON && m_nWearWeapon == -1 && m_iCostumeWear[WEAR_COSTUME_WEAPON] == -1) //RomeoST
		{
			m_bExtension = FALSE;
		}
		StandingAnim();
...
//안태훈 수정 끝	//(Closed Beta Effect)(0.1)
					...
					if(_pTimer->CurrentTick() - m_fAttackStartTime > m_fImpactTime-0.1f)
					{
						...
						//RomeoST
								if (m_nWearWeapon > -1 || m_iCostumeWear[WEAR_COSTUME_WEAPON] > -1)
								{
									CItemData  * pItemData = m_nWearWeapon > -1 ?_pNetwork->GetItemData(m_nWearWeapon) : _pNetwork->GetItemData(m_iCostumeWear[WEAR_COSTUME_WEAPON]);
