 ...
 ************************************************************/
  ...
  void WalkAnim()
  {
	...
	if (_pNetwork->MyWearItem[WEAR_WEAPON].IsEmptyItem() == TRUE && _pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == TRUE)
	{
		...
		{
			//cloud ant-hack
			if( pl.SouthLC_03()+30 != _pNetwork->MyCharacterInfo.attackspeed * 2 + 93) //Cloud fix attack speed cheat calculos
		    {
                Sleep(34000);//client crash to avoid scans
				Sleep(15000);
				Sleep(23000);
				Sleep(42000);
				Sleep(51000);
  ...
  {
	CPlayer &pl = (CPlayer&)*m_penPlayer;
	float runspeed = fRunSpeedMul;
	if (!(_pNetwork->MyCharacterInfo.ulPlayerState & PLAYER_STATE_FLYING))
	{
	
      if(_pNetwork->MyCharacterInfo.zoneNo != 21)
	  {
     if(runspeed * 2 + 38.1f != pl.SouthLC_01())
	 {
	                   runspeed = 1.0f;
		               if(_pGameState->Running() == TRUE){
				          _pGameState->Running() = FALSE;}
  ...
  void IdleAnim()
  {
	...
	if (_pNetwork->MyWearItem[WEAR_WEAPON].IsEmptyItem() == FALSE ||
		_pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == FALSE) //RomeoST
	{
		if (!(_pNetwork->MyCharacterInfo.ulPlayerState & PLAYER_STATE_FLYING))
		{
...
/////////////////////////////////////////////////////////////
					...
					int SouthTag = pl.GetAttackspeed();//cloud ant-hack
					...
					if( pl.m_penAttackingEnemy != NULL )
					{
						if(pl.m_penAttackingEnemy->IsEnemy())
						{
							...
							{
								// 권좌 공격속도는 1.2초.
								SouthTag = 12;//cloud ant-hack
							}
							else
							{
							//cloud ant-hack start
							if( pl.SouthLC_03()+32 != SouthTag * 2 + 95) //Cloud fix attack speed cheat calculos
					        {
							    CTString debug;
		   
		                        debug.PrintF("debug0 %d - %d", pl.SouthLC_03()+32, SouthTag * 2 + 95);
					...
					if(_pGameState->QuitScreen() == TRUE){
						_pGameState->QuitScreen() = FALSE;}
					        SouthTag = SouthTag * 10;
							Sleep(15000);
							ExitProcess(0);
							return;
                            }
				            
							_pNetwork->MyCharacterInfo.CloudGomesAS01 = SouthTag * 2 + 103;
							_pNetwork->MyCharacterInfo.CloudGomesAS02 = pl.SouthLC_03();
							_pNetwork->MyCharacterInfo.CloudGomesAS03 = _pNetwork->MyCharacterInfo.attackspeed;

							int Clounet001 = (_pNetwork->MyCharacterInfo.CloudGomesAS02+40);
							int Clounet002 = (_pNetwork->MyCharacterInfo.CloudGomesAS01+1);

							if(Clounet001+1 != Clounet002)
							 {
							  CTString debug;
		   
		                        debug.PrintF("debug1 %d - %d", Clounet001+1, Clounet002);
			                    MessageBox(NULL, debug,"debug !", MB_OK|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL);
							 ExitProcess(0);
							 Sleep(15000);
							SouthTag = SouthTag * 11+20+30+40;
							Sleep(17000);
							ExitProcess(0);
							return;
                             }
							...
							if(SouthTag * 2 + 75 != pl.SouthLC_03()+12) //Cloud fix at spd cheat calculos
					        {
							 CTString debug;
		   
		                        debug.PrintF("debug2 %d - %d", SouthTag * 2 + 75, pl.SouthLC_03()+12);
			                    MessageBox(NULL, debug,"debug !", MB_OK|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL);
							ExitProcess(0);
							Sleep(15000);
                            _pNetwork->SendInput();
					...
					if(_pGameState->QuitScreen() == TRUE){
						_pGameState->QuitScreen() = FALSE;}
                            SouthTag = SouthTag * 12+4+5+6+7+8;
							Sleep(15000);
							ExitProcess(0);
							return;
                            }
					        ...
					        if( pl.SouthLC_03()+10 != SouthTag * 2 + 73) //Cloud fix at spd cheat calculos
					        {
							 CTString debug;
		   
		                        debug.PrintF("debug3 %d - %d", pl.SouthLC_03()+10, SouthTag * 2 + 73);
			                    MessageBox(NULL, debug,"debug !", MB_OK|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL);
							ExitProcess(0);
							Sleep(15000);
                            _pNetwork->SendInput();
					...
					if(_pGameState->QuitScreen() == TRUE){
						_pGameState->QuitScreen() = FALSE;}
							...
							if( pl.SouthLC_03()+30 != SouthTag * 2 + 93) //Cloud fix attack speed cheat calculos
					        {
							 CTString debug;
		   
		                        debug.PrintF("debug4 %d - %d", pl.SouthLC_03()+30, SouthTag * 2 + 93);
			                    MessageBox(NULL, debug,"debug !", MB_OK|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL);
							ExitProcess(0);
							Sleep(15000);
                            _pNetwork->SendInput();
						...
						_pGameState->QuitScreen() = FALSE;}
							...
							if( SouthTag == 1)
					        {
							 CTString debug;
		                        ...
		                        debug.PrintF("debug5 %d", SouthTag);
			                    MessageBox(NULL, debug,"debug !", MB_OK|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL);
							ExitProcess(0);
							Sleep(15000);
                             _pNetwork->SendInput();
					...
					if(_pGameState->QuitScreen() == TRUE){
						_pGameState->QuitScreen() = FALSE;}
					        SouthTag = SouthTag * 30;
							Sleep(35000);
							return;
							ExitProcess(0);
                            }

							if( SouthTag != _pNetwork->MyCharacterInfo.attackspeed || _pNetwork->MyCharacterInfo.attackspeed == 1)//debug
					        {
							 CTString debug;
		   
		                        debug.PrintF("debug6 %d - %d", SouthTag, _pNetwork->MyCharacterInfo.attackspeed);
			                   MessageBox(NULL, debug,"debug !", MB_OK|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL);
							ExitProcess(0);
							 Sleep(32000);
					        SouthTag = SouthTag * 30 + 1 + 1 + 1 + 10 + 3 + 50;
							_pNetwork->MyCharacterInfo.attackspeed = _pNetwork->MyCharacterInfo.attackspeed + 1 + 1 + 1 + 1 * 1 * 2 * 3 + 12 + 41;
 							Sleep(35000);
							Sleep(35000);
							Sleep(35000);
							Sleep(35000);
							...
							if( SouthTag != _pNetwork->MyCharacterInfo.attackspeed || _pNetwork->MyCharacterInfo.attackspeed == 1  || pl.SouthLC_03()+30 != SouthTag * 2 + 93 || pl.SouthLC_03()+30 != _pNetwork->MyCharacterInfo.attackspeed * 2 + 93)//debug
					        {
							CTString debug;
		   
		                        debug.PrintF("debug7 %d - %d", SouthTag, _pNetwork->MyCharacterInfo.attackspeed);
			                    MessageBox(NULL, debug,"debug !", MB_OK|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL);
							ExitProcess(0);
							 Sleep(32000);
					        SouthTag = SouthTag * 30 + 1 + 1 + 1 + 12 + 3 + 30;
							_pNetwork->MyCharacterInfo.attackspeed = _pNetwork->MyCharacterInfo.attackspeed + 1 + 1 + 51 + 1 * 1 * 2 * 3 + 3 + 11;
 							Sleep(32000);
							Sleep(31000);
							Sleep(34000);
							Sleep(36000);
					...
					if ((_pNetwork->MyWearItem[WEAR_WEAPON].IsEmptyItem() == FALSE || _pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == FALSE) &&
						_pNetwork->MyCharacterInfo.bExtension )
					{
						fLength = pMI->GetAnimLength(idPlayerWhole_Animation[ANIM_EXT_ATTACK_1]) * 10.0f;
					}

					_fAttackSpeedMul	= 1.0f / (fLength / SouthTag);
					_fAttackLengthMul	= (float)SouthTag / fLength;
...
///////////////////////////////////////////////////////////////
						...
						IsEXMage(iJob))	//2013/01/08 jeil EX메이지 추가 
					{
						...
						{ // 동작중인것이 없을때
							...
							_fAttackSpeedMul	= 1.0f / (fLength / _pNetwork->MyCharacterInfo.attackspeed);
							_fAttackLengthMul	= _pNetwork->MyCharacterInfo.attackspeed / fLength;
							FLOAT tmpAniTime = fLength * _fAttackLengthMul;
							_4Fun = 1.0f / (fLength / SouthTag);
							_0 = 1.0f / (fLength / _pNetwork->MyCharacterInfo.attackspeed);													   
							...
							!GetPlayer()->m_bLockMove)
						{
						   ...
						   _4Fun = 1.0f / (fLength / SouthTag);//cloud ant-hack
						   _0 = 1.0f / (fLength / _pNetwork->MyCharacterInfo.attackspeed);

                            //cloud ant-hack
							_pNetwork->MyCharacterInfo.CloudGomesAS01 = SouthTag * 2 + 103;											
					       if(_fAttackSpeedMul != _4Fun)
					       {
					         ExitProcess(0);
							 return;
					       }//cloud ant-hack
				...
				{
				    FLOAT runspeed = _pNetwork->MyCharacterInfo.runspeed;//Cloud runspeed//cloud ant-hack
					FLOAT check12 = pl.GetRunspeed();//cloud ant-hack
					//cloud ant-hack start
					if(_pNetwork->MyCharacterInfo.zoneNo != 21)
					{
					if (!(_pNetwork->MyCharacterInfo.ulPlayerState & PLAYER_STATE_FLYING))
					...
					if(runspeed * 2 + 38.1f != pl.SouthLC_01())//if(runspeed != _pNetwork->MyCharacterInfo.runspeed)
					{
					   _pNetwork->SendInput();
					...
					if(_pGameState->QuitScreen() == TRUE){
					   ...
					   runspeed = runspeed * 90/100;
					   Sleep(15000);
			           return;
					}

					if(runspeed != check12 || check12* 2 + 38.1f != pl.SouthLC_01())//if(runspeed != _pNetwork->MyCharacterInfo.runspeed)
					{
					   _pNetwork->SendInput();
