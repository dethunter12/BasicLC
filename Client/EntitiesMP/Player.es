...
static void GoTo(FLOAT x, FLOAT y, FLOAT z, FLOAT r)
{
	...
	if(pen!=NULL)
	{
		...
		pen->NewClearState(CLEAR_STATE_LENGTH);//Cloud ep1 animation
		pen->IdleAnim();	//Cloud ep1 animation
	}
}
...
//	128 FLOAT m_tmSeriousBombFired = -10.0f,  // when the bomb was last fired
	...
	135 INDEX m_sbAttackspeedfix = 0, //cloud ant-hack
	136 FLOAT m_fRunspeedfix = 0.0f, //cloud ant-hack
	137 FLOAT m_sbCloudGomes = 0.0f, //cloud ant-hack													
...
//		penWeapons->GetWeaponName(strWeaponName);
	...
	virtual void Read_net_StatusPC(CNetworkMessage *istr)
	{
		...
		if(pPack->charIndex == _pNetwork->MyCharacterInfo.index)
		{
			...
			if( _pNetwork->MyCharacterInfo.statusEffect.IsState(EST_ASSIST_BLIND) )
			{
                ...
                //strSysMessage  . PrintF  (_S  (2083 , "????? ???????? ????? ???????."));
                //_pNetwork  -> ClientSystemMessage  (strSysMessage  , SYSMSG_ERROR );
                penPlr->m_bLockSkillCancel  = FALSE ;
                penPlr->m_bWaitForSkillResponse  = FALSE ;
		...
		if (pObject != NULL)
		{
			...
			pTarget->cha_hp = CurrentHp; //dethunter12
			pTarget->cha_maxhp = FullHp;
			penEntity = pTarget->GetEntity();
			//if (_pNetwork->ga_World.EntityExists(ct.m_nIdxClient, penEntity)) 
			{
					...
					&& _pNetwork->m_ubGMLevel < 2)
				{
					...
					{
						if(penPlr->IsSkilling())
						{
							penPlr->CancelSkill(TRUE, g_iAutoAttack,FALSE);
							penEntity->InflictDirectDamage(penEntity, penPlr, DMT_NONE, 0, FLOAT3D(0,0,0), ANGLE3D(0,0,0));
						}
	...
	virtual void Read_net_Damage_Character(CNetworkMessage *istr)
	{
		...
		// ��� ������ �˻�
		int nType = 0;
		//cloud ant-hack start
		int damageSPD = pPack->attackSpeed;//Cloud ant-hacks
		int ataqueSPD = _pNetwork->MyCharacterInfo.attackspeed;
		int ataqueSPD1 = GetAttackspeed();
		int ataqueSPD2 = SouthLC_03();

		if (_pNetwork->MyCharacterInfo.index == pPack->charIndex)
		{

		if(ataqueSPD != ataqueSPD1)
		{
			CTString debug;
		   
		    debug.PrintF("debug01 %d - %d", ataqueSPD, ataqueSPD1);
			MessageBox(NULL, debug,"debug !", MB_OK|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL);
			_pNetwork->SendInput();
			ExitProcess(0);
		...
		if(ataqueSPD1 * 2 + 95 != ataqueSPD2+32)//Cloud ant-hacks
		{
			CTString debug;
		   
		    debug.PrintF("debug02 %d - %d", ataqueSPD1 * 2 + 95 , ataqueSPD2+32);
			MessageBox(NULL, debug,"debug !", MB_OK|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL);
			_pNetwork->SendInput();
			 Sleep(15000);
			ExitProcess(0);
        ...
        _pNetwork->MyCharacterInfo.CloudGomesSS01 = skillSpeed;
		_pNetwork->MyCharacterInfo.CloudGomesSS02 = SouthLC_04();		
	    if(skillSpeed != SouthLC_04() || _pNetwork->MyCharacterInfo.CloudGomesSS01 != _pNetwork->MyCharacterInfo.CloudGomesSS02)
		{
		   CTString debug;
		   
		   debug.PrintF("debug03 %f-  %f", SouthLC_04(), skillSpeed);
		   MessageBox(NULL, debug,"debug !", MB_OK|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL);
		   _pNetwork->SendInput();
		   if(_pGameState->Running() == TRUE){
			_pGameState->Running() = FALSE;}
		...
		bool hacked = true;
	if(_pNetwork->MyCharacterInfo.bExtension == TRUE || _pNetwork->MyCharacterInfo.bExtension == FALSE)
	{
	     hacked = false;
	}
			...
			if( _pNetwork->SearchEntityByNetworkID( pPack->charIndex, MSG_CHAR_PC, penEntity ) )
			{
				...
				if(pPack->targetType == MSG_CHAR_PC)
				{
					...
					if(pPack->targetIndex == _pNetwork->MyCharacterInfo.index)
					{
						...
						ShowDamageResult( penEntity, pPack->damage, pPack->flag, pPack->skillIndex );//Cloud add
...
//������ ���� ��	//(Open beta)(2004-12-14)
	...
	virtual void SetImportantValues( FLOAT fWalkspeed, FLOAT fRunspeed, FLOAT fRunspeedfix, FLOAT fAttackrange, SBYTE sbAttackspeed , SBYTE sbAttackspeedfix, SBYTE sbMagicspeed , SBYTE sbSkillspeed) //Cloud ant-hack
	{
		m_fWalkspeed = fWalkspeed;
		m_fRunspeed = fRunspeed;
		m_fRunspeedfix = (fRunspeedfix);//Cloud ant-hack									
		m_fAttackrange = fAttackrange;
		m_sbAttackspeed = sbAttackspeed;
		m_sbAttackspeedfix = (sbAttackspeedfix);//Cloud ant-hack											
		m_sbMagicspeed = sbMagicspeed;
		m_sbCloudGomes = (107 - sbSkillspeed) / 100.0f;//Cloud ant-hack										   
	}
	...
	virtual FLOAT SouthLC_01()//Cloud ant-hack 
	{
		return m_fRunspeedfix; 
	}						
    virtual SBYTE SouthLC_03() //Cloud ant-hack
	{ 
		return m_sbAttackspeedfix; 
	}
    virtual FLOAT SouthLC_04()//Cloud ant-hack 
	{ 
		return m_sbCloudGomes;
	}		
...
//0605 kwon
	...
	virtual void SetCharCostumeWearing(CCharacterTarget* pTarget, CEntity* penEntity, CNetworkMessage *istr)
    {
        ...
        if (((CCharacter  *)penEntity)->m_iCostumeWear[WEAR_COSTUME_WEAPON] == -1) {return;}
 
        CItemData  * pItemData = _pNetwork->GetItemData(((CCharacter  *)penEntity)->m_iCostumeWear[WEAR_COSTUME_WEAPON]);
        if (pItemData->GetType() == CItemData::ITEM_WEAPON)
        {
            const BOOL bExtension = _pNetwork->IsExtensionState(((CCharacter  *)penEntity)->m_ChaList, *pItemData);
            ((CCharacter  *)penEntity)->m_bExtension = bExtension;
        }
    }
	...
	virtual void WearingCostumeArmor( CNetworkMessage *istr )
	{
		...
		if (pObject != NULL)
		{
			...
			CModelInstance *pMI = ((CCharacter*)penEntity)->GetCurrentPlayerModelInstance();
			INDEX wear_index = ((CCharacter*)penEntity)->IsThereWearing(NormWearPos);
			...
			if (realWearIndex > 0) // ���� (�ڽ�Ƭ)
			{
				...
				if (_pNetwork->GetItemData(realWearIndex)->IsFlag(ITEM_FLAG_INVISIBLE_COSTUME) == false)
				{
					...
					else if ( NormWearPos == WEAR_HELMET && (CTString)_pNetwork->GetItemData( realWearIndex )->GetItemSmcFileName() == MODEL_TREASURE )
					{
						((CCharacter*)penEntity)->DeleteDefaultArmor(NormWearPos);
					}
				}
				else
				{
			...
			if (item_index > 0) // �Ա�(�ڽ�Ƭ)
			{
				...
				{
					((CCharacter*)penEntity)->WearingArmor(item_index, NormWearPos);
			...
			{
				((CCharacter*)penEntity)->WearingArmor(wear_index, NormWearPos);
			...
			else // �Ա�( default )
			{
				...
				{
					((CCharacter*)penEntity)->WearingDefaultArmor(NormWearPos);
	if (wear_pos == WEAR_COSTUME_WEAPON) //RomeoST
					{
						((CCharacter  *)penEntity)->StandingAnim();
					}
				}
				else
				{
				...
				pTarget->cha_itemEffect.DeleteEffect(NormWearPos);
			...
			if (realWearIndex >= 0)
			{
				pTarget->cha_itemEffect.Change(
					...
					, NormWearPos
					, pTarget->cha_itemPlus[NormWearPos]
					, &pMI->m_tmSkaTagManager
					, (pTarget->m_nType == NIGHTSHADOW) ? 1 : pTarget->cha_sbItemEffectOption
...
/*			case 683: // ���鸮 ����Ʈ
            ...
            _pNetwork->MyCharacterInfo.CloudGomesSS01 = skillSpeed;	//cloud ant-hack
			if (bEndSkill_Act) // ��ų ������
			{
				m_bReadySendSkillMessage = TRUE;
...
//������ ���� �� Ŭ���� �غ� �۾�		08.10	
	...
	void IdleAnim()
	{
		...
		if(_pNetwork->MyWearItem[WEAR_WEAPON].IsEmptyItem() == FALSE || //RomeoST
		   _pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == FALSE)		{
					...
					|| idBodyAnim== idPlayerWhole_Animation[ANIM_GATHER] )
			{
				...
				{					
					if(!MD->IsCollect())
					{
						...
						if(_pNetwork->MyWearItem[WEAR_WEAPON].IsEmptyItem() == TRUE && _pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == TRUE)
						{
							CancelProduct();
							IdleAnim();
							return;
						}
						const int  iWeaponType = _pNetwork->MyWearItem[WEAR_WEAPON].IsEmptyItem() == TRUE ? //RomeoST
											_pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].ItemData->GetSubType() :
											_pNetwork->MyWearItem[WEAR_WEAPON].ItemData->GetSubType();// ���굵���� �����ϱ� ������...
...
//�米���۽ÿ� ���⸦ �����ٰ� ��Ÿ���� �ϱ�.
	void DeleteWearingWeapon(BOOL bException, BOOL bSkillReady)
	{
		...
		//if(!m_bDisappearWeapon)
		{
			...
			if (_pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == FALSE) //RomeoST
			{
				CModelInstance * pMI = GetCurrentPlayerModelInstance();
				CItemData  * pItemData = _pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].ItemData;
				_pGameState->TakeOffArmor(pMI, *pItemData);
			}
			m_bDisappearWeapon = TRUE;
			//�� �Ѹ��� �׼��� ��
	...
	void AppearWearingWeapon(BOOL bSkillEnd)
	{
		if(m_bDisappearWeapon)
		{
				...
				_pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == FALSE)
			{
				...
				if(_pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == FALSE) //RomeoST
				{
					_pGameState->WearingArmor(pMI, *_pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].ItemData);
				}
				else
				{
...
virtual BOOL CheckSkill(void)
{
	...
	_pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == FALSE)
	{
		...
		switch (nUsingWeapon)
		{
		case 1: // WeaponType0�� ���� �� //RomeoST
			{
				if (_pNetwork->MyWearItem[WEAR_WEAPON].IsEmptyItem() == FALSE ? 
				_pNetwork->MyWearItem[WEAR_WEAPON].ItemData->GetSubType() != SkillData.Skill_Data.useWeaponType0 :
				_pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].ItemData->GetSubType() != SkillData.Skill_Data.useWeaponType0){
					bCanUse = FALSE;
				}
			}
			break;
		case 2: // WeaponType1�� ���� ��
			{
				...
				_pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].ItemData->GetSubType() != SkillData.Skill_Data.useWeaponType1){
					bCanUse = FALSE;
				}
			}
			break;
		...
		else
		{
			if( penTarget )
			{
				...
				{				
					...
					if(!m_bWaitForSkillResponse)
					{
						if ((_pNetwork->MyWearItem[WEAR_WEAPON].IsEmptyItem() == TRUE && _pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == TRUE) && (SkillData.Skill_Data.useState  & SCT_WEAPON)) //RomeoST
						{
						...
						{
							if (_pNetwork->MyWearItem[WEAR_WEAPON].IsEmptyItem() == FALSE || _pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].IsEmptyItem() == FALSE) //RomeoST
							{
								// FIXME : CItemData ���� �о�ö� �̸� ����ϸ� ó���ϱ� ���� �κ�...
								const int  iWeaponType = _pNetwork->MyWearItem[WEAR_WEAPON].IsEmptyItem() == FALSE ? _pNetwork->MyWearItem[WEAR_WEAPON].ItemData->GetSubType() : _pNetwork->MyWearCostItem[WEAR_COSTUME_WEAPON].ItemData->GetSubType();
								if( iWeaponType == CItemData::ITEM_WEAPON_MINING ||
									iWeaponType == CItemData::ITEM_WEAPON_GATHERING || 
									iWeaponType == CItemData::ITEM_WEAPON_CHARGE )
								{
...
void SpellSkill(INDEX m_nCurrentSkillNum)
{
	...
	FLOAT skillSpeed = (100 - _pNetwork->MyCharacterInfo.skillSpeed) / 100.0f;
	//cloud ant-hack start
	if(skillSpeed != SouthLC_04())
		{
		   CTString debug;
		   
		   debug.PrintF("%f-  %f", SouthLC_04(), skillSpeed);
		   _pNetwork->SendInput();
		   skillSpeed = 1.0f;//20
		   if(_pGameState->Running() == TRUE){
						_pGameState->Running() = FALSE;}
	...
	if(_pNetwork->MyCharacterInfo.CloudGomesSS01 != _pNetwork->MyCharacterInfo.CloudGomesSS02)//20 Cloudspeed SouthLC_02() <= 0.20f
		{
		   CTString debug;
	...
	//CPrintF("SkilSpoul()!!! m_idCurrentSkillAnim=%f..\n",m_idCurrentSkillAnim);
	...
	if ( !m_bNoAniGuildSkill )
	{
     ...
     //cloud ant-hack start replace code
    FLOAT GuildQuests = GetModelInstance()->GetAnimLength(m_idCurrentSkillAnim) * SouthLC_04();

	 if (m_nCurrentSkillNum != 230 && _pNetwork->MyCharacterInfo.skillSpeed == 0)
	 {
		
		m_fSkillAnimTime = GetModelInstance()->GetAnimLength(m_idCurrentSkillAnim) * 1.0f;//Cloud test
	 }
	 else
	 {
	    m_fSkillAnimTime = GetModelInstance()->GetAnimLength(m_idCurrentSkillAnim) * skillSpeed;//Cloud test
	 }

	 if (m_nCurrentSkillNum != 230 && m_fSkillAnimTime <= 1.0f)
	 {
	    if(_pNetwork->MyCharacterInfo.skillSpeed != 0)
		{
		    m_fSkillAnimTime = GetModelInstance()->GetAnimLength(m_idCurrentSkillAnim) * SouthLC_04();
		}
		else
		{
		    m_fSkillAnimTime = GetModelInstance()->GetAnimLength(m_idCurrentSkillAnim) * 1.0f;
		}
	...
	FLOAT GuildQuests02 = m_fSkillAnimTime + 0.5f;

	if(GuildQuests + 0.5f != GuildQuests02 || GuildQuests02 < 0.5f)
	{
		   CTString debug;
		   
		   debug.PrintF("%f-  %f", GuildQuests, GuildQuests02);
		   _pNetwork->SendInput();
					if(_pGameState->Running() == TRUE){
						_pGameState->Running() = FALSE;}
					if(_pGameState->QuitScreen() == TRUE){
						_pGameState->QuitScreen() = FALSE;}
		   m_fSkillAnimTime = GetModelInstance()->GetAnimLength(m_idCurrentSkillAnim) * 100.0f;//Cloud test
		   Sleep(15000);
		   ExitProcess(0);
		   return;
    }//cloud ant-hack end
...
*/
	...
	FLOAT skillSpeed = (100 - _pNetwork->MyCharacterInfo.skillSpeed) / 100.0f;
    //cloud ant-hack start
	if(skillSpeed  != SouthLC_04())//20 Cloudspeed SouthLC_02() <= 0.20f
	{
		   _pNetwork->SendInput();
		   //skillSpeed = 0.20f;//20
		   _pGameState->Running()		= FALSE;
