...
#include <Contents/Base/ChattingUI.h>
...
DOUBLE MyInfoSkill::GetReuseSkill( int nIndex )
{
	...
	{ 
		...
		if (!(rSkill.Skill_Data.appState & SCT_NOCOOLTIME))
		{
			...
			dCoolTime -= dCoolTime * nCoolTimeReductionRate / 100;
		}
	}

	if(rSkill.GetReUseTime())
	{
		dReuseTime = DOUBLE(rSkill.GetReUseTime() + _pNetwork->MyCharacterInfo.magicspeed) / 10.0;
		dReuseTime -= dReuseTime * nCoolTimeReductionRate / 100;
		if (dReuseTime < 0.0)
			return 0.0;
		CUIManager* pUIManager = CUIManager::getSingleton();
		double test = dReuseTime;
		double cooltime = dCoolTime;
		return dReuseTime;
	}
