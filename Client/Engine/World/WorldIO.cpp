...
int readWLDencrypted(char *  filename , unsigned char* &buffer,long &size ){
  ...
  if (buffer == NULL) 
  {
	MessageBox(NULL,"XML ENCRYPT", "MEMORY ALLOC ERROR" ,MB_OK);
 ...
 int ret = 0;
 size = lSize;
unsigned char encsig[4] ={0x48 ,0x41 ,0x43 ,0x4B};;
...
unsigned char wldkey2[] = { 0x4D, 0x35, 0x62, 0x5B, 0x5A, 0x3C, 0x62, 0xE1, 0xC2, 0xC5, 0x21, 0x17, 0xB2, 0xC4, 0x62, 0xB4, 0xA1, 0x19, 0x12, 0x4A, 0xA7, 0xA4, 0x64, 0xB8, 0x28, 0x66, 0xBA, 0xBB, 0xBA, 0x16, 0x6E, 0x39 };
unsigned char wldkey1[] = { 0xA8, 0x96, 0x36, 0x92, 0x4A, 0xC9, 0x4D, 0xD0, 0xAD, 0x14, 0x56, 0x79, 0x88, 0x61, 0x73, 0x5D, 0x62, 0x3D, 0xAA, 0xCA, 0xA3, 0x5F, 0x1D, 0x9C, 0x9B, 0x56, 0x63, 0x70, 0x52, 0xB9, 0xBA, 0xBB, 0xCB, 0xCA, 0x41, 0xF5, 0x6C, 0x92, 0x61, 0xC8, 0xA7, 0x72, 0xE1, 0x56, 0x83, 0x61, 0x42, 0xA6, 0xDA, 0xD9, 0xC2, 0x91, 0xBB, 0x12, 0x15, 0x51, 0x54, 0x64, 0x18, 0xB1, 0x5E, 0x4D, 0x81, 0x7B };

void decryptwld(unsigned char* buffer, int size){
	
	//DECRYPT DATA BUFFER//
	//key 2 decrypt from encrypted stream to buffer
	for (DWORD i = 4; i < size; i++){//bytes
    buffer[i] = (buffer[i] ^ wldkey2[(i-4) % sizeof(wldkey2)]);
	}

	//key1
	for (DWORD i = 4; i < size; i++){//bytes
    buffer[i] = (buffer[i] ^ wldkey1[(i-4) % sizeof(wldkey1)]);
	}
...
void CWorld::Load_t(const CTFileName &fnmWorld) // throw char *
{
	...
	if (strmFile.PeekID_t()==CChunkID("HACK"))
	{

		/*CTFileStream CTFS;
		CTFileName TEMPFNM;
		TEMPFNM.PrintF("%s%s",wo_fnmFileName.str_String,"dec");
		CTFS.Create_t(TEMPFNM);
		*/
		unsigned char* buffer = NULL;
		long sizemela = 0; 
		readWLDencrypted(wo_fnmFileName.str_String,buffer,sizemela);
		decryptwld(buffer,sizemela);
		
		/*
		CTFS.Write_t(buffer+4,sizemela-4);
		CTFS.Seek_t(0,CTStream::SD_BEG);
		CTFS.Close();
		strmFile.Close();

		strmFile.Open_t(TEMPFNM);
		wo_fnmFileName = TEMPFNM;
		*/
		
		CTMemoryStream CTS;
		CTS.Write_t(buffer+4, sizemela-4);
		CTS.strm_strStreamDescription = strmFile.strm_strStreamDescription;
		CTS.Seek_t(0,(CTStream::SeekDir)0);
		
		// check engine build allowing reinit
		_pNetwork->CheckVersion_t(CTS, TRUE, bNeedsReinit);
