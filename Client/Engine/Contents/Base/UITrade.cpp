...
void CUITrade::OpenUI(int npc_index) //dethunter12 check
{
	...
	UpdateItemList(npc_index); //dethunter12 add 11/13/2019
...
//int npc_index
void CUITrade::UpdateItemList( int npc_index) //dethunter12 adjust 11/13/2019
{
	...
	for (i = 0; i < max; ++i)
	{
		...
		if (pTradeItem->npc_index != npc_index ) //dethunter12 need to write a check for the trade item being correct for the npc.
			continue;

		if (nItemCnt <= i)
			m_pList[eLIST_AMEND_ITEM]->AddListItem(pItem->Clone());

		pItemTmp = (CUIListItem*)m_pList[eLIST_AMEND_ITEM]->GetListItem(nPos);

		if (pItemTmp == NULL)
			continue;

		m_currentItems.push_back(i);
	...
	int nMaxItem = m_pList[eLIST_AMEND_ITEM]->getListItemCount();

	m_pList[eLIST_AMEND_ITEM]->UpdateScroll(nMaxItem);
	m_pList[eLIST_AMEND_ITEM]->UpdateList();

	UpdateAmendItem(-1);
	UpdateAmendCondition(-1);

	int nShowCnt = m_pList[eLIST_AMEND_ITEM]->GetItemShowNum();
...
{
	CPrintF("UpdateAmendCondition(%d)\n", nAmendIndex);
...
{
		CPrintF("UpdateAmendConditionCount(%d)\n", nAmendIndex);
	...
	{
		int selectedIndex = m_pList[eLIST_AMEND_ITEM]->getCurSel();
		if(selectedIndex >= 0 && selectedIndex < m_currentItems.size())
			nAmendIndex = m_currentItems[selectedIndex];
	}
...
{
		CPrintF("UpdateAmendItemCount(%d)\n", nAmendIndex);
...
{
	CPrintF("CheckItem(%d, %d)\n", nAmendIndex, nConditionIndex);
	stEventTrade* pTradeItem = CTradeItem::GetTradeItem(nAmendIndex);
...
void CUITrade::SendTrade()
{
	...
	nSelect = m_currentItems[nSelect];
...
void CUITrade::ResetPosition( PIX pixMinI, PIX pixMinJ, PIX pixMaxI, PIX pixMaxJ )
{
	SetPos( ( pixMaxI + pixMinI - GetWidth() ) / 2, ( pixMaxJ + pixMinJ - GetHeight() ) / 2 );
}
void CUITrade::AdjustPosition( PIX pixMinI, PIX pixMinJ, PIX pixMaxI, PIX pixMaxJ )
{
