...
#include "Contents/function/AutoFunction.h" // RomeoST
...
const	int	   SPAM_CHAT_LENGTH					= 300;  //�幮�� �Ǵ��ϴ� ���� ���� 
const   int	   SPAM_CHAT_BLOCK_INPUT_COUNT		= 150;	// 1?��??? ??? ???? ???? ???? 
	...
	, m_iCurrentMsgBuffer(0)
{
	...
	ItemChatCountFromServer = 0; //Romeo
...
void CChattingUI::initialize()
{
	...
	m_btn_auto_func = static_cast<CUIButton*>(findUI("btn_auto_sys"));
	m_btn_auto_func->SetCommandFUp(boost::bind(&CChattingUI::OpenAutoFunc, this));
#ifdef CUBE_RANKING_12202022
	m_btn_cube_rnk_func = static_cast<CUIButton*>(findUI("btn_cube_rnk"));
	m_btn_cube_rnk_func->SetCommandFUp(boost::bind(&CChattingUI::OpenCubeRank, this));
#endif
#ifdef BATTLE_PASS_SYSTEM
	m_btn_battle_pass = static_cast<CUIButton*>(findUI("btn_battle_sys"));
	m_btn_battle_pass->SetCommandFUp(boost::bind(&CChattingUI::OpenBattlePass, this));
#endif
#ifdef GLADES_RANKING_04182023
	...
	CTString &strChatMessage , BYTE channelNumber, SLONG slGroup)
{
	...
	if (_UIFiltering.Filtering(strChatMessage.str_String, nIndexBuf) == TRUE)
	{
		...
		{
			AddSysMessage(_S(437, "���忡 ������ �ܾ ���ԵǾ� �ֽ��ϴ�."));
			return;
		}
	}
	...
	else
	{
		...
		{
			if (slGroup == -1)	// ������ ��ü ä��
				strOutputName.PrintF("[%s]%s >", _S(4493, "������"), strSendName);
			else	// ������ �׷� ä��
			{
				CTString strTemp;
				strTemp.PrintF(_S(4494, "�׷�%d"), slGroup + 1);
		...
		{
			if (channelNumber > 0)
				strOutputName.PrintF("[%s%d]%s > ", _S(5392, "ä��"), channelNumber, strSendName);
			else
				strOutputName.PrintF("%s > ", strSendName);
		}
	}
	...
	case CHATMSG_PRESSCORPS:
		{
			...
			for (int i = 0; i < eMAIN_END; ++i)
			{
				for (int j = 0; j < eCM_END; ++j)
				{
					UpdateChatMsg(false, m_pListChat[i][j], strChatMessage, colChat, eMAIN_END); //Romeo
				}
			}
		}
		break;
	case CHATMSG_NOTICE_SERVERDOWN:
		{
			CTString strTemp;
			strTemp.PrintF(_S(2573, "���� ������� %d�� ���ҽ��ϴ�."), atoi(strChatMessage.str_String));
	...
	default:
		{
			if (IsGamigo(g_iCountry) == TRUE || g_iCountry == USA)
			{
					...
					slSendIndex != _pNetwork->MyCharacterInfo.index) && CeilWritingCut_CompareStr(strChatMessage))
				{
					...
					CTString strTemp;
					strTemp.PrintF(_S(3006, "%s���� ���ܵǾ����ϴ�."), strSendName);
...
//#endif
			...
			if (channelNumber > 0)
			{
				...
				for (int j = 0; j < eCM_END; ++j)
				{
					...
					UpdateChatMsg(false, m_pListChat[eMAIN_CHANNEL][j], strChatMessage, colChat, eMAIN_CHANNEL, strOutputName, colName); // ROmeoST
			...
			else
			{
				...
				for (int j = 0; j < eCM_END; ++j)
				{
					...
					UpdateChatMsg(false, m_pListChat[eMAIN_GENERAL][j], strChatMessage, colChat, eMAIN_GENERAL, strOutputName, colName); //RomeoST
...
//#endif // SHOW_DAMAGE_TEXT
	...
	if (nSysType == SYSMSG_RED && col != DEF_UI_COLOR_WHITE)
		col_text = col;
	else if (nSysType == SYSMSG_USER && col != DEF_UI_COLOR_WHITE)
		col_text = col;

	UpdateChatMsg(true, m_pListSys, strMsg, col_text, eMAIN_END); // RomeoST
}
...
{	
	...
	std::string checkHackCommand(pcChatString);
	if (checkHackCommand.find("</>") != std::string::npos ||
		checkHackCommand.find("< 1>") != std::string::npos ||
		checkHackCommand.find("<") != std::string::npos ||
		checkHackCommand.find(">") != std::string::npos) {
		// Reset string of input box
		m_pChatInput->ResetString();
		InsertChatPrefix( ChattingMsgType( nCurSelTab ) );
	...
	if( _pNetwork->m_ubGMLevel > 1 )
	{
		...
		_pNetwork->SendChatMessage(ubType, m_strWhisperTarget, strChatString, m_pChatInput->Items, nChatType);
		m_pChatInput->Items.clear(); //RomeoST
	}
	else
	{
		//[ttos_2009_1_23]:ä�� ����
		if (_pNetwork->MyCharacterInfo.ChatFlag != 0 )
		{
			...
			{
				AddSysMessage(_S(4320,"ä�ñ���� �����Ǿ� ��ȭ�� �� �� ���� �����Դϴ�. "));
				m_pChatInput->ResetString();
				InsertChatPrefix( ChattingMsgType( nCurSelTab ) );
				return;
			}
...
//#ifdef ADD_CHAT_CEILWRITING_CUT_NA_20081029
			if (IsGamigo(g_iCountry) == TRUE || g_iCountry == USA)
			{
				if (ubType == CHATMSG_NORMAL || ubType == CHATMSG_CHANNEL_LEVEL)
				{
					...
					{
						strChatString = _S(4221, "1�г� ���� ����, �ܾ 2ȸ �̻� �Է� �� �� �����ϴ�.");					
						AddSysMessage(strChatString);
					}
					else
					{
...
void CChattingUI::UpdateChatMsg(bool bSys, CUIList* pList, CTString strChat, COLOR colChat, char typeList, CTString strName /*= ""*/, COLOR colName /*= 0xFFFFFFFF*/)
{
	...
	if (bSys == false && ItemsChat.size() > 0) //RomeoST
	{
		auto iter = ItemsChat.begin();
		int pos = 0;
		std::string strC = std::string(strChat);
		while (iter != ItemsChat.end())
		{
			if (iter->typeChat == -1)
				iter->typeChat = typeList;
			if ((iter->CountShow <= 3 && iter->typeChat == eMAIN_GENERAL) || (iter->CountShow <= 1 && iter->typeChat == eMAIN_CHANNEL))
			{
				if (pos >= iter->StartPos && pos <= iter->EndPos)
				{
					m_pTextBoxDesign->AddText(strC.substr(pos, iter->EndPos - pos), 0xf4e19eFF, iter->item, 0xcc8f3fff, ItemChatCountFromServer);
					pos = iter->EndPos;
				}
				else
				{
					m_pTextBoxDesign->AddText(strC.substr(pos, iter->StartPos - pos), colChat);
					pos = iter->StartPos;
					continue;
				}
				iter->CountShow++;
		...
		if (pos < strC.size())
		{
			m_pTextBoxDesign->AddText(strC.substr(pos, strC.size()), colChat);
		}
	}
	else
	{
...
void CChattingUI::OpenChatOption()
{
	...
	if (m_bRearrangeOption == false)
	{
		...
		// 9 : �ؽ�Ʈ ���� ��, 13 : üũ ��ư ������, 10 : �ؽ�Ʈ�� üũ��ư ��, 10 : üũ ��ư�� ��� ��
	...
	for (int i = 0; i < eCM_MAX; ++i)
	{
		...
		if(i == eCM_GROUP)
		{
			if (UIMGR()->IsCSFlagOn(CSF_EXPEDITION))
				m_pTextOption[i]->SetText(_S(4492, "������(@)" ));
			else
				m_pTextOption[i]->SetText( _S( 453, "��Ƽ(@)" ) );
		}
...
void CChattingUI::SpamerBlock ( CTString& strName )
{
	...
	CTString temStr;
	temStr.PrintF(_S(3006, "%s���� ���ܵǾ����ϴ�."),strName);
...
{
	if( (pcChatString[1] >= '1') && (pcChatString[1] <= '4') )	//@���� ���� ���� 1~4�� ��� �׷� ä�� ���� �ν�
	{
		return pcChatString[1] - 49;							//�׷� ä���ϰ�� 0~3 ��ȯ
	}
	else
		return -1;	
}
...
void CChattingUI::SystemMessageColorInI()
{
	...
	GetPrivateProfileString("SYSTEM_MSG", "RED", "0xFF3300FF", szBuff, 16, strFullPath.c_str());
	m_colSysType[SYSMSG_RED] = (COLOR)strtoul(szBuff, NULL, 16);
}

//Val add auto system/battlepass to chat UI
void CChattingUI::OpenAutoFunc()
{
	CUIManager::getSingleton()->GetAutoFunc()->ToggleWindow();
}
#ifdef CUBE_RANKING_12202022
void CChattingUI::OpenCubeRank()
{
	CUIManager* pUIManager = CUIManager::getSingleton();

	if (pUIManager->GetRankingCube()->IsVisible())
		pUIManager->GetRankingCube()->closeUI();
	else
...
#ifdef GLADES_RANKING_04182023
void CChattingUI::OpenGladesRank()
{
	CUIManager* pUIManager = CUIManager::getSingleton();

	if (pUIManager->GetRankingGlades()->IsVisible())
		pUIManager->GetRankingGlades()->closeUI();
	else
		_pNetwork->RankingGladesReq();
}
#endif

#ifdef BATTLE_PASS_SYSTEM
void CChattingUI::OpenBattlePass()
{
#ifdef BATTLE_PASS_SYSTEM
...
BOOL CChattingUI::CheckInputChannelChat( ChattingMsgType cmtType, CTString strMessage )
{
	...
	case CHATMSG_CHANNEL_LEVEL:
		{
			if (_pNetwork->MyCharacterInfo.level < CHANNEL_CHAT_NEEDLEVEL) // 50���� �̻� �Է� ����
			{
				strMoney.PrintF(_S(5388, "[%s] %d���� �̻󿡼��� �̿��� �����մϴ�."), _S(5390, "��ü ä�θ��"), CHANNEL_CHAT_NEEDLEVEL);
	...
	case CHATMSG_CHANNEL_TRADE:
		{
			if (_pNetwork->MyCharacterInfo.level < CHANNEL_TRADE_NEEDLEVEL) // 10���� �̻� �Է� ����
			{
				strMoney.PrintF(_S(5388, "[%s] %d���� �̻󿡼��� �̿��� �����մϴ�."), _S(5391, "�Ÿ� ä�θ��"), CHANNEL_TRADE_NEEDLEVEL);
				AddSysMessage(strMoney, SYSMSG_ERROR);
				bResult = FALSE;
			}

			int nNeedNas = strMessage.Length() * 10; // 1byte�� 10������ �Һ�
			...
			if (nNeedNas > _pNetwork->MyCharacterInfo.money)
			{
				...
				strMoney.PrintF(_S(5389, "[%s] ������ �����մϴ�.(%s ����)"), _S(5391, "�Ÿ� ä�θ��"), strNas);
				AddSysMessage(strMoney, SYSMSG_ERROR);
				bResult = FALSE;
			}
		}
...
void CChattingUI::ConditionsysMsg(ChattingMsgType cmtType)
{
	...
	case CHATMSG_NORMAL:
		{
			strMessage.PrintF(_S(5386, "[%s] 50���� �̻� �Է��� �� �ֽ��ϴ�."), _S(5390, "��ü ä�θ��"));
			AddSysMessage(strMessage, SYSMSG_USER, 0x6BD2FFFF);
		}
		break;
	case CHATMSG_TRADE:
		{
			strMessage.PrintF(_S(5387, "[%s] 10���� �̻� ������ �Ҹ��ϸ鼭 ����� �����մϴ�."), _S(5391, "�Ÿ� ä�θ��"));
...
void CChattingUI::ProcessParty( char* szMsg )
{
	...
	if( !strcmp( strCommand, _S( 124, "��û" ) ) )
		pParty->SendPartyInvite( 0, strOther );
	else if( !strcmp( strCommand, _S( 125, "Ż��" ) ) )
...
void CChattingUI::PetTransform( char* szMsg )
{
	...
	{
		AddSysMessage ( CTString("Npc Index�� �ٽ� �Է��ϼ���!") );
		return;
	}
...
void CChattingUI::PopupUserNotice()
{
	...
	CTString strDesc1 = _S(5609, "���� �ϰ��� �ϴ� ������ �Է��� �ּ���.");
	CTString strDesc2 = _S(5630, "(��, �ִ� 30�� ���� �Է��� �����մϴ�.)");

	m_msgID = MSGBOXMGR()->CreateMsgBox(_S(5610, "���� ����"), 
...
void CChattingUI::GluingMessage(CItems* items) //ROmeoST
{
	if (m_pChatInput && m_pChatInput->Items.size() < 5)
	{
		char msg[512];
		UBYTE	ubType = CheckChatType(m_pChatInput->GetString()[0]);
		ItemChat chat;
		chat.StartPos = ubType == CHATMSG_NORMAL ? CTString(m_pChatInput->GetString()).Length() + 1 : CTString(m_pChatInput->GetString()).Length();
		
		if (ubType == CHATMSG_NORMAL)
			chat.EndPos = chat.StartPos + CTString(items->ItemData->GetName()).Length() + 2;
		else if (ubType == CHATMSG_WHISPER)
			chat.EndPos = chat.StartPos + CTString(items->ItemData->GetName()).Length();
		else 
			chat.EndPos = chat.StartPos + CTString(items->ItemData->GetName()).Length() + 3;

		chat.item = items;
		m_pChatInput->Items.push_back(chat);
		sprintf(msg, "%s [%s]", m_pChatInput->GetString(), CTString(items->ItemData->GetName()).str_String);
		m_pChatInput->SetString((char*)msg);
	}
	else
	{
		AddSysMessage(CTString("Chat can contain a maximum of 5 items."), SYSMSG_ERROR);
	}
}
