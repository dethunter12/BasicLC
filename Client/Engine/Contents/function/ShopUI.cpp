...
#define SHOP_SLOT_SELL_MAX				(ITEM_COUNT_IN_INVENTORY_NORMAL + ITEM_COUNT_IN_INVENTORY_CASH_1 + ITEM_COUNT_IN_INVENTORY_CASH_2 + ITEM_COUNT_IN_INVENTORY_CASH_3 + ITEM_COUNT_IN_INVENTORY_CASH_4 + ITEM_COUNT_IN_INVENTORY_CASH_5 + ITEM_COUNT_IN_INVENTORY_CASH_6)
#define	SHOP_TRADE_SLOT_TOTAL			20
...
void CShopUI::CheckLease()
{
	if(m_nSelShopItemID >= 0)
	{
		...
		MsgBoxInfo.SetMsgBoxInfo( _S( 263, "����" ), UMBS_YESNO,	UI_SHOP, MSGCMD_SHOP_LEASE_ITEM );
...
void CShopUI::AddShopItem( int invenIdx, int nUniIndex, SQUAD llCount )
{
	...
	if (bBuyShop == FALSE)
	{
		if (invenIdx >= (ITEM_COUNT_IN_INVENTORY_NORMAL + ITEM_COUNT_IN_INVENTORY_CASH_1 + ITEM_COUNT_IN_INVENTORY_CASH_2 + ITEM_COUNT_IN_INVENTORY_CASH_3 + ITEM_COUNT_IN_INVENTORY_CASH_4 + ITEM_COUNT_IN_INVENTORY_CASH_5))
		{
			nTempTab = INVENTORY_TAB_CASH_6; //4-?6
			nTempIdx = invenIdx - (ITEM_COUNT_IN_INVENTORY_NORMAL + ITEM_COUNT_IN_INVENTORY_CASH_1 + ITEM_COUNT_IN_INVENTORY_CASH_2 + ITEM_COUNT_IN_INVENTORY_CASH_3 + ITEM_COUNT_IN_INVENTORY_CASH_4 + ITEM_COUNT_IN_INVENTORY_CASH_5);
		}
		else if (invenIdx >= (ITEM_COUNT_IN_INVENTORY_NORMAL + ITEM_COUNT_IN_INVENTORY_CASH_1 + ITEM_COUNT_IN_INVENTORY_CASH_2 + ITEM_COUNT_IN_INVENTORY_CASH_3 + ITEM_COUNT_IN_INVENTORY_CASH_4))
		{
			nTempTab = INVENTORY_TAB_CASH_5;
			nTempIdx = invenIdx - (ITEM_COUNT_IN_INVENTORY_NORMAL + ITEM_COUNT_IN_INVENTORY_CASH_1 + ITEM_COUNT_IN_INVENTORY_CASH_2 + ITEM_COUNT_IN_INVENTORY_CASH_3 + ITEM_COUNT_IN_INVENTORY_CASH_4);
		}
		else if (invenIdx >= (ITEM_COUNT_IN_INVENTORY_NORMAL + ITEM_COUNT_IN_INVENTORY_CASH_1 + ITEM_COUNT_IN_INVENTORY_CASH_2 + ITEM_COUNT_IN_INVENTORY_CASH_3))
		{
			nTempTab = INVENTORY_TAB_CASH_4;
			nTempIdx = invenIdx - (ITEM_COUNT_IN_INVENTORY_NORMAL + ITEM_COUNT_IN_INVENTORY_CASH_1 + ITEM_COUNT_IN_INVENTORY_CASH_2 + ITEM_COUNT_IN_INVENTORY_CASH_3);
		}
		else if (invenIdx >= (ITEM_COUNT_IN_INVENTORY_NORMAL + ITEM_COUNT_IN_INVENTORY_CASH_1 + ITEM_COUNT_IN_INVENTORY_CASH_2))
		{
			nTempTab = INVENTORY_TAB_CASH_3;
			nTempIdx = invenIdx - (ITEM_COUNT_IN_INVENTORY_NORMAL + ITEM_COUNT_IN_INVENTORY_CASH_1 + ITEM_COUNT_IN_INVENTORY_CASH_2);
		}
		else if (invenIdx >= (ITEM_COUNT_IN_INVENTORY_NORMAL + ITEM_COUNT_IN_INVENTORY_CASH_1))
		{
...
//dethunter12 adjust here
		...
		{
			// Add system message
			CUIManager::getSingleton()->GetChattingUI()->AddSysMessage(_S(100084, "��OAo��YAI ��IA��CO��I��U."), SYSMSG_ERROR);
			return;
		}
	#endif
	...
	if( iItem < SHOP_TRADE_SLOT_TOTAL )
	{
		...
		if( pItemData->GetFlag() & ITEM_FLAG_COUNT )
		{
			...
			if( !bBuyShop )
			{
				...
				if (nTempTab > INVENTORY_TAB_CASH_3)
					nTempIdx += ITEM_COUNT_IN_INVENTORY_CASH_3;

				if (nTempTab > INVENTORY_TAB_CASH_4)
					nTempIdx += ITEM_COUNT_IN_INVENTORY_CASH_4;

				if (nTempTab > INVENTORY_TAB_CASH_5)
					nTempIdx += ITEM_COUNT_IN_INVENTORY_CASH_5;

				if (nTempTab > INVENTORY_TAB_CASH_6)
					nTempIdx += ITEM_COUNT_IN_INVENTORY_CASH_6;
...
void CShopUI::updateUI()
{
	...
	case eSHOP_BUY:
		_updateBuy();
		strBtnString.PrintF(_S(253, "����"));
		break;

	case eSHOP_SELL:
		_updateSell();
		strBtnString.PrintF(_S(254, "�Ǹ�"));
		break;

	case eSHOP_LEASE:
		_updateLease();
		strBtnString.PrintF(_S(3046, "�뿩"));
		break;
		...
		if (pItems != NULL)
		{
			if (m_pArry[eARRY_TRADE] && m_pArry[eARRY_TRADE]->IsInside(x, y) && m_pArry[eARRY_TRADE]->GetHide() == FALSE)
			{
				...
				if (m_nSelTradeItemID < 0 || (pIcon != NULL && pIcon->getItems() != NULL && pIcon->getItems()->Item_UniIndex != pItems->Item_UniIndex))
				{
					...
					if (pItems->Item_Tab > INVENTORY_TAB_CASH_2)
						nTab += ITEM_COUNT_IN_INVENTORY_CASH_2;
					if (pItems->Item_Tab > INVENTORY_TAB_CASH_3)
						nTab += ITEM_COUNT_IN_INVENTORY_CASH_3;
					if (pItems->Item_Tab > INVENTORY_TAB_CASH_4)
						nTab += ITEM_COUNT_IN_INVENTORY_CASH_4;
					if (pItems->Item_Tab > INVENTORY_TAB_CASH_5)
						nTab += ITEM_COUNT_IN_INVENTORY_CASH_6;
					if (pItems->Item_Tab > INVENTORY_TAB_CASH_6)
						nTab += ITEM_COUNT_IN_INVENTORY_CASH_6;
			...
			else if (m_pArry[eARRY_SELECT] && m_pArry[eARRY_SELECT]->IsInside(x, y) && m_pArry[eARRY_SELECT]->GetHide() == FALSE)
			{
				...
				if (m_nSelShopItemID < 0 || (pIcon && pIcon->getItems() && pIcon->getItems()->Item_UniIndex != pItems->Item_UniIndex))
				{
					...
					if (pItems->Item_Tab > INVENTORY_TAB_CASH_2)
					{
						nInvenIdx += ITEM_COUNT_IN_INVENTORY_CASH_2;
					}

					if (pItems->Item_Tab > INVENTORY_TAB_CASH_3)
					{
						nInvenIdx += ITEM_COUNT_IN_INVENTORY_CASH_3;
					}
					if (pItems->Item_Tab > INVENTORY_TAB_CASH_4)
					{
						nInvenIdx += ITEM_COUNT_IN_INVENTORY_CASH_4;
					}
					if (pItems->Item_Tab > INVENTORY_TAB_CASH_5)
					{
						nInvenIdx += ITEM_COUNT_IN_INVENTORY_CASH_5;
					}
					if (pItems->Item_Tab > INVENTORY_TAB_CASH_6)
					{
						nInvenIdx += ITEM_COUNT_IN_INVENTORY_CASH_6;
					}
	...
	for( int i = 0; i < SHOP_TRADE_SLOT_TOTAL; i++ )
 	{
		...
		if (iIndex == 7340 ||
 		...
 		if (bRate == TRUE&& iShopID != 1654) //dethunter12 adjust here
