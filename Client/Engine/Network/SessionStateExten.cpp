		...
		case MSG_EX_STRING :
			{
				...
				switch(outType)
				{
					...
					{
						if (strIdx == 7999)
						_UIAutoHelp->SetGMNotice(strMessage, 0xE18600FF);
...
void CSessionState::RecieveMasterStoneMessage( CNetworkMessage* istr )
{
	...
	pUIManager->GetMixNew()->MixNewMasterStoneRep(pPacket->error, pPacket->success_type, pPacket->percent, pPacket->max_attack);
}
...
void CSessionState::ReceivePromoMessage(CNetworkMessage* istr)
{
	UBYTE	slType;
	(*istr) >> slType;
	CTString strMessage;
	...
	switch (slType)
	{
	case MSG_PROMO_KEY_USE:
	{
		strMessage.PrintF(_S(100538, "인벤토리의 공간이 부족 합니다. 인벤토리를 비운 후 다시 시도해주시기 바랍니다."));
	}
	break;
	case MSG_PROMO_KEY_NO_SPACE:
	{
		strMessage.PrintF(_S(100536, "인벤토리의 공간이 부족 합니다. 인벤토리를 비운 후 다시 시도해주시기 바랍니다."));
	}
	break;
	case MSG_PROMO_KEY_ALREADY_USED:
	{
		strMessage.PrintF(_S(100537, "인벤토리의 공간이 부족 합니다. 인벤토리를 비운 후 다시 시도해주시기 바랍니다."));
	}
	break;
