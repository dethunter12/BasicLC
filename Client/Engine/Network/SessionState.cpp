...
void CSessionState::SessionStateLoop(void)
{
	...
	while (bSomethingToDo && !IsDisconnected()) 
	{
		...
		if(!_bLoginProcess || STAGEMGR()->GetCurStage() == eSTAGE_CREATECHAR || STAGEMGR()->GetCurStage() == eSTAGE_SELCHAR)
		{
			...
			static DWORD SendTodinhoTime = _pTimer->GetHighPrecisionTimer().GetMilliseconds();//Cloud ant-hack
...
#endif
			{
				...
				if (STAGEMGR()->GetCurStage() == eSTAGE_GAMEPLAY) {
					...
					if (_pNetwork->MyCharacterInfo.bWildPetRide | _pNetwork->MyCharacterInfo.bPetRide) {
						...
						nmAliveMsg << (SBYTE)((CPlayerEntity*)CEntity::GetPlayerEntity(0))->SouthLC_03();//check attackspeed 
						nmAliveMsg << (UBYTE)_pNetwork->MyCharacterInfo.CloudGomesAS01;//check attackspeed 
						nmAliveMsg << (FLOAT)((CPlayerEntity*)CEntity::GetPlayerEntity(0))->GetRunspeed(); //check runspeed
						nmAliveMsg << (FLOAT)((CPlayerEntity*)CEntity::GetPlayerEntity(0))->SouthLC_01(); //check runspeed																		  

					}//nmAliveMsg<< (SBYTE)_pNetwork->MyCharacterInfo.attackspeed;
					//nmAliveMsg<< (FLOAT)_pNetwork->MyCharacterInfo.runspeed;
					nmAliveMsg << (FLOAT)_pNetwork->MyCharacterInfo.animation;
					nmAliveMsg << (INDEX)_pNetwork->MyCharacterInfo.animationId;
					nmAliveMsg << (ULONG)_pNetwork->MyCharacterInfo.animationFlag;
					nmAliveMsg << (FLOAT)_pNetwork->MyCharacterInfo.animationStrength;
					nmAliveMsg << (INDEX)_pNetwork->MyCharacterInfo.animationGroupId;
				}
				else
				{
					nmAliveMsg << (SBYTE)0;
				...
				nmAliveMsg << (FLOAT)_pNetwork->MyCharacterInfo.CloudGomesSS01;//check skillspeed
				nmAliveMsg << (FLOAT)((CPlayerEntity*)CEntity::GetPlayerEntity(0))->SouthLC_04();//check skillspeed																				 
				nmAliveMsg << (DWORD)(_pNetwork->MyCharacterInfo.skillSpeed);
				nmAliveMsg << (UBYTE)_pNetwork->m_ubGMLevel;
				//####END
#endif		    //Cloud ant-hack
			...
			if ((DWORD)GetModuleHandleA("speedhack.dll"))
				_pNetwork->SendInput();
			if ((DWORD)GetModuleHandleA("speedhack-i386.dll"))
				_pNetwork->SendInput();
			if ((DWORD)GetModuleHandleA("speedhack-x86_64.dll"))
				_pNetwork->SendInput();
			if ((DWORD)GetModuleHandleA("speedhack-x86.dll"))
				_pNetwork->SendInput();
			...
			else GetThisTime = tmpTime2;
#endif

			/*			if (_pTimer->tmTimeDelay > 0.07f)
						{ // ????? ?? ????? ??? (TickCount?? 0.05f?? ?????? 0.05f???? ????????? ??????? ?????? ????.
							_pNetwork->SendUsingSpeedHack();
						}*/
			static int checktime = 0;//Cloud ant-hack

			if (tmpTime - SendTodinhoTime >= 1000)//Cloud ant-hack
			{

				_pNetwork->New_Ranking_Reset();

				if (_pTimer->tmTimeDelay > 0.051f)
				{
					if (checktime < 10)
					{
						checktime++;
						if (_pNetwork->m_ubGMLevel == 10)// gm debug only
						{
							CTString debug;

							debug.PrintF("WARNING count %d %f", checktime, _pTimer->tmTimeDelay);
							pUIManager->GetChattingUI()->AddSysMessage(debug, SYSMSG_ERROR);
						}
					}
					else
						_pNetwork->SendInput();
	...
	// [KH_070316] 프리미엄 메모리 관련 추가 및 변경
					...
					case MSG_DOMINATION_EVENT:
					{
						...
						switch(subtype)
						{
							case MSG_DOMINATION_EVENT_REGISTER_REP:
							{
								int error;
								(nmMessage) >> error;
								if (error == 0) {
									pUIManager->GetChattingUI()->AddSysMessage( _s("You can't register now, event is closed or running!"), SYSMSG_ERROR );
								} else if (error == 1) {
									pUIManager->GetChattingUI()->AddSysMessage( _s("You are already registered!"), SYSMSG_ERROR );
								} else if (error == 2) {
									pUIManager->GetChattingUI()->AddSysMessage( _s("Successfully registered!"), SYSMSG_ERROR );
								}
							}
							break;
							case MSG_DOMINATION_EVENT_END:
							{
								pUIManager->GetChattingUI()->AddSysMessage( _s("Event has been ended!"), SYSMSG_ERROR );
							}
							break;
							case MSG_DOMINATION_EVENT_UPDATE_POINT:
							{
								int pointsA, pointsB, status, sbFlagJoin, zone;
								...
								pUIManager->GetRadar()->UpdateDominationInfo(pointsA, pointsB, status);

								for (int i = 0; i < 3; i++) {
									(nmMessage) >> status;
									pUIManager->GetRadar()->UpdateDominationInfoFlag(i, status);
								}
								
								_pNetwork->MyCharacterInfo.dominationJoin = sbFlagJoin;
								_pNetwork->MyCharacterInfo.dominationZone = zone;
...
void CSessionState::ReceiveSkillMessage( CNetworkMessage *istr )
{
	...
	case MSG_SKILL_ERROR:
		{
			...
			case MSG_SKILL_ERROR_CANNOT_ZONE:
				...
				break;
			case MSG_SKILL_ERROR_CANNOT_ARTIFACT_INVIS:
				pUIManager->GetChattingUI()->AddSysMessage(_S(100073, "Not possible use invisibility with artifact"), SYSMSG_ERROR);
				break;
			}
	...
	case MSG_SKILL_ARENA_HEAL:
	{
		...
		{
		case 0:
			strMessage = _S(100065, "레벨이 맞지 않아 마법을 받을 수 없습니다.");
			break;
		case 1:
			strMessage = _S(100066, "마법을 받는데 성공하였습니다.");
			break;
		}
...
*/
	...
	case MSG_SYS_PLAYER_DEAD:
	{
		...
		(*istr) >> df;
		CTString msg;
		msg.PrintF(_S(100072, "The player %s killed the enemy %s"), of, df);
	...
	case MSG_SYS_EVENT_SUMMON_NPC_DEAD:
	{
		...
		(*istr) >> total;
		CTString msg;
		msg.PrintF(_S(100526, "%s found one, %d left!"), of, total);
		pUIManager->GetChattingUI()->AddSysMessage(msg, SYSMSG_RED, DEF_UI_COLOR_ORANGE);
	}
	default:
		{
...
#ifdef CUBE_RANKING_12202022
	case MSG_QUEST_RANKING_CUBE_REP:
	{
		SBYTE		sbMultiCount;
		CTString	strName;
		INDEX		iCubeKills;
		...
		pUIManager->GetRankingCube()->ClearRankingList();
		...
		for (int i = 0; i < sbMultiCount; ++i)
		{
			...
			(*istr) >> iCubeKills;
			(*istr) >> iJob;
			pUIManager->GetRankingCube()->AddToRankingList(strName, iCubeKills, iJob);
		}

		if (!pUIManager->GetRankingCube()->IsVisible())
			pUIManager->GetRankingCube()->OpenRankingList();
		else
			pUIManager->GetRankingCube()->RefreshRankingList();
	}
	break;
#endif
...
#ifdef GLADES_RANKING_04182023
	case MSG_QUEST_RANKING_GLADES_REP:
	{
		SBYTE		sbMultiCount;
		CTString	strName;
		INDEX		iGladeKills;
		...
		pUIManager->GetRankingGlades()->ClearRankingList();
		...
		for (int i = 0; i < sbMultiCount; ++i)
		{
			...
			(*istr) >> iGladeKills;
			(*istr) >> iJob;
			pUIManager->GetRankingGlades()->AddToRankingList(strName, iGladeKills, iJob);
		}

		if (!pUIManager->GetRankingGlades()->IsVisible())
			pUIManager->GetRankingGlades()->OpenRankingList();
		else
			pUIManager->GetRankingGlades()->RefreshRankingList();
	}
	break;
#endif
...
void CSessionState::ReceiveBattlePassMessage(CNetworkMessage* istr)
{
	...
	case MSG_BATTLE_PASS_ERROR_EXP_NEEDED:
		strMessage.PrintF(_S(100513, "Sorry, you don't have the required exp for next battle pass"));
		pUIManager->GetChattingUI()->AddSysMessage(strMessage, SYSMSG_ERROR);
		break;
	case MSG_BATTLE_PASS_ERROR_NOT_EXIST_NPC:
		strMessage.PrintF(_S(100516, "This npc doesn't exist"));
		pUIManager->GetChattingUI()->AddSysMessage(strMessage, SYSMSG_ERROR);
		break;
	case MSG_BATTLE_PASS_ERROR_NOCONDITION_LEVEL:
		strMessage.PrintF(_S(100514, "You don't have the required level for next battle pass"));
		pUIManager->GetChattingUI()->AddSysMessage(strMessage, SYSMSG_ERROR);
		break;
	case MSG_BATTLE_PASS_ERROR_EMPTY_SLOT:
		strMessage.PrintF(_S(100515, "You need more empty slot in your inventory"));
		pUIManager->GetChattingUI()->AddSysMessage(strMessage, SYSMSG_ERROR);
		break;
	...
	case MSG_BATTLE_PASS_REP:
	{
		...
		{
			CUIMsgBox_Info	MsgBoxInfo;
			strMessage.PrintF(_S(100511, "Prestige"));
			MsgBoxInfo.SetMsgBoxInfo(strMessage, UMBS_OK, UI_NONE, MSGCMD_NULL);
			strMessage.PrintF(_S(100510, "Congratulations! You achieved a new prestige level! %d"), pPack->level);
			MsgBoxInfo.AddString(strMessage);
			pUIManager->CreateMessageBox(MsgBoxInfo);
		}
	}
