...
#ifdef ENABLE_ENCRYPT_TEXTURE
	BOOL crypt = FALSE;
	BOOL crypt2 = FALSE;
	BOOL crypt_AssasinPL = FALSE;
	if (iVersion == 17)
	{
		iVersion = 4;
		crypt = TRUE;
...
//안태훈 수정 시작	//(Encode Data)(0.1)
			if(iNewVer == TEX_DATA_VER || iNewVer == 5)
			{
				...
				if (crypt2)
				{
					td_mexWidth -= inFile->GetStreamSize();
					td_mexWidth += 12;
				}
				else if (crypt_AssasinPL)
				{
					td_mexWidth -= inFile->GetStreamSize();
					td_mexWidth += 20;
				}
					...
					*inFile >> td_mexWidth;
				td_mexWidth = DecodeSimpleTex(td_mexWidth, checker);
#else
