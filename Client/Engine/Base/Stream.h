 ...
 */
#ifdef DECRYPTXOR
inline void CTStream::Decrypt_XOR()
{
	UBYTE key[20] = { 0x01, 0x09, 0x05, 0x03, 0x08, 0x06, 0x08, 0x01, 0x02, 0x03, 0x05, 0x06, 0x00, 0x01, 0x05, 0x03, 0x00, 0x09, 0x01, 0x02 };

	UBYTE* cur = strm_pubCurrentPos;
	int j = 0;
	while (cur != strm_pubBufferEnd)
	{
		*cur = *cur ^ key[0];
		j++;
		if (j > 4)
