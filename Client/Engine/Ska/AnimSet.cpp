...
#define ANIMSET_ENCRYPT  15 //dethunter12 tari
...
#define CODE_CRYPT_ANIMSET_1 22
#define CODE_CRYPT_ANIMSET_2 39
#define CODE_CRYPT_ANIMSET_3 6
#define CODE_CRYPT_ANIMSET_4 12
#define CODE_CRYPT_ANIMSET_5 43

template<class Type>
inline Type EncryptSimpleAnimset(Type val, UBYTE &ubChecker)
{
	if(sizeof(Type) == 4)
	{
		ULONG ulChecker = 0;
		ubChecker += (UBYTE)CODE_CRYPT_ANIMSET_1;
		ulChecker |= ULONG(ubChecker) << 24;
		ubChecker += (UBYTE)CODE_CRYPT_ANIMSET_2;
		ulChecker |= ULONG(ubChecker) << 16;
		ubChecker += (UBYTE)CODE_CRYPT_ANIMSET_3;
		ulChecker |= ULONG(ubChecker) << 8;
		ubChecker += (UBYTE)CODE_CRYPT_ANIMSET_4;
		ulChecker |= ULONG(ubChecker) << 0;
		ubChecker += CODE_CRYPT_ANIMSET_5;
...
template<class Type>
inline Type DecryptSimpleAnimset(Type val, UBYTE &ubChecker)
{
	if(sizeof(Type) == 4)
	{
		ULONG ulChecker = 0;
...
// write to stream
void CAnimSet::Write_t_enc(CTStream *ostrFile)
{
  UBYTE checker = ANIMSET_ENCRYPT;
  // write id
  ostrFile->WriteID_t(CChunkID(ANIMSET_ID));
  // write version
  (*ostrFile)<<(INDEX)ANIMSET_ENCRYPT;

  INDEX ctan = as_Anims.Count();
  (*ostrFile)<<EncryptSimpleAnimset(ctan, checker);
  ...
  for(int ian=0;ian<ctan;ian++)
  {
    ...
    (*ostrFile)<<EncryptSimpleAnimset(ctbe, checker);
    ...
    for(int ibe=0;ibe<ctbe;ibe++)
    {
      ...
      (*ostrFile)<<EncryptSimpleAnimset(ctp, checker);
      // for each position
      ...
      (*ostrFile)<<EncryptSimpleAnimset(ctRotations, checker);
      // for each rotation
    ...
    (*ostrFile)<<EncryptSimpleAnimset(ctme, checker);
...
// read from stream
void CAnimSet::Read_t_enc(CTStream *istrFile) //dethunter12 tari
{
  UBYTE checker = ANIMSET_ENCRYPT;
  INDEX ctan;
  // read anims count
  (*istrFile)>>ctan; ctan = DecryptSimpleAnimset(ctan, checker);
  // create anims array
  as_Anims.New(ctan);
  ...
  for(int ian=0;ian<ctan;ian++) {
    ...
    (*istrFile)>>ctbe; ctbe = DecryptSimpleAnimset(ctbe, checker);
    ...
    for(INDEX ibe=0;ibe<ctbe;ibe++) {
      ...
      INDEX ctp;
      // read pos array
      (*istrFile)>>ctp; ctp = DecryptSimpleAnimset(ctp, checker);
      be.be_apPos.New(ctp);
      for(INDEX ip=0;ip<ctp;ip++) {
      ...
      (*istrFile)>>ctr; ctr = DecryptSimpleAnimset(ctr, checker);
    ...
    (*istrFile)>>ctme; ctme = DecryptSimpleAnimset(ctme, checker);
...
void CAnimSet::Read_t(CTStream *istrFile)
{
  ...
  if(iFileVersion != ANIMSET_VERSION) //dethunter12 tari
  {
 		if (iFileVersion == ANIMSET_ENCRYPT)
	  {
		  Read_t_enc(istrFile);
		  return;
	  }
