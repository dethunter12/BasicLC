...
#define CODE_CRYPT_SKELETON_1 42
#define CODE_CRYPT_SKELETON_2 74
#define CODE_CRYPT_SKELETON_3 31
#define CODE_CRYPT_SKELETON_4 9
#define CODE_CRYPT_SKELETON_5 27
template<class Type>
inline Type EncryptSimpleSkeleton(Type val, UBYTE &ubChecker)
{
	if(sizeof(Type) == 4)
	{
		ULONG ulChecker = 0;
		ubChecker += (UBYTE)CODE_CRYPT_SKELETON_1;
		ulChecker |= ULONG(ubChecker) << 24;
		ubChecker += (UBYTE)CODE_CRYPT_SKELETON_2;
		ulChecker |= ULONG(ubChecker) << 16;
		ubChecker += (UBYTE)CODE_CRYPT_SKELETON_3;
		ulChecker |= ULONG(ubChecker) << 8;
		ubChecker += (UBYTE)CODE_CRYPT_SKELETON_4;
		ulChecker |= ULONG(ubChecker) << 0;
		ubChecker += CODE_CRYPT_SKELETON_5;
		return Type( val ^ ulChecker);
	}
	else return val;
}
template<class Type>
inline Type DecryptSimpleSkeleton(Type val, UBYTE &ubChecker)
{
	if(sizeof(Type) == 4)
	{
		ULONG ulChecker = 0;
...
// write to stream
void CSkeleton::Write_t_enc(CTStream *ostrFile)
{
  UBYTE checker = SKELETON_ENCRYPT;
  INDEX ctslods = skl_aSkeletonLODs.Count();
  // write id
...
void CSkeleton::Write_t(CTStream *ostrFile)
{
  ...
  INDEX ctslods = skl_aSkeletonLODs.Count();
  ostrFile->WriteID_t(CChunkID(SKELETON_ID));
  (*ostrFile)<<(INDEX)SKELETON_ENCRYPT;
  (*ostrFile)<<EncryptSimpleSkeleton(ctslods, checker);
  for(INDEX islod=0;islod<ctslods;islod++) {
    SkeletonLOD &slod = skl_aSkeletonLODs[islod];
    (*ostrFile)<<slod.slod_fnSourceFile;
    (*ostrFile)<<slod.slod_fMaxDistance;
    INDEX ctb = slod.slod_aBones.Count();
    (*ostrFile)<<EncryptSimpleSkeleton(ctb, checker);
...
// read from stream
void CSkeleton::Read_t_enc(CTStream *istrFile)
{
  UBYTE checker = SKELETON_ENCRYPT;
  INDEX ctslods;
  (*istrFile)>>ctslods; ctslods = DecryptSimpleSkeleton(ctslods, checker);
  if(ctslods>0) {
  ...
  for(INDEX islod=0;islod<ctslods;islod++) {
    ...
    INDEX ctb;
    (*istrFile)>>ctb; ctb = DecryptSimpleSkeleton(ctb, checker);
...
void CSkeleton::Read_t(CTStream *istrFile)
{
  ...
  if(iFileVersion != SKELETON_VERSION) {
	  if (iFileVersion == SKELETON_ENCRYPT)
	  {
		  Read_t_enc(istrFile);
		  return;
	  }
