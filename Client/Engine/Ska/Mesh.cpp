...
#define MESH_ENCRYPT  18 //dethunter12 TARI
//?????? ???? ??
#define MESH_ID       "MESH"
...
#define CODE_CRYPT_MESH_1 23
#define CODE_CRYPT_MESH_2 42
#define CODE_CRYPT_MESH_3 7
#define CODE_CRYPT_MESH_4 18
#define CODE_CRYPT_MESH_5 29
template<class Type>
inline Type EncryptSimpleMesh(Type val, UBYTE &ubChecker)
{
	if(sizeof(Type) == 4)
	{
		ULONG ulChecker = 0;
		ubChecker += (UBYTE)CODE_CRYPT_MESH_1;
		ulChecker |= ULONG(ubChecker) << 24;
		ubChecker += (UBYTE)CODE_CRYPT_MESH_2;
		ulChecker |= ULONG(ubChecker) << 16;
		ubChecker += (UBYTE)CODE_CRYPT_MESH_3;
		ulChecker |= ULONG(ubChecker) << 8;
		ubChecker += (UBYTE)CODE_CRYPT_MESH_4;
		ulChecker |= ULONG(ubChecker) << 0;
		ubChecker += CODE_CRYPT_MESH_5;
		return Type( val ^ ulChecker);
	}
	else return val;
}
template<class Type>
inline Type DecryptSimpleMesh(Type val, UBYTE &ubChecker)
{
	if(sizeof(Type) == 4)
	{
		ULONG ulChecker = 0;
...
void CMesh::Read_t(CTStream *istrFile)
{
	...
	else if (iFileVersion == MESH_ENCRYPT) Read_t_enc(istrFile, bHasContext);//dethunter12 TARi
	else
	{
...
void CMesh::Read_t_enc(CTStream *istrFile, BOOL bHasContext)
{
	UBYTE checker = MESH_ENCRYPT;
	// Read size of mesh
	SLONG slMeshSize = 0;
	(*istrFile) >> slMeshSize;
	...
	(*istrFile) >> ctmlod;		ctmlod = DecryptSimpleMesh(ctmlod, checker);
	if(ctmlod>0) 
	{
	...
	for(INDEX imlod=0;imlod<ctmlod;imlod++) 
	{
		...
		(*istrFile) >> ctmvwi;		ctmvwi = DecryptSimpleMesh(ctmvwi, checker);
		(*istrFile) >> ctmwm;		ctmwm = DecryptSimpleMesh(ctmwm, checker);
		(*istrFile) >> ctmuvm;		ctmuvm = DecryptSimpleMesh(ctmuvm, checker);
		(*istrFile) >> ctmvx;		ctmvx = DecryptSimpleMesh(ctmvx, checker);
		(*istrFile) >> ctmsrf;		ctmsrf = DecryptSimpleMesh(ctmsrf, checker);
		(*istrFile) >> ctmmm;		ctmmm = DecryptSimpleMesh(ctmmm, checker);
		...
		// read flags
		(*istrFile) >> mlod.mlod_ulFlags;			mlod.mlod_ulFlags = DecryptSimpleMesh(mlod.mlod_ulFlags, checker);
		...
		for(INDEX imsrt=0;imsrt<ctmsrf;imsrt++) 
		{
			...
			// read first vertex
			(*istrFile) >> msrf.msrf_iFirstVertex;		msrf.msrf_iFirstVertex = DecryptSimpleMesh(msrf.msrf_iFirstVertex, checker);
			// read vertices count
			(*istrFile) >> msrf.msrf_ctVertices;		msrf.msrf_ctVertices = DecryptSimpleMesh(msrf.msrf_ctVertices, checker);

			// read tris count
			(*istrFile) >> ctTris;		ctTris = DecryptSimpleMesh(ctTris, checker);
			...
			// read surface flags
			(*istrFile) >> msrf.msrf_ulFlags;		msrf.msrf_ulFlags = DecryptSimpleMesh(msrf.msrf_ulFlags, checker);
			
			// read table of surface relative weight map indices
			INDEX ctirt;
			(*istrFile) >> ctirt;		ctirt = DecryptSimpleMesh(ctirt, checker);
			if(ctirt>0) 
			{
			...
			INDEX bShaderExists;
			(*istrFile) >> bShaderExists;		bShaderExists = DecryptSimpleMesh(bShaderExists, checker);
			// if shader exists read its params
			if(bShaderExists) 
			{
				INDEX cttx,cttc,ctcol,ctfl;
				// read color count
				(*istrFile) >> ctcol;		ctcol = DecryptSimpleMesh(ctcol, checker);
				// read float count
				(*istrFile) >> ctfl;		ctfl = DecryptSimpleMesh(ctfl, checker);
				// read texture count
				(*istrFile) >> cttx;		cttx = DecryptSimpleMesh(cttx, checker);
				// read texture coords count
				(*istrFile) >> cttc;		cttc = DecryptSimpleMesh(cttc, checker);
...
//안태훈 수정 끝	//(For Performance)(0.1)
				...
				// read shader flags
				ULONG ulFlags;
				(*istrFile) >> ulFlags;		ulFlags = DecryptSimpleMesh(ulFlags, checker);
				pshpShaderParams->sp_ulFlags = ulFlags;
				// read shader colors
				for(INDEX icol=0;icol<ctcol;icol++) 
				{
					COLOR colColor;
					(*istrFile) >> colColor;		colColor = DecryptSimpleMesh(colColor, checker);
				...
				for(INDEX itc=0;itc<cttc;itc++) 
				{
					...
					(*istrFile) >> iTexCoorsIndex;		iTexCoorsIndex = DecryptSimpleMesh(iTexCoorsIndex, checker);
		...
		if(ctmwm) 
		{
			...
			for(INDEX imwm=0;imwm<ctmwm;imwm++) 
			{
				...
				INDEX ctmvw;
				(*istrFile) >> ctmvw;		ctmvw = DecryptSimpleMesh(ctmvw, checker);
				// Read vertex weights
				if(ctmvw>0) 
				{
		...
		if(ctmmm>0) 
		{
			...
			for(INDEX immm=0;immm<ctmmm;immm++) 
			{
				...
				// read bRelative
				(*istrFile) >> mmm.mmp_bRelative;		mmm.mmp_bRelative = DecryptSimpleMesh(mmm.mmp_bRelative, checker);
				// read morph sets count
				INDEX ctmvm;
				(*istrFile) >> ctmvm;		ctmvm = DecryptSimpleMesh(ctmvm, checker);
		...
		if(mlod.mlod_aUVMaps.Count() > 0 && (bHaveTangentStream || _bSkaStudioApp))
		{
			...
			{
				ASSERT(FALSE && "Tagent ????? ??????? Vertex ????? ?븻, ????? ???[0]?? ???? ?????? ???.");
			}
		}
...
//?????? ???? ??	//(For Performance)(0.1)
		...
		for(INDEX iSurface=0; iSurface<mlod.mlod_aSurfaces.Count(); ++iSurface) 
		{
			...
			else if( !strName.RemovePrefix(strTagIndicator) ) continue;	//?????? VTCT_VERTEX???
			...
			if(vtct == VTCT_VERTEX)
				mlod.mlod_aSkaTags.Expand( mlod.mlod_aSkaTags.Count() + surface.msrf_ctVertices );
			else if(vtct == VTCT_TRIANGLE || vtct == VTCT_GROUP)	//??? ????? Tag?? ???.
				mlod.mlod_aSkaTags.Expand( mlod.mlod_aSkaTags.Count() + 1 );
				//mlod.mlod_aSkaTags.Expand( mlod.mlod_aSkaTags.Count() + INDEX(floorf(surface.msrf_ctVertices/3.0f)) );
...
void CMesh::Write_t_enc(CTStream *ostrFile)
{
	UBYTE checker = MESH_ENCRYPT;
	ostrFile->WriteID_t(CChunkID(MESH_ID));
	// write version
	(*ostrFile)<<(INDEX)MESH_ENCRYPT;
	...
	INDEX ctmlod = msh_aMeshLODs.Count();
	// write mlod count
	(*ostrFile) << EncryptSimpleMesh(ctmlod, checker);
	// for each lod in mesh
	for(INDEX imlod=0;imlod<ctmlod;imlod++) 
	{
		...
		(*ostrFile) << EncryptSimpleMesh(ctmvwi, checker);
		(*ostrFile) << EncryptSimpleMesh(ctmwm, checker);
		(*ostrFile) << EncryptSimpleMesh(ctmuvm, checker);
		(*ostrFile) << EncryptSimpleMesh(ctmvx, checker);
		(*ostrFile) << EncryptSimpleMesh(ctmsrf, checker);
		(*ostrFile) << EncryptSimpleMesh(ctmmm, checker);
		...
		// write flags
		(*ostrFile) << EncryptSimpleMesh(mlod.mlod_ulFlags, checker);
		...
		for(INDEX imsrt=0;imsrt<ctmsrf;imsrt++) 
		{
			...
			// write first vertex
			(*ostrFile) << EncryptSimpleMesh(msrf.msrf_iFirstVertex, checker);
			// write vertices count
			(*ostrFile) << EncryptSimpleMesh(msrf.msrf_ctVertices, checker);

			// write tris count
			(*ostrFile) << EncryptSimpleMesh(ctTris, checker);
			...
			(*ostrFile) << EncryptSimpleMesh(bShaderExists, checker);
			if(bShaderExists) 
			{
...
//?????? ???? ??	//(For Performance)(0.1)
				...
				// write color count 
				(*ostrFile) << EncryptSimpleMesh(ctcol, checker);
				// write float count 
				(*ostrFile) << EncryptSimpleMesh(ctfl, checker);
				// write texture count 
				(*ostrFile) << EncryptSimpleMesh(cttx, checker);
				// write texture coords count 
				(*ostrFile) << EncryptSimpleMesh(cttc, checker);
				...
				(*ostrFile) << EncryptSimpleMesh(ulFlags, checker);
				// write shader colors
				for(INDEX icol=0;icol<ctcol;icol++)
				{
					COLOR colColor = msrf.msrf_ShadingParams.sp_acolColors[icol];
					(*ostrFile) << EncryptSimpleMesh(colColor, checker);
				}
				// write shader floats
				for(INDEX ifl=0;ifl<ctfl;ifl++)
				{
				...
				for(INDEX itc=0;itc<cttc;itc++)
				{
					...
					(*ostrFile) << EncryptSimpleMesh(iTexCoorsIndex, checker);
				}
			}
		}
		...
		for(INDEX imwm=0;imwm<ctmwm;imwm++) 
		{
			...
			(*ostrFile) << EncryptSimpleMesh(ctmvw, checker);
			if(ctmvw>0) 
			{
		...
		for(INDEX immm=0;immm<ctmmm;immm++) 
		{
			...
			// write bRelative
			(*ostrFile) << EncryptSimpleMesh(mmm.mmp_bRelative, checker);
			// write morph count
			(*ostrFile) << EncryptSimpleMesh(ctmvm, checker);
			// write morph sets
