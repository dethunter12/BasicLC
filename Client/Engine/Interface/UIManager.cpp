...
#define MAX_MY_NAME_CUSTOM_SIZE 24
#define MAX_GUILD_NAME_SIZE		16 
#define MAX_GUILD_NAME_CUSTOM_SIZE 24

#define WIDTH_INPUT_CUSTOM_NAME	250
...
//	InitRenderTarget();
	...
	ItemNameColorInI();
	g_threadCheck = boost::thread(boost::bind(&CUIManager::CRCCheck, this));

#ifdef NOTICE_PICKUP_ITEM
	m_pNoticePickupItemTexture = NULL;
#endif
}
...
#ifdef NOTICE_PICKUP_ITEM
	if (m_pNoticePickupItemTexture)
	{
		_pTextureStock->Release(m_pNoticePickupItemTexture);
		m_pNoticePickupItemTexture = NULL;
	}
#endif
}
...
void CUIManager::Create()
{
    ...
    #ifdef ENABLE_HP_BAR
	m_rtHPLine.SetRect(247, 148, 334, 155); //dethunter12
    #endif
...
#ifdef PREMIUM_CHAR
	#ifdef ENABLE_HP_BAR
	m_rtPremiumNameL.SetUV( 238, 5, 240, 55, fTexWidth, fTexHeight ); //dethunter12
	m_rtPremiumNameC.SetUV( 240, 5, 241, 55, fTexWidth, fTexHeight );
	m_rtPremiumNameR.SetUV( 241, 5, 304, 55, fTexWidth, fTexHeight );
	#else
...
#define ST_DEADLY		2
	...
	m_apUIs[UI_INVEN_CASH1]->Create(NULL, 786, 346, INVEN_CASH_BAG_W, INVEN_CASH_BAG_NEW_H);
#else
	...
	m_apUIs[UI_INVEN_CASH2]->Create(NULL, 786, 103, INVEN_CASH_BAG_W, INVEN_CASH_BAG_NEW_H);
#else
	...
	m_apUIs[UI_INVEN_CASH3] = new CUIInvenCashBag(INVEN_TAB_CASH3);
#ifdef EXPANSION_INVENTORY
	m_apUIs[UI_INVEN_CASH3]->Create(NULL, 786, 103, INVEN_CASH_BAG_W, INVEN_CASH_BAG_NEW_H);
#else
	m_apUIs[UI_INVEN_CASH3]->Create(NULL, 786, 103, INVEN_CASH_BAG_W, INVEN_CASH_BAG_H);
#endif
	if (CUIInvenCashBag* pCashBag = dynamic_cast<CUIInvenCashBag*>(m_apUIs[UI_INVEN_CASH3]))
		pCashBag->SetRearrangeOrderID(UI_INVEN_CASH3);

	m_apUIs[UI_INVEN_CASH4] = new CUIInvenCashBag(INVEN_TAB_CASH4);
#ifdef EXPANSION_INVENTORY
	m_apUIs[UI_INVEN_CASH4]->Create(NULL, 786, 103, INVEN_CASH_BAG_W, INVEN_CASH_BAG_NEW_H);
#else
	m_apUIs[UI_INVEN_CASH4]->Create(NULL, 786, 103, INVEN_CASH_BAG_W, INVEN_CASH_BAG_H);
#endif
	if (CUIInvenCashBag* pCashBag = dynamic_cast<CUIInvenCashBag*>(m_apUIs[UI_INVEN_CASH4]))
		pCashBag->SetRearrangeOrderID(UI_INVEN_CASH4);

	//dethunter12 add
	m_apUIs[UI_INVEN_CASH5] = new CUIInvenCashBag(INVEN_TAB_CASH5);
#ifdef EXPANSION_INVENTORY
	m_apUIs[UI_INVEN_CASH5]->Create(NULL, 786, 103, INVEN_CASH_BAG_W, INVEN_CASH_BAG_NEW_H);
#else
	m_apUIs[UI_INVEN_CASH5]->Create(NULL, 786, 103, INVEN_CASH_BAG_W, INVEN_CASH_BAG_H);
#endif
	if (CUIInvenCashBag* pCashBag = dynamic_cast<CUIInvenCashBag*>(m_apUIs[UI_INVEN_CASH5]))
		pCashBag->SetRearrangeOrderID(UI_INVEN_CASH5);
		
	m_apUIs[UI_INVEN_CASH6] = new CUIInvenCashBag(INVEN_TAB_CASH6);
#ifdef EXPANSION_INVENTORY
	m_apUIs[UI_INVEN_CASH6]->Create(NULL, 786, 103, INVEN_CASH_BAG_W, INVEN_CASH_BAG_NEW_H);
#else
	m_apUIs[UI_INVEN_CASH6]->Create(NULL, 786, 103, INVEN_CASH_BAG_W, INVEN_CASH_BAG_H);
#endif
	if (CUIInvenCashBag* pCashBag = dynamic_cast<CUIInvenCashBag*>(m_apUIs[UI_INVEN_CASH6]))
		pCashBag->SetRearrangeOrderID(UI_INVEN_CASH6);
...
#ifdef NOTICE_PICKUP_ITEM
	m_pNoticePickupItemTexture = _pTextureStock->Obtain_t(CTString("Data\\Interface\\NoticePickupItem.tex"));

	fTexWidth = m_pNoticePickupItemTexture->GetPixWidth();
	fTexHeight = m_pNoticePickupItemTexture->GetPixHeight();
	m_rtNoticePickupItemL.SetUV(0, 0, 32, 19, fTexWidth, fTexHeight);
	m_rtNoticePickupItem¬³.SetUV(39, 0, 46, 19, fTexWidth, fTexHeight);
	m_rtNoticePickupItemR.SetUV(54, 0, 86, 19, fTexWidth, fTexHeight);
#endif
}
...
#ifdef NOTICE_PICKUP_ITEM
	if (m_pNoticePickupItemTexture && !m_vecNoticePickupItem.empty())
	{
		pdp->InitTextureData(m_pNoticePickupItemTexture);

		SQUAD nCurTime = _pTimer->GetHighPrecisionTimer().GetMilliseconds();
		LONG nPosX = NOTICE_PICKUP_ITEM_ICON_POSITION == 0 ? NOTICE_PICKUP_ITEM_ICON_SHIFT_X : pdp->GetWidth() - NOTICE_PICKUP_ITEM_ICON_SHIFT_X;
		LONG nPosY = pdp->GetHeight() - NOTICE_PICKUP_ITEM_ICON_SHIFT_Y;

		std::vector<CNoticePickupItem>::iterator it = m_vecNoticePickupItem.begin();

		while (it != m_vecNoticePickupItem.end())
		{
			COLOR colBlend = 0xFFFFFFFF;

			if ((nCurTime - it->nTime) <= NOTICE_PICKUP_ITEM_FIDE_IN_TIME)
			{
				FLOAT fRatio = (nCurTime - it->nTime) / FLOAT(NOTICE_PICKUP_ITEM_FIDE_IN_TIME);
				colBlend = 0xFFFFFF00 | UBYTE(0xFF * fRatio);
			}
			else if ((nCurTime - it->nTime) > (NOTICE_PICKUP_ITEM_FIDE_IN_TIME + NOTICE_PICKUP_ITEM_FIDE_SHOW_TIME))
			{
				FLOAT fTime = NOTICE_PICKUP_ITEM_FIDE_IN_TIME + NOTICE_PICKUP_ITEM_FIDE_SHOW_TIME + NOTICE_PICKUP_ITEM_FIDE_OUT_TIME;
				FLOAT fRatio = (fTime - (nCurTime - it->nTime)) / (fTime - (NOTICE_PICKUP_ITEM_FIDE_IN_TIME + NOTICE_PICKUP_ITEM_FIDE_SHOW_TIME));
				colBlend = 0xFFFFFF00 | UBYTE(0xFF * fRatio);

				if (fRatio <= 0.f)
				{
					it = m_vecNoticePickupItem.erase(it);
					continue;
				}
			}

			if (NOTICE_PICKUP_ITEM_ICON_POSITION == 0)
			{
				pdp->AddTexture(nPosX - NOTICE_PICKUP_ITEM_BACK_PADDING, nPosY - NOTICE_PICKUP_ITEM_ICON_HEIGHT - NOTICE_PICKUP_ITEM_BACK_PADDING, nPosX + NOTICE_PICKUP_ITEM_ICON_WIDTH + it->nBackWidth, nPosY + NOTICE_PICKUP_ITEM_BACK_PADDING,
					m_rtNoticePickupItem¬³.U0, m_rtNoticePickupItem¬³.V0, m_rtNoticePickupItem¬³.U1, m_rtNoticePickupItem¬³.V1, colBlend);

				pdp->AddTexture(nPosX + NOTICE_PICKUP_ITEM_ICON_WIDTH + it->nBackWidth, nPosY - NOTICE_PICKUP_ITEM_ICON_HEIGHT - NOTICE_PICKUP_ITEM_BACK_PADDING, nPosX + NOTICE_PICKUP_ITEM_ICON_WIDTH + it->nBackWidth + 32, nPosY + NOTICE_PICKUP_ITEM_BACK_PADDING,
					m_rtNoticePickupItemR.U0, m_rtNoticePickupItemR.V0, m_rtNoticePickupItemR.U1, m_rtNoticePickupItemR.V1, colBlend);

				pdp->PutTextEx(it->strText, nPosX + NOTICE_PICKUP_ITEM_ICON_WIDTH + 10, nPosY - NOTICE_PICKUP_ITEM_ICON_HEIGHT / 2 - 7, colBlend);

				pdp->AddBtnTexture(it->nIconTexId, nPosX, nPosY - NOTICE_PICKUP_ITEM_ICON_HEIGHT, nPosX + NOTICE_PICKUP_ITEM_ICON_WIDTH, nPosY,
					it->rtIcon.U0, it->rtIcon.V0, it->rtIcon.U1, it->rtIcon.V1, colBlend);
			}
			else
			{
				pdp->AddTexture(nPosX - NOTICE_PICKUP_ITEM_ICON_WIDTH - it->nBackWidth - 32, nPosY - NOTICE_PICKUP_ITEM_ICON_HEIGHT - NOTICE_PICKUP_ITEM_BACK_PADDING, nPosX - NOTICE_PICKUP_ITEM_ICON_WIDTH - it->nBackWidth, nPosY + NOTICE_PICKUP_ITEM_BACK_PADDING,
					m_rtNoticePickupItemL.U0, m_rtNoticePickupItemL.V0, m_rtNoticePickupItemL.U1, m_rtNoticePickupItemL.V1, colBlend);

				pdp->AddTexture(nPosX - NOTICE_PICKUP_ITEM_ICON_WIDTH - it->nBackWidth, nPosY - NOTICE_PICKUP_ITEM_ICON_HEIGHT - NOTICE_PICKUP_ITEM_BACK_PADDING, nPosX + NOTICE_PICKUP_ITEM_BACK_PADDING, nPosY + NOTICE_PICKUP_ITEM_BACK_PADDING,
					m_rtNoticePickupItem¬³.U0, m_rtNoticePickupItem¬³.V0, m_rtNoticePickupItem¬³.U1, m_rtNoticePickupItem¬³.V1, colBlend);

				pdp->PutTextExRX(it->strText, nPosX - NOTICE_PICKUP_ITEM_ICON_WIDTH - 10, nPosY - NOTICE_PICKUP_ITEM_ICON_HEIGHT / 2 - 7, colBlend);

				pdp->AddBtnTexture(it->nIconTexId, nPosX - NOTICE_PICKUP_ITEM_ICON_WIDTH, nPosY - NOTICE_PICKUP_ITEM_ICON_HEIGHT, nPosX, nPosY,
					it->rtIcon.U0, it->rtIcon.V0, it->rtIcon.U1, it->rtIcon.V1, colBlend);
			}

			nPosY -= (NOTICE_PICKUP_ITEM_ICON_HEIGHT + NOTICE_PICKUP_ITEM_ICON_ICON_SHIFT);
			++it;
		}

		pdp->FlushRenderingQueue();
		pdp->EndTextEx();
		pdp->FlushBtnRenderingQueue(UBET_ITEM);
	}
#endif
...
{	
...
#ifdef ENABLE_HP_BAR
	UIRect			rcHUD, rcHUDHP; //dethunter12
#endif
	...
	{	
			...
			_pNetwork->MyCharacterInfo.pk_mode != 0 )
		{
			...
			if(bShowHpBar)
			{
				float fHPRatio = (float)_pNetwork->MyCharacterInfo.hp / (float)_pNetwork->MyCharacterInfo.maxHP;
				...
				// LEFT
				m_pUIDrawPort->AddTexture(rcHUD.Left, rcHUD.Top, rcHUD.Left + 7, rcHUD.Top + 13,
					ACTORMGR()->m_rtHUDL.U0, ACTORMGR()->m_rtHUDL.V0, ACTORMGR()->m_rtHUDL.U1, ACTORMGR()->m_rtHUDL.V1, 0xFFFFFFFF, fPopupZ);
				// MIDDLE
				m_pUIDrawPort->AddTexture(rcHUD.Left + 7, rcHUD.Top, rcHUD.Left + nBoxWidth + 10, rcHUD.Top + 13,
					ACTORMGR()->m_rtHUDM.U0, ACTORMGR()->m_rtHUDM.V0, ACTORMGR()->m_rtHUDM.U1, ACTORMGR()->m_rtHUDM.V1, 0xFFFFFFFF, fPopupZ);
				// RIGHT
				m_pUIDrawPort->AddTexture(rcHUD.Left + nBoxWidth + 10, rcHUD.Top, rcHUD.Left + nBoxWidth + 17,
					rcHUD.Top + 13, ACTORMGR()->m_rtHUDR.U0, ACTORMGR()->m_rtHUDR.V0, ACTORMGR()->m_rtHUDR.U1, ACTORMGR()->m_rtHUDR.V1, 0xFFFFFFFF, fPopupZ);
				...
				m_pUIDrawPort->AddTexture(rcHUDHP.Left, rcHUDHP.Top, rcHUDHP.Right, rcHUDHP.Bottom,
            ...
            #ifdef ENABLE_HP_BAR
			m_rcName.Bottom = GAMEDATAMGR()->GetPremiumChar()->getType() == PREMIUM_CHAR_TYPE_FIRST  //dethunter12
								? rcHUD.Top - 16
								: rcHUD.Top - 12;
            #else
			...
			if (GAMEDATAMGR()->GetPremiumChar()->getType() == PREMIUM_CHAR_TYPE_FIRST)
			{
				UIRect			rcPcName;
                #ifdef ENABLE_HP_BAR
				rcPcName.Left = m_rcName.Left - 3;
				rcPcName.Top = m_rcName.Top - 12;
				rcPcName.Right = m_rcName.Right + 31;
				rcPcName.Bottom = m_rcName.Bottom + 15;
                #else
	...
	//			buseblindchao = FALSE;
			...
			if( _pNetwork->MyCharacterInfo.pk_mode != 0 )
				nColIndex--;
	#endif
			if (_pNetwork->MyCharacterInfo.colorName)
				nColIndex = _pNetwork->MyCharacterInfo.colorName;
...
#ifdef ENABLE_HP_BAR
	return; // disable render target object name
#endif
	vObjectPos = penObject->GetLerpedPlacement().pl_PositionVector;
	...
	case CHARACTER:
		{
			...
			if( nPkMode != 0 )
				nColIndex--;
#endif	
			nColIndex = GetTargetColor(penObject, nColIndex);
...
void CUIManager::CRCCheck()
{
	while( _pGameState->Running() ) {
...
void CUIManager::MsgBoxCommand( int nCommandCode, BOOL bOK, CTString &strInput )
{
	...
	case MSGCMD_USE_WARP_ITEM:
		{
			...
			if (checkName(strName, eCHECK_TYPE_PLAYER_NAME) == FALSE)
				return;
	...
	case MSGCMD_CONFIRM_CHANGE_MY_NAME:
		{
			...
			CUIManager::getSingleton()->GetMessageBox(MSGCMD_USE_CHANGE_MY_NAME_ITEM)->GetInputBox().SetCursorMove( TRUE );
		}
		break;
	case MSGCMD_CONFIRM_CHANGE_MY_NAME_CUSTOM:
		{
			CloseMessageBox(MSGCMD_CONFIRM_CHANGE_MY_NAME_CUSTOM);
			strTitle = _S( 1919, "???????" ); 
			strMsg = _S( 2140, "???¥ï? ©¦?????? ????? ????????" ); 
			MsgBoxInfo.SetMsgBoxInfo(strTitle,UMBS_INPUTBOX|UMBS_OKCANCEL,UI_NONE,MSGCMD_USE_CHANGE_MY_NAME_ITEM_CUSTOM, WIDTH_INPUT_CUSTOM_NAME);
			MsgBoxInfo.m_nSEInputMaxChar = MAX_MY_NAME_CUSTOM_SIZE;
			MsgBoxInfo.m_nInputMaxChar = MAX_MY_NAME_CUSTOM_SIZE;
			MsgBoxInfo.m_nInputWidth = MSGBOX_INPUT_WIDTH + 32;
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);

			CUIManager::getSingleton()->GetMessageBox(MSGCMD_USE_CHANGE_MY_NAME_ITEM_CUSTOM)->GetInputBox().SetCursorMove( TRUE );
		}
		break;
	case MSGCMD_CONFIRM_CHANGE_GUILD_NAME:
		{
			...
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);
			CUIManager::getSingleton()->GetMessageBox(MSGCMD_USE_CHANGE_GUILD_NAME_ITEM)->GetInputBox().SetCursorMove( TRUE );
		}
		break;
	case MSGCMD_CONFIRM_CHANGE_GUILD_NAME_CUSTOM:
		{
			CloseMessageBox(MSGCMD_CONFIRM_CHANGE_GUILD_NAME_CUSTOM);
			strTitle = _S( 2141, "");
			strMsg = _S( 2142, "" ); 
			MsgBoxInfo.SetMsgBoxInfo(strTitle,UMBS_INPUTBOX|UMBS_OKCANCEL,UI_NONE,MSGCMD_USE_CHANGE_GUILD_NAME_ITEM_CUSTOM, WIDTH_INPUT_CUSTOM_NAME);
			MsgBoxInfo.m_nSEInputMaxChar = MAX_GUILD_NAME_CUSTOM_SIZE;
			MsgBoxInfo.m_nInputMaxChar = MAX_GUILD_NAME_CUSTOM_SIZE;
			MsgBoxInfo.m_nInputWidth = MSGBOX_INPUT_WIDTH + 32;
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);
			CUIManager::getSingleton()->GetMessageBox(MSGCMD_USE_CHANGE_GUILD_NAME_ITEM_CUSTOM)->GetInputBox().SetCursorMove( TRUE );
		}
		break;
	case MSGCMD_USE_CHANGE_MY_NAME_ITEM: 
		{	
			...
			if (!checkName(strMsg, eCHECK_TYPE_PLAYER_NAME)) return;

			_pNetwork->SendChangMyName(strMsg, CHANGE_MY_NAME_ITEM);
			CloseMessageBox(MSGCMD_USE_CHANGE_MY_NAME_ITEM);
		}
		break;
	case MSGCMD_USE_CHANGE_MY_NAME_ITEM_CUSTOM: 
		{	
			strMsg	= GetMessageBox(MSGCMD_USE_CHANGE_MY_NAME_ITEM_CUSTOM)->GetInputBox().GetString();

			if (!checkName(strMsg, eCHECK_TYPE_PLAYER_NAME_CUSTOM)) return;

			_pNetwork->SendChangMyName(strMsg, CHANGE_MY_NAME_ITEM_CUSTOM);
			CloseMessageBox(MSGCMD_USE_CHANGE_MY_NAME_ITEM_CUSTOM);
		}
		break;	
	case MSGCMD_USE_CHANGE_GUILD_NAME_ITEM: 
		{
			...
			if (!checkName(strMsg, eCHECK_TYPE_GUILD_NAME)) return;
			
			_pNetwork->SendChangGuildName(strMsg, CHANGE_GUILD_NAME_ITEM);			
			CloseMessageBox(MSGCMD_USE_CHANGE_GUILD_NAME_ITEM);
		}
		break;
	case MSGCMD_USE_CHANGE_GUILD_NAME_ITEM_CUSTOM: 
		{
			strMsg	= GetMessageBox(MSGCMD_USE_CHANGE_GUILD_NAME_ITEM_CUSTOM)->GetInputBox().GetString();
			
			if (!checkName(strMsg, eCHECK_TYPE_GUILD_NAME_CUSTOM)) return;
			
			_pNetwork->SendChangGuildName(strMsg, CHANGE_GUILD_NAME_CUSTOM_ITEM);			
			CloseMessageBox(MSGCMD_USE_CHANGE_GUILD_NAME_ITEM_CUSTOM);
		}
		break;	
	...
	case MSGCMD_PET_NAMECARD_INPUT:
		{
			if( !GetPetTrainingUI()->IsNotPetWear())
			{
				...
				/**/									
				if ( !checkName( strInput, eCHECK_TYPE_PET_NAME ))
					return;
...
#define HEALED_COLOR			0x1ff70a00
...
void CUIManager::RenderDamage( int nIndex, DAMAGE_deque::iterator iter, DOUBLE dDealyTime, int nX, int nY )
{
	...
	if ((*iter).bHealed)	//dethunter12 add heal test
	{
		colDefault = HEALED_COLOR;
	}
...
BOOL CUIManager::checkName(CTString strMsg, int chkType)
{
	...
	if( chkType == eCHECK_TYPE_PLAYER_NAME) // ©¦?? ??? ?????
	{
		strTitle = _S(191,"È®ÀÎ");
		...
		{			
		 ...
		 if ( chkType == eCHECK_TYPE_PLAYER_NAME && strMsg.Length() > MAX_MY_NAME_SIZE )
		{
			strMsg.PrintF(_S(434,"©¦???? ????? ??? ????.\n(??? 2~8??, ???? 4~16??)"), MAX_MY_NAME_SIZE);
			MsgBoxInfo.SetMsgBoxInfo(strTitle,UMBS_OK,UI_NONE,MSGCMD_BAN_NAME);
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);
			return FALSE;
	...
	else if (chkType == eCHECK_TYPE_PLAYER_NAME_CUSTOM)
	{
		strTitle = _S(191, "È®ÀÎ");
		...
		if (strMsg.Length() > MAX_MY_NAME_CUSTOM_SIZE) {
			strMsg.PrintF(_S(434, "©¦???? ????? ??? ????.\n(??? 2~8??, ???? 4~16??)"), MAX_MY_NAME_CUSTOM_SIZE);
			MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK, UI_NONE, MSGCMD_BAN_NAME);
			MsgBoxInfo.AddString(strMsg);
			CreateMessageBox(MsgBoxInfo);
			return FALSE;
		...
		for (const char* chr = strMsg.str_String; *chr != 0; chr++)
		{
			if ((*chr) == ' ' || (*chr) == '\t' || (*chr) == '\n' || (*chr) == '\r' ||
				(*chr) == '%' || (*chr) == '#' || (*chr) == '&' || (*chr) == '?' ||
				((*chr) == '+' && chkType == eCHECK_TYPE_PLAYER_NAME) || (*chr) == '=')
			{
				CloseMessageBox(MSGCMD_CREATE_ERROR);
				CUIMsgBox_Info	MsgBoxInfo;
				MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK,
	...
	else if( chkType == eCHECK_TYPE_GUILD_NAME) // ?????? ?????
	{
		strTitle = _S(191,"???");
		if (strMsg.Length() < MIN_NAME_SIZE)
		{
	  ...
	  if (strMsg.Length() > MAX_GUILD_NAME_SIZE) 
		...
		for(const char *chr = strMsg.str_String; *chr != 0; chr++)
		{
			...
			if ((IsGamigo(g_iCountry) == TRUE || g_iCountry == USA ||
				IsBila(g_iCountry) == TRUE) && chkType == eCHECK_TYPE_GUILD_NAME)
			{
				// ??????©­? ??????? ???? ????????? ???????
				if (!((*chr) >= 48 && (*chr) <= 57)  //! 0 ~ 9
					&& !((*chr) >= 65 && (*chr) <= 90) // ! A ~ Z 
	...
	else if (chkType == eCHECK_TYPE_GUILD_NAME_CUSTOM) // ?????? ?????
	{
	strTitle = _S(191, "???");
	if (strMsg.Length() < MIN_NAME_SIZE)
	{
	...
	if (strMsg.Length() > MAX_GUILD_NAME_CUSTOM_SIZE) {
		strMsg = _S(2143, "??? ????? ??? ???? \n(??? 2~8?? ???? 4~16?? ????)");
		MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK, UI_NONE, MSGCMD_BAN_NAME);
		MsgBoxInfo.AddString(strMsg);
		CreateMessageBox(MsgBoxInfo);
	...
	for (const char* chr = strMsg.str_String; *chr != 0; chr++)
	{
		if ((*chr) == ' ' || (*chr) == '\t' || (*chr) == '\n' || (*chr) == '\r' ||
			(*chr) == '%' || (*chr) == '#' || (*chr) == '&' || (*chr) == '?' ||
			((*chr) == '+' && chkType == eCHECK_TYPE_GUILD_NAME) || (*chr) == '=')
		{
			CloseMessageBox(MSGCMD_CREATE_ERROR);
			CUIMsgBox_Info	MsgBoxInfo;
			MsgBoxInfo.SetMsgBoxInfo(strTitle, UMBS_OK,
	...
	else if ( chkType == eCHECK_TYPE_PET_NAME )
	{
		strTitle = _S(191,"È®ÀÎ");
		if(strMsg.Length()<1)
		{
...
int CUIManager::GetTargetColor(CEntity* entity, int color)
{
	if (entity == NULL)
		return color;

	ObjectBase* pObject = ACTORMGR()->GetObject(eOBJ_CHARACTER, entity->GetNetworkID());
	if (pObject != NULL) {
		CCharacterTarget* pTarget = static_cast< CCharacterTarget* >(pObject);
		if (pTarget->cha_color_name) {
			color = pTarget->cha_color_name;
		}
	}
...
#ifdef NOTICE_PICKUP_ITEM
void CUIManager::AddNoticePickupItem(LONG nIndex, SQUAD nCount)
{
	while (m_vecNoticePickupItem.size() >= NOTICE_PICKUP_ITEM_MAX)
	{
		m_vecNoticePickupItem.erase(m_vecNoticePickupItem.end() - 1);
	}

	CNoticePickupItem pItem;

	CItemData* pItemData = _pNetwork->GetItemData(nIndex);

	pItem.nIconTexId = pItemData->GetIconTexID();
	FLOAT fTexWidth = _pUIBtnTexMgr->GetTexWidth(UBET_ITEM, pItem.nIconTexId);
	FLOAT fTexHeight = _pUIBtnTexMgr->GetTexHeight(UBET_ITEM, pItem.nIconTexId);

	int	nUVSX = BTN_SIZE * pItemData->GetIconTexCol();
	int	nUVSY = BTN_SIZE * pItemData->GetIconTexRow();
	pItem.rtIcon.SetUV(nUVSX + 1, nUVSY + 1, nUVSX + BTN_SIZE - 1, nUVSY + BTN_SIZE - 1, fTexWidth, fTexHeight);

	if (nCount > 1)
	{
		pItem.strText.PrintF("%s (%I64d)", pItemData->GetName(), nCount);
	}
	else
	{
		pItem.strText.PrintF("%s", pItemData->GetName());
	}

#if defined G_RUSSIA
	ENGINE_API extern CFontData* _pfdDefaultFont;
	pItem.nBackWidth = UTIL_HELP()->GetNoFixedWidth(_pfdDefaultFont, pItem.strText.str_String);
#else
	pItem.nBackWidth = _pUIFontTexMgr->GetFontSpacing() + pItem.strText.Length() * (_pUIFontTexMgr->GetFontWidth() + _pUIFontTexMgr->GetFontSpacing());
#endif

	pItem.nTime = _pTimer->GetHighPrecisionTimer().GetMilliseconds();
	m_vecNoticePickupItem.insert(m_vecNoticePickupItem.begin(), pItem);
}
#endif