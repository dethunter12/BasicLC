...
// Desc :
// ----------------------------------------------------------------------------
void CUIRankingPvP::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
...
// Desc :
// ----------------------------------------------------------------------------
void CUIRankingPvP::ResetPosition( PIX pixMinI, PIX pixMinJ, PIX pixMaxI, PIX pixMaxJ )
{
...
// Desc :
// ----------------------------------------------------------------------------
void CUIRankingPvP::AdjustPosition( PIX pixMinI, PIX pixMinJ, PIX pixMaxI, PIX pixMaxJ )
{
...
//------------------------------------------------------------------------------
// CUIRankingPvP::SetFocus
// Explain:  
// Date : 2005-03-10(���� 1:02:39) Lee Ki-hwan
//------------------------------------------------------------------------------
void CUIRankingPvP::SetFocus( BOOL bVisible )
{
	CUIWindow::SetFocus( bVisible );
}

void CUIRankingPvP::closeUI()
{
	SetVisible(FALSE);
	ResetRanking();
}
...
// Desc : 
// ----------------------------------------------------------------------------
void CUIRankingPvP::OpenRankingList( )
{	
	if( IsVisible() )
	{		
		return;
	...
	CUIManager::getSingleton()->RearrangeOrder( UI_RANKING_PVP, TRUE );
}
...
// Desc : 
// ----------------------------------------------------------------------------
void CUIRankingPvP::ResetRanking()
{		
	m_lbRankingList.ResetAllStrings();
	m_sTotal.Clear();
	...
	CUIManager::getSingleton()->RearrangeOrder( UI_RANKING_PVP, FALSE );
}
...
// Desc : ��� ����� Ŭ�����մϴ�.
// ----------------------------------------------------------------------------
void CUIRankingPvP::ClearRankingList()
{
	if(!m_vectorRankingList.empty())
	{
		m_vectorRankingList.clear();
...
// Desc : ��� ��Ͽ� �߰�
// ----------------------------------------------------------------------------
void CUIRankingPvP::AddToRankingList( const CTString& strName, INDEX iNumOfDeadMonster, INDEX iJob )
{	
	sUserRanking TempRankingUser;
	TempRankingUser.strName			= strName;
	TempRankingUser.iNumDeadNpc 	= iNumOfDeadMonster;
...
// Desc : ����� �����մϴ�.
// ----------------------------------------------------------------------------
void CUIRankingPvP::RefreshRankingList( BOOL bInit )
{
	if( !bInit && !IsVisible() )
		return;
...
// Desc :
// ----------------------------------------------------------------------------
void CUIRankingPvP::Render(void)
{
	CDrawPort* pDrawPort = CUIManager::getSingleton()->GetDrawPort();
	...
	m_sTotal.PrintF(_S( 100069, "Score PvP" ));
	pDrawPort->PutTextEx( m_sTotal, m_nPosX + 230,
		m_nPosY + 29, 0xFFFFFFFF );	

	// Text in guild
	pDrawPort->PutTextEx( _S( 100070, "Ranking PvP" ), m_nPosX + RANKING_TITLE_TEXT_OFFSETX,
...
// Desc :
// ----------------------------------------------------------------------------
WMSG_RESULT	CUIRankingPvP::MouseMessage( MSG *pMsg )
{
	if (!IsVisible())
	{
		return WMSG_FAIL;
