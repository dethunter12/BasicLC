...
void CUIInvenCashBag::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
    ...
    if (nHeight == INVEN_CASH_BAG_NEW_H)
        m_rcItemSlot.SetRect( ITEM_SLOT_SX, CASH_SLOT_SY, ITEM_SLOT_SX + ITEM_SLOT_WIDTH, CASH_SLOT_SY + ITEM_SLOT_NEW_HEIGHT );
    else
#endif
    ...
    if (nHeight == INVEN_CASH_BAG_NEW_H)
        m_rsItemSlotSurface.AddRectSurface(UIRect(0, 0, nWidth, nHeight),
            UIRectUV(0, 0, INVEN_CASH_BAG_W, INVEN_CASH_BAG_NEW_H,fTexWidth,fTexHeight));    // ??? ???? UV
    else
#endif
    m_rsItemSlotSurface.AddRectSurface(UIRect(0, 0, nWidth, nHeight),
        UIRectUV(0, 0, INVEN_CASH_BAG_W, INVEN_CASH_BAG_H,fTexWidth,fTexHeight));
...
void CUIInvenCashBag::SetInventoryAllItem( btnAllItems* pAllItem, int nTab )
{
    ...
    for( i = 0; i < ITEM_COUNT_IN_INVENTORY_CASH_1; i++ )
    {
        ...
        #ifdef EXPANSION_INVENTORY
        count++;
        if (count >= INVEN_SLOT_COL)
        {
            nY = tmpY + (BTN_SIZE + btnGap);
            tmpY = nY;
            count = 0;
        }
        #endif
	...
	case WM_LBUTTONUP:
		{
			if ( bLButtonClick && IsInsideRect(nX, nY, m_rcLockArea) )
			{
				...
				{
					int t;
					if (m_nIndexBag - INVEN_TAB_CASH1 == 0) t = INVENTORY_TAB_CASH_1;
					else if (m_nIndexBag - INVEN_TAB_CASH2 == 0) t = INVENTORY_TAB_CASH_2;
					else if (m_nIndexBag - INVEN_TAB_CASH3 == 0) t = INVENTORY_TAB_CASH_3;
					else if (m_nIndexBag - INVEN_TAB_CASH4 == 0) t = INVENTORY_TAB_CASH_4;
					//dethunter12 ADD
					else if (m_nIndexBag - INVEN_TAB_CASH5 == 0) t = INVENTORY_TAB_CASH_5;
					else if (m_nIndexBag - INVEN_TAB_CASH5 == 0) t = INVENTORY_TAB_CASH_6;
