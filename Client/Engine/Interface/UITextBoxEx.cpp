...
void CUITextBoxEx::AddText(std::string& str, DWORD dwCol /*= DEF_UI_COLOR_WHITE*/, CItems* item, DWORD dwColMove, int countItems)
{
	if (str.empty() == true)
		return;
...
void CUITextBoxEx::split(std::string str, CItems* item, int countItems)
{
	...
	while (str.empty() == false)
	{
		...
		case SPLIT_NONE:
			...
			break;
		case SPLIT_SPACE:
			splitSpace(strText, val, strTagID, nBoxWidth, item);
			break;
		case SPLIT_ONELINE:
...
void CUITextBoxEx::splitSpace(std::string& strAdd, vecValcont& val, std::string& strTagID, int nTextArea, CItems* item)
{
	int nCurPos = 0;
	int nCurUIPos = 0;
	int nEndPos;
	...
	while (1)
	{
		...
		strAdd = strAdd.substr(nEndPos, strAdd.length() - nEndPos);
		createControl(strTagID, pCurUI, val, strTemp, item);
...
void CUITextBoxEx::insertText(CUITextEx* pTextEx, vecValcont& val, std::string& str, CItems* item)
{
	COLOR col = m_colBase;
	COLOR colMove = m_colMove;

	if (val.size() == 2)
	{
		col = strtoul(val[eTVC_COL].c_str(), NULL, 16);
		colMove = strtoul(val[eTVC_COL].c_str(), NULL, 16);
	}

	pTextEx->AddText(str.c_str(), col, -1, item, colMove);
}
...
void CUITextBoxEx::createControl(std::string& strTagID, CUITextEx* pTextEx, vecValcont& val, std::string& str, CItems* item)
{
	if (strcmpi(strTagID.c_str(), "col") == 0)
	{
		insertText(pTextEx, val, str, item);
	}
