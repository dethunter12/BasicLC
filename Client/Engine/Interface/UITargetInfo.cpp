...
CUITargetInfo::CUITargetInfo()
{
	...
	m_colNameColor[20] = 0xcc9900FF;/*Admin Gold*/
	m_colNameColor[21] = 0x0066ffFF;/* Dark Blue*/
	m_colNameColor[22] = 0xff3300FF;/*Red*/
	m_colNameColor[23] = 0xcc0099FF;/*Purple*/
	m_colNameColor[24] = 0x33cc33FF;/*green*/
	m_colNameColor[25] = 0x00ffffFF;/*LightBlue*/
	m_colNameColor[26] = 0xff9900FF;/*Orange*/
	m_colNameColor[27] = 0xff66ccFF; /*Pink*/
#endif
	...
	case CHARACTER:
		{
			...
			if (INFO()->_TargetInfo->cha_color_name) {
				nIndex = INFO()->_TargetInfo->cha_color_name;
			}
	...
	case WM_MOUSEMOVE:
		{
			...
			if(INFO()->_TargetInfo.TargetType == CHARACTER && IsInsideRect(nX, nY, m_rcInfoMark))
			{
				...
				SLONG wins = 0;
				SLONG loses = 0;
				if(INFO()->_TargetInfo.pen_pEntity == CEntity::GetPlayerEntity(0))//?? ????
				{
					bFound = true;
				...
				if(bFound)
				{
					...
					if (wins == -1 && loses == -1)
						m_strInfo[0].PrintF( _S(1741, "I'm newbie") );
					else
						m_strInfo[0].PrintF( _S(1742, "Wins : %d  Loses: %d"), wins, loses );
