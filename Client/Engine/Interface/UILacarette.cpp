...
void		CUILacaball::_receiveUsedResult(CNetworkMessage* istr )
{
	...
	{
		m_bAutoClick = FALSE;
		MsgBoxInfo.SetMsgBoxInfo( _S( 191, "???" ), UMBS_OK, UI_LACABALL, MSGCMD_LACABALL_ERROR );
		MsgBoxInfo.AddString( _S( 116, "?κ??? ?????? ??????? ?????? ???? ?? ???????." ) );
	}
	else if( byErrorCode == 2 )
	{
		m_bAutoClick = FALSE;
		MsgBoxInfo.SetMsgBoxInfo( _S( 191, "???" ), UMBS_OK, UI_LACABALL, MSGCMD_LACABALL_ERROR );
		MsgBoxInfo.AddString( _S( 4860, "????? ????????." ) );
	}
	else
	{
		;//??
