...
void CUIMixNew::RenderMasterStone()
{
	...
	if (m_bIsMixReforgeHammer) {
		pDrawPort->PutTextEx( _S(100503, "?????? ????" ), m_nPosX + MIXNEW_TITLE_TEXT_OFFSETX,
											m_nPosY + MIXNEW_TITLE_TEXT_OFFSETY, 0xFFFFFFFF );
	} else {
	...
	if (m_bIsMixReforgeHammer)
		m_strItemSlot[0] = _S(100503, "Reforge Hammer");
	else
		...
		case 0 : 
			{
				...
				if (m_bIsMixReforgeHammer) {
					if (nItemIndex != 16002 ) {
						...
						pUIManager->GetChattingUI()->AddSysMessage( _S(100505, "You can only load Reforge Hammer" ), SYSMSG_ERROR );
						pUIManager->ResetHoldBtn();
						return;
					}
				} else if (!((nItemIndex >= 6111 && nItemIndex <= 6115)		// ?????~???? ???? ?????? ????
				...
				{
					tv_str.PrintF("%p,%x",pItemData->GetFlag(),pItemData->GetFlag());
					pUIManager->GetChattingUI()->AddSysMessage( _S( 5438, "?????? ???? ?????? ??????? ?ø? ?? ??????." ), SYSMSG_ERROR );
					pUIManager->ResetHoldBtn();
					return;
				}
			}
		...
		case 1 :
			{
				if ( m_pIconsSlot[0]->IsEmpty() ){					
					if (m_bIsMixReforgeHammer)
						pUIManager->GetChattingUI()->AddSysMessage( _S(100506, "You must insert Reforge Hammer  first." ), SYSMSG_ERROR );
					else
						pUIManager->GetChattingUI()->AddSysMessage( _S( 5439, "?????? ?????? ???? ???? ????." ), SYSMSG_ERROR );	
					pUIManager->ResetHoldBtn();
					return;
				}
				...
				if (m_bIsMixReforgeHammer && !CheckReforgeHammer(pItems)) {
					...
					MsgBoxInfo.AddString( _S(100508, "You cannot reforge a item that is not a weapon" ) );
					pUIManager->CreateMessageBox( MsgBoxInfo );
					pUIManager->ResetHoldBtn();
					return;
				} 
...
// Desc :
// ----------------------------------------------------------------------------
void CUIMixNew::MixNewMasterStoneRep( int err_type, int success_type, int percent, int max_attack)//dethunter12 adjust 3/8/2023
{
	CUIManager* pUIManager = CUIManager::getSingleton();
	...
	case 0:
		{
			...
			case MASTERSTONE_DECREASE:
				MsgBoxInfo.AddString( _S( 177, "???????? ????????????. -1 ???????????." ) );
				break;
			case REFORGE_HAMMER_CHANGE:
				CTString hammerMsg;
				hammerMsg.PrintF(_S(100507, "Reforge complete! Your new attack is %d out of %d!"), percent,max_attack);
	...
	case 3:
	{
		pUIManager->GetChattingUI()->AddSysMessage(_S(100509, "업그레이드 가능 아이템이 아닙니다."), SYSMSG_ERROR);
		CloseMixNew();
	}
	break;
	}
...
void CUIMixNew::UpdateExplication( int nPlus /*= -1*/ )
{
	...
	if (m_bIsMixReforgeHammer)
	{
		AddString( _S( 100043, "You can reforge items using Reforge Hammer" ) );
	}
	...
	{
		if (m_bIsMixReforgeHammer) {
			AddString( _S( 100044, "You are able to reform the base attack of your weapon by using the Reforge Hammer" ) );
		} else {
			AddString( _S( 5436, "?????? ?????? ?????? ??? ???????? ??? ???? ??? ??????? ??? +30???? ???????? ????????." ) );
		}
	}
}
