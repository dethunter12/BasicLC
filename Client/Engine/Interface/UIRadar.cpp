...
void CUIRadar::initialize()
{
	...
	m_dominInfo = (CUIBase*)findUI("domination_event");
	m_pPointTeamA = (CUIText*)findUI("team_a_points_txt");
	m_pPointTeamB = (CUIText*)findUI("team_b_points_txt");
	m_pDominationFlagStatus[0] = (CUIImage*)findUI("flag1");
	m_pDominationFlagStatus[1] = (CUIImage*)findUI("flag2");
	m_pDominationFlagStatus[2] = (CUIImage*)findUI("flag3");
	#endif
...
#ifdef DOMINATION_EVENT_SYSTEM
void CUIRadar::UpdateDominationInfoFlag(int pos, int status)
{
	UIRectUV uv;
	
	m_pDominationFlagStatus[pos]->setTexString("CommonBtn.tex");

	switch (status)
	{
	case FLAG_EMPTY:
		uv.SetUV(208, 98, 217, 107);
		m_pDominationFlagStatus[pos]->SetUV(uv);
		break;
	case FLAG_TEAM_A:
		uv.SetUV(208, 118, 217, 127);
		m_pDominationFlagStatus[pos]->SetUV(uv);
		break;
	case FLAG_TEAM_B:
		uv.SetUV(217, 107, 228, 118);
		m_pDominationFlagStatus[pos]->SetUV(uv);
		break;
	case FLAG_ATTACK:
		uv.SetUV(228, 118, 238, 127);
		m_pDominationFlagStatus[pos]->SetUV(uv);
		break;
	default:
		break;
...
void CUIRadar::UpdateDominationInfo(int pointsTeamA, int pointsTeamB, int status)
{
	CTString strTmp;

	if (status == DOMIN_OPEN ||
		status == DOMIN_RUNNING)
	{
		if (m_dominInfo->IsVisible() == FALSE || m_dominInfo->GetHide() == TRUE)
		{
			m_dominInfo->SetVisible(TRUE);
			m_dominInfo->Hide(FALSE);
		}		
	} else {
		m_dominInfo->SetVisible(FALSE);
		m_dominInfo->Hide(TRUE);
	}

	strTmp.PrintF("%d", pointsTeamA);
	if (m_pPointTeamA)
		m_pPointTeamA->SetText(strTmp);

	strTmp.PrintF("%d", pointsTeamB);
	if (m_pPointTeamB)
		m_pPointTeamB->SetText(strTmp);
}
#endif