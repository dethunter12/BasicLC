...
void CUIInvenCashBagBox::SetPutOnItemData()
{
	...
	if (pUIManager->GetDragIcon()->getBtnType() == UBET_ITEM )
	{
			...
			pItemData->GetSubType() == CItemData::ITEM_SUB_INVEN_CASH_BAG_KEY)
		{
			...
			if (item_index - INVEN_CASH_KEY1 == 0 ||
				item_index - INVEN_CASH_KEY2 == 0 ||
				item_index - INVEN_CASH_KEY3 == 0 ||
				item_index - INVEN_CASH_KEY4 == 0 || 
				item_index - INVEN_CASH_KEY5 == 0 || 
				item_index - INVEN_CASH_KEY6 == 0)
			{
				if( m_pIconKey )
				{
					m_pSendItems = pItems;
