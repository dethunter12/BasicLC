...
void CUIPlayerInfo::Create( CUIWindow *pParentWnd, int nX, int nY, int nWidth, int nHeight )
{
	...
	// TopUI �ϳ��� ���� �ϸ鼭 ���þ� ����, ��ī�� ��ī�� �ؽ��� ��ġ �ٸ��� �߰� [12/14/2012 Ranma]
...
void CUIPlayerInfo::MsgBoxCommand( int nCommandCode, BOOL bOK, CTString &strInput )
{
	switch( nCommandCode )
	{
		...
		case MSGCMD_EXPEDITION_COLLECTITEM: // ������ ������ ����
			if( bOK )
			{
				// ������ ����Ʈ������ �˻���û �޽��� ����
...
void CUIPlayerInfo::ShowToolTip( BOOL bShow, int nToolTipID )
{
	...
	if( nOldToolTipID != nToolTipID )
	{
		...
		case 0:		// Web board
			{
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S( 385, "�Խ���" ), "(B,Alt+B)" );
				else
					m_strToolTip.PrintF( "%s %s", _S( 385, "�Խ���" ), "(Alt+B)" );
		...
		case 1:		// Messanger
			{
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S( 458, "�޽���" ), "(F,Alt+F)" );
				else
					m_strToolTip.PrintF( "%s %s", _S( 458, "�޽���" ), "(Alt+F)" );
		...
		case 2:		// System menu
			{
				...
				{
					if( g_iEnterChat )
						m_strToolTip.PrintF( "%s %s", _S( 299, "�ý���" ), "(Y,Alt+Y)" );
					else
						m_strToolTip.PrintF( "%s %s", _S( 299, "�ý���" ), "(Alt+Y)" );
				}
				else
				{
					if( g_iEnterChat )
						m_strToolTip.PrintF( "%s %s", _S( 299, "�ý���" ), "(Z,Alt+Z)" );
					else
						m_strToolTip.PrintF( "%s %s", _S( 299, "�ý���" ), "(Alt+Z)" );
				}
		...
		case 3:		// Inventory
			{
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S( 166, "�κ��丮" ), "(I,Alt+I)" );
				else
					m_strToolTip.PrintF( "%s %s", _S( 166, "�κ��丮" ), "(Alt+I)" );
		...
		case 4:		// Character information
			{
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S( 101, "�������ͽ�" ), "(T,Alt+T)" );
				else
					m_strToolTip.PrintF( "%s %s", _S( 101, "�������ͽ�" ), "(Alt+T)" );
...
BOOL CUIPlayerInfo::ProcessShortCut( MSG *pMsg )
{
	...
	if( pUIManager->GetReformSystem()->IsVisible() ) // ���� �ý��� UI�� ���� ���� ��� ����Ű ���� [9/6/2012 Ranma]
		return FALSE;
	if (pUIManager->GetCashShopEX()->IsVisible()) // ĳ�ü� UI�� ���� ���� ��� ����Ű �Է� ����
		return FALSE;
	if (pUIManager->GetRankingViewEx()->IsVisible() && pUIManager->GetRankingViewEx()->IsEditBoxFocused()) // ��ŷ �ý��� ��������, ����Ʈ�ڽ� ��Ŀ���� ��� �Է� ����
		return FALSE;
	...
	else
	{
		...
		if( (_abKeysPressed[KID_LALT] || g_iEnterChat) && !pUIManager->IsFocusAllEditBox())
		{
			...
			case 37:		// 'k' - skill
				...
				break;
 			case 27:// ']'UI ���� �׽�Ʈ�� ���ؼ� ����� ���� �׽�Ʈ ������ �� �ּ� ó�� testcode mail
 				{
 				}
 				break;
			...
			case 48:		// 'b' - web board
				if (pUIManager->GetRankingPvP()->IsVisible())
					pUIManager->GetRankingPvP()->closeUI();
				else
...
void CUIPlayerInfo::ShowQuickMenuToolTip(BOOL bShow, int nToolTipID)
{
	...
	if( nOldToolTipID != nToolTipID )
	{
		...
		case QUICKMENU_BTN_CHAR_INFO:		// Character information
			{
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S( 69, "ĳ���� ����â" ), "(T,Alt+T)" );
				else
					m_strToolTip.PrintF( "%s %s", _S( 69, "ĳ���� ����â" ), "(Alt+T)" );
			}
			break;
		...
		case QUICKMENU_BTN_SKILL:		// Skill
			{
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S( 91, "��ų" ), "(K,Alt+K)" );
				else
					m_strToolTip.PrintF( "%s %s", _S( 91, "��ų" ), "(Alt+K)" );
			}
			break;
		case QUICKMENU_BTN_QUEST:		// Quest Book
			{
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S(1700, "����Ʈ��" ), "(L,Alt+L)" );
				else
					m_strToolTip.PrintF( "%s %s", _S(1700, "����Ʈ��" ), "(Alt+L)" );
			}
			break;
		case QUICKMENU_BTN_PET_INFO:		// Pet Info
			{
				if( g_iEnterChat )
					m_strToolTip.PrintF( "%s %s", _S(2173, "�� ����â" ), "(P,Alt+P)" );
				else
					m_strToolTip.PrintF( "%s %s", _S(2173, "�� ����â" ), "(Alt+P)" );
			}
			break;
		...
		case QUICKMENU_BTN_RANKING:		// Help
			{
				if (IsGamigo(g_iCountry) == TRUE)
				{
					m_strToolTip.PrintF( "%s", _S(5213, "��ŷ"));
				}
				else
				{
					if( g_iEnterChat )
						m_strToolTip.PrintF( "%s %s", _S(5213, "��ŷ"), "(V,Alt+V)");
					else
						m_strToolTip.PrintF( "%s %s", _S(5213, "��ŷ"), "(Alt+V)");
				}
			}
			break;
		case QUICKMENU_BTN_CASH_SHOP:		// Cash Shop
			{
				m_strToolTip.PrintF( "%s", _S(2572, "���̸��� ����"));
			}
			break;
		case QUICKMENU_BTN_LAKA_BALL:
			{
				m_strToolTip.PrintF( "%s", _S( 5350, "��ī��"));
			}
			break;
		}
...
void CUIPlayerInfo::OpenQuickMenu(int nMenu)
{
	...
	// ITS : 4378 [Ranking] ä��â �ϴܿ� �ý��� �޴����� ��ŷ �ý��� ��ư Ŭ�� �� ���� UI�� ������ ���� [10/6/2011 rumist]
		pUIManager->GetRankingViewEx()->ToggleVisible();
		break;
		//���̸��� ����
	case QUICKMENU_BTN_CASH_SHOP:
		...
		break;
	case QUICKMENU_BTN_LAKA_BALL:	// ��ī��
		{
			if( !pUIManager->GetLacaBall()->IsVisible() )
