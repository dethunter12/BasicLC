...
void CUIBuff::GetBuffInfo( int nItemIndex, int nSkillIndex, int nSkillLevel, int &nInfoWidth, int &nInfoHeight )
{
	...
	else
	{
		...
		{
			if (pItemData->GetLevel() > 0)
				strTemp.PrintF("%s Lv %d", szItemName, pItemData->GetLevel());
			else
	...
	{
		const char* szItemNameDesc = _pNetwork->GetItemDesc(nItemIndex);
		CItemData* pItemData = _pNetwork->GetItemData(nItemIndex);

		if (pItemData->GetLevel() > 0)
		{
			strTemp.PrintF("Level: %d", pItemData->GetLevel());
			AddBuffInfoString(strTemp);
		}

		strTemp.PrintF("%s", szItemNameDesc);
		AddBuffInfoString(strTemp, 0x94B7C6FF);
	}
...
#ifdef ENABLE_REMOVE_BUFF_RIGHT_CLICK
	if (pMsg->message == WM_RIGHT_CLICK)
	{
			...
			nY > m_rcMyGoodBuff.Top && nY < m_rcMyGoodBuff.Bottom )
		{
			for(int i=0 ; i< MAX_BUFF_ROW ; i++){
					...
					nY > m_rcMyGoodBuffRow[i].Top && nY < m_rcMyGoodBuffRow[i].Bottom )
				{
					...
					nSelBuff = nSelBuff + MAX_BUFF_COL*i;
					
					m_nBuffSelectedSkill = m_aMyBuff[nSelBuff].GetSkillIndex();
					m_nBuffSelectedItem = m_aMyBuff[nSelBuff].GetItemIndex();
					
					switch (m_nBuffSelectedSkill)
					{
					case 590:
					case 465:
					case 1860: //ADD 2/5/2023 siege can't move buff`
						UIMGR()->GetChattingUI()->AddSysMessage(_S(100042, "You cannot remove this buff or item"), SYSMSG_ERROR);
						return WMSG_FAIL;
						break;
					default:
						break;
					...
					MsgBox.SetMsgBoxInfo(_S(100039, ""), UMBS_OKCANCEL, UI_QUEST, MSGLCMD_REMOVE_SELECTED_BUFF);
					MsgBox.AddString(_S(100040, "Do you want to remove this buff?"), 0xa3a1a3ff);
					MsgBox.AddString(CTString(" "), 0xa3a1a3ff);

					if (m_nBuffSelectedItem > 0) {
						CTString strName(_pNetwork->GetItemName(m_nBuffSelectedItem));
						MsgBox.AddString(strName, 0xE18600FF);
					}
					else if (m_nBuffSelectedSkill > 0) {
						CSkill& rSkillData = _pNetwork->GetSkillData(m_nBuffSelectedSkill);
						CTString strName(rSkillData.GetName());
						MsgBox.AddString(strName, 0xE18600FF);
					}
					else {
						return WMSG_FAIL;
					...
					MsgBox.AddString(CTString(" "), 0xa3a1a3ff);
					MsgBox.AddString(_S(100041, "Warning: A wrongly remove buff can not be restored"), 0xa3a1a3ff);
					
					UIMGR()->CreateMessageBox( MsgBox );
