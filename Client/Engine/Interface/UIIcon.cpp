...
void CUIIcon::setData(CItems* pItems, bool bRef, bool isChat)
{
	if (pItems == NULL)
		return;
	...
	// 플러스 수치 표시
	{
		if (pItemData != NULL && pItemData->GetItemIndex() > 0 && !isChat)
		{
	...
	// 보석 등급 추가
	{
	    if (pItemData->GetType() == CItemData::ITEM_ETC && !isChat && //dethunter12 test
	...
	// 봉인
	{
		if (pItems->IsFlag(FLAG_ITEM_COMPOSITION) && pItemData->IsFlag(ITEM_FLAG_COMPOSITE) == false && !isChat)
	...
	// 대여
	{
		if (pItems->IsFlag(FLAG_ITEM_LENT) && !isChat)
...
#ifdef DURABILITY
	{
		if ( pItemData->IsFlag( ITEM_FLAG_DURABILITY) && !isChat)
		{
	...
	// ??? ???? ???
	if ( pItemData->IsFlag( ITEM_FLAG_QUEST) && !isChat)
