...
void CUISiegeWarfareNew::RenderConsensus()
{
	...
	if( CheckConsensus() > 0 ){
		...
		if (pObject != NULL)
		{
			if (pObject->GetType() == DOMINATION_LORD_SYMBOL_A_INDEX ||
				pObject->GetType() == DOMINATION_LORD_SYMBOL_B_INDEX ||
				pObject->GetType() == DOMINATION_LORD_SYMBOL_C_INDEX)
			{
				CNetworkMessage nmReg( (UBYTE)MSG_DOMINATION_EVENT );	
				nmReg << (UBYTE)MSG_DOMINATION_EVENT_ADD_FLAG;
				nmReg << (INDEX) pObject->GetType();
				_pNetwork->SendToServerNew(nmReg);
			}
			else if (pObject->GetType() == DRATAN_LORD_SYMBOL_INDEX)
			{
				IsDratanNpc = true;
			}
		}
	...
	if (pObject != NULL)
	{
		pDrawPort->PutTextEx(_s("Domination Crystal"),nX+19,nY+6);
		if (pObject->GetType() == DOMINATION_LORD_SYMBOL_A_INDEX ||
			pObject->GetType() == DOMINATION_LORD_SYMBOL_B_INDEX ||
			pObject->GetType() == DOMINATION_LORD_SYMBOL_C_INDEX)
		{
...
{	
	...
	{
		int timeup = 0;
		if (pObject->GetType() == DRATAN_LORD_SYMBOL_INDEX)
			timeup = CONSENSUS_TIMEUP;
		else if (pObject->GetType() == DOMINATION_LORD_SYMBOL_A_INDEX ||
				pObject->GetType() == DOMINATION_LORD_SYMBOL_B_INDEX ||
				pObject->GetType() == DOMINATION_LORD_SYMBOL_C_INDEX)
			timeup = 30;
		
		int tSec = (tmNow - m_tmStartTime)/1000;
		if ( (tSec-timeup) <= 0 ) m_iConsensusBarSize = tSec*260/timeup ;
		return tSec-timeup;
	}
#else
	int tSec = (tmNow - m_tmStartTime)/1000;
...
#ifdef DOMINATION_EVENT_SYSTEM
BOOL CUISiegeWarfareNew::CloseWindowByEsc()
{
	StopConsensus(_pNetwork->MyCharacterInfo.index);
	return TRUE;
}

void CUISiegeWarfareNew::SendConsensusDominationEvent(int npcIdx)
{
	CNetworkMessage nmReg( (UBYTE)MSG_DOMINATION_EVENT );	
	nmReg << (UBYTE)MSG_DOMINATION_EVENT_COLLECT_START;
	nmReg << (INDEX) npcIdx;
	_pNetwork->SendToServerNew(nmReg);
}
#endif