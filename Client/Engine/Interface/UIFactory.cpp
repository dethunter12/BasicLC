...
int readXMLencrypted(char *  filename , unsigned char* &buffer,long &size ){
...
bool UIFactory::loadXML( CUIBase* root, const char* pathname )
{
	...
	long sizemela = 0; 
	if(readXMLencrypted( const_cast<char*>(strPath.c_str()),buffer,sizemela) == 0){
		
		decryptxml(buffer, sizemela);
		
		doc.Parse((const char *)&buffer[4]);
