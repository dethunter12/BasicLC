...
CUIOption::CUIOption()
	...
	, m_pChkMobsHpBar(NULL)
	, m_pChkPlayersHpBar(NULL)
{
	setInherit(false);
...
void SaveWindowSizeToTxt(PIX pixSizeI, PIX pixSizeJ)
{
	CTString strFullPath = _fnmApplicationPath.FileDir();
	CTFileName fileName = CTString(strFullPath+"Data\\etc\\DisplaySize.txt");
...
bool CUIOption::checkResolutionExist(PIX width, PIX height)
{
	for (int i = 0; i < m_vecPixWidth.size(); i++) {
		if (m_vecPixWidth[i] == width && m_vecPixHeight[i] == height) {
			return true;
		}
	}
	return false;
...
// [2011/04/07 : Sora] DX9.0 �̻� ���������� �������� �����Ƿ� ��Ȱ��ȭ
	...
	//set it if its not equal to 0 when you initialize? default 1
#ifdef NOTICE_PICKUP_ITEM
	m_pChkItemLog->SetCheck(g_bItemLog != 0);
#endif
	m_nViewTerrain = -1;
...
#ifdef ENABLE_MOB_PLAYER_HP
	m_pChkMobsHpBar->SetCheck(g_iHpBarShowFlag & SHOW_MOB_HP_BAR);
	m_pChkPlayersHpBar->SetCheck(g_iHpBarShowFlag & SHOW_PLAYER_HP_BAR);
#endif
}
...
void CUIOption::CreateResolutionList()
{
	...
	for( INDEX idm = 0; idm < ctDisplayMode; idm++ )
	{
		...
		if( dmDisplayMode.dm_pixSizeI >= 800 && dmDisplayMode.dm_pixSizeJ >= 600 )
		{
			...
			if (!checkResolutionExist(dmDisplayMode.dm_pixSizeI, dmDisplayMode.dm_pixSizeJ)) {
	...
	if( ctDisplayCount == 0 )
	{
		if (!checkResolutionExist(800, 600)) {
	...
	if (!checkResolutionExist(1250, 850)) {
		m_vecPixWidth.push_back( 1250 );
		m_vecPixHeight.push_back( 850 );
		if ( m_pChkFullscreen->IsChecked())
			m_vecPixDepth.push_back( 32 );
		else 
			m_vecPixDepth.push_back( pixDepth );
	...
	if (!checkResolutionExist(1440, 900)) {
		m_vecPixWidth.push_back( 1440 );
		m_vecPixHeight.push_back( 900 );
		if ( m_pChkFullscreen->IsChecked())
			m_vecPixDepth.push_back( 32 );
		else 
			m_vecPixDepth.push_back( pixDepth );
	...
	if (!checkResolutionExist(1680, 1050)) {
		m_vecPixWidth.push_back( 1680 );
		m_vecPixHeight.push_back( 1050 );
		if ( m_pChkFullscreen->IsChecked())
			m_vecPixDepth.push_back( 32 );
		else 
			m_vecPixDepth.push_back( pixDepth );
	...
	if (!checkResolutionExist(1920, 1080)) {
		m_vecPixWidth.push_back( 1920 );
		m_vecPixHeight.push_back( 1080 );
		if ( m_pChkFullscreen->IsChecked())
			m_vecPixDepth.push_back( 32 );
		else 
			m_vecPixDepth.push_back( pixDepth );
	...
	if (!checkResolutionExist(1920, 1200)) {
		m_vecPixWidth.push_back( 1920 );
		m_vecPixHeight.push_back( 1200 );
		if ( m_pChkFullscreen->IsChecked())
			m_vecPixDepth.push_back( 32 );
		else 
			m_vecPixDepth.push_back( pixDepth );
	...
	if( m_pChkFullscreen->IsChecked() )
	{
		...
		{
			if (!checkResolutionExist(m_vecPixWidth[iRes], m_vecPixHeight[iRes])) {
...
#ifdef NOTICE_PICKUP_ITEM
	g_bItemLog = m_pChkItemLog->IsChecked();
#endif
...
{	
...
#ifdef ENABLE_MOB_PLAYER_HP
	m_pChkMobsHpBar =	(CUICheckButton*)findUI("chk_mob_bar");
	m_pChkPlayersHpBar = (CUICheckButton*)findUI("chk_player_bar");
#endif
	m_pComboItemPlusEffect =	(CUIComboBox*)findUI("combo_eff_item");

	if (m_pComboItemPlusEffect != NULL)
	{
		m_pComboItemPlusEffect->AddString(_S(1031, "�� ȿ�� �����"));
		m_pComboItemPlusEffect->AddString(_S(1032, "�� ȿ�� ����"));
	}
	...
	if (m_pComboSelectTarget != NULL)
	{
		m_pComboSelectTarget->AddString(_S(4163, "��� �㰡"));
		m_pComboSelectTarget->AddString(_S(4164, "��ȯ��&�� �� ����"));
		m_pComboSelectTarget->AddString(_S(4386, "ĳ����,��ȯ��&�� ����"));
	}
...
#ifdef NOTICE_PICKUP_ITEM
	m_pChkItemLog = (CUICheckButton*)findUI("chk_item_log");
#endif
...
#ifdef ENABLE_MOB_PLAYER_HP
	if (m_pChkMobsHpBar != NULL)
		m_pChkMobsHpBar->SetCommandF(boost::bind(&CUIOption::_change_show_mob_bar, this));
	
	if (m_pChkPlayersHpBar != NULL)
		m_pChkPlayersHpBar->SetCommandF(boost::bind(&CUIOption::_change_show_player_bar, this));
#endif
	//------------------------------------------------------------------------
...
#ifdef NOTICE_PICKUP_ITEM
	if (m_pChkItemLog != NULL)
		m_pChkItemLog->SetCommandF(boost::bind(&CUIOption::_change_notice_log, this));
#endif
...
#ifdef NOTICE_PICKUP_ITEM
void CUIOption::_change_notice_log()
{
	if ( m_pChkItemLog != NULL)
		g_bItemLog = m_pChkItemLog->IsChecked() ? 1 : 0;
}
#endif
...
void CUIOption::_change_show_mob_bar()
{
#ifdef ENABLE_MOB_PLAYER_HP
	BOOL bOn = FALSE;
	if (m_pChkMobsHpBar != NULL)
		bOn = m_pChkMobsHpBar->IsChecked();
	
	if (bOn)
		g_iHpBarShowFlag |= SHOW_MOB_HP_BAR;
	else
		g_iHpBarShowFlag &= ~SHOW_MOB_HP_BAR;
#endif
}

void CUIOption::_change_show_player_bar()
{
#ifdef ENABLE_MOB_PLAYER_HP
	BOOL bOn = FALSE;
	if (m_pChkPlayersHpBar != NULL)
		bOn = m_pChkPlayersHpBar->IsChecked();

	if (bOn)
		g_iHpBarShowFlag |= SHOW_PLAYER_HP_BAR;
	else
		g_iHpBarShowFlag &= ~SHOW_PLAYER_HP_BAR;
#endif
}
