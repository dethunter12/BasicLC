...
//dethunter12
	...
	if (pObj != NULL)
	{
		CMobTarget* pTarget = static_cast<CMobTarget*>(pObj);
		pTarget->mob_hp = fHealth;
		pTarget->mob_maxhp = fMaxHealth;
	}

	pObj = ACTORMGR()->GetObject(eOBJ_CHARACTER, en_lNetworkID);
	if (pObj != NULL)
	{
		CCharacterTarget* pTarget = static_cast<CCharacterTarget*>(pObj);
		pTarget->cha_hp = fHealth;
		pTarget->cha_maxhp = fMaxHealth;
	}
...
void CEntity::SetTargetInfoReal( float fMaxHealth, float fHealth, int level,int Pk,int pkstate,SQUAD llCount,int pkLegit,CCharacterTarget* ct, int nType )
{
	...
	{
		targetInfo.TargetType = CHARACTER;
		targetInfo.colorName = ct->cha_color_name;
	}
	else if(IsSlave())
	{
		targetInfo.TargetType = SUMMON;
...
void CEntity::AddAnimation(INDEX iAnimID, ULONG ulFlags, FLOAT fStrength, INDEX iGroupID, INDEX iModelInstanceID /*=master*/, FLOAT fSpeedMul /*=1.0f*/) 
{
	...
	*/
	...
	_pNetwork->MyCharacterInfo.animationId = iAnimID;
	_pNetwork->MyCharacterInfo.animationFlag = ulFlags;
	_pNetwork->MyCharacterInfo.animationStrength = fStrength;
	_pNetwork->MyCharacterInfo.animationGroupId = iGroupID;
	_pNetwork->MyCharacterInfo.animation = fSpeedMul;
}  
